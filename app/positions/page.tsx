    ...
    <main className="relative min-h-full overflow-hidden rounded-[28px] border border-gold-400/16 bg-[#04060a] p-3 text-white md:p-4">
      <div className="relative z-10 space-y-3">
        <header className="panel-gold rounded-[30px] p-5 md:p-7">
          ...
          <p className={`mt-2 max-w-4xl rounded-2xl border px-4 py-3 text-[12px] leading-6 ${productionRuntime ? "border-emerald-400/25 bg-emerald-500/5 text-emerald-100/85" : "border-amber-400/25 bg-amber-500/5 text-amber-100/85"}`}>{productionRuntime ? `Production ${productionRuntime.releaseSha.slice(0, 12)} に直結中。 ` : `Production runtime未接続${productionRuntimeError ? `: ${productionRuntimeError}` : "。"} `}Q102 {q102?.selectorMode ?? config.quality102Runtime.selectorMode}は1-slotスリーブ。上限はQ102 {(caps?.quality102Gross ?? config.quality102Runtime.strategyGrossCap).toFixed(2)}x / Crypto {(caps?.cryptoGross ?? config.quality102Runtime.cryptoGrossCap).toFixed(2)}x / Total {(caps?.totalGross ?? config.quality102Runtime.totalGrossCap).toFixed(2)}x。</p>
        </header>
        ...
        <section className="grid gap-3 xl:grid-cols-3">
          <div className="panel-gold rounded-[28px] p-4"><div className="flex items-center gap-2 text-sm font-black"><Activity className="h-4 w-4 text-gold-100" />V12 X1.00 ALL / Top2 / Dynamic Residual</div><p className="mt-3 text-[12px] leading-6 text-white/78">14銘柄のAster Futures V3 LIVE runner。1建玉は最大 {(caps?.v12PerPositionGross ?? config.v12PerPositionGross).toFixed(2)}x、最大{config.v12MaximumPositions}建玉。Base合計上限 {(caps?.v12BaseGross ?? config.v12BaseGross).toFixed(2)}x、未使用余力を使うDynamic Residual込みV12最大 {(caps?.v12DynamicGross ?? config.v12Gross).toFixed(2)}x。V12・PENGU・Q102を含むCrypto共有上限は {(caps?.cryptoGross ?? config.sharedCryptoGross).toFixed(2)}x、Asterは5x Cross必須です。</p><p className="mt-2 text-[11px] leading-5 text-white/58">{config.v12Symbols.join(" / ")}</p></div>
          <div className="panel-gold rounded-[28px] p-4"><div className="flex items-center gap-2 text-sm font-black"><Layers3 className="h-4 w-4 text-gold-100" />PENGU V2 / Short V20 / Recovery V8</div><p className="mt-3 text-[12px] leading-6 text-white/78">PENGUUSDTの独立LIVE runner。通常Short V20 / Longを優先し、通常シグナル不成立時だけRecovery V8（R_BTC3、補助Long 0.50x）を評価します。allocation {(caps?.penguGross ?? config.penguGross).toFixed(2)}x、hard-stop後 cooldown {config.penguHardStopCooldownHours}h。V12・Q102とCryptoリスクを共有します。</p><p className="mt-2 text-[11px] leading-5 text-white/58">通常Gross上限 {(caps?.penguGross ?? config.penguGross).toFixed(2)}x / Short V20 / Recovery V8 / Crypto共有 {(caps?.cryptoGross ?? config.sharedCryptoGross).toFixed(2)}x / shared daily loss {caps?.sharedCryptoDailyLossPct ?? config.sharedCryptoDailyLossPct}%</p></div>
          <div className="panel-gold rounded-[28px] p-4"><div className="flex items-center gap-2 text-sm font-black"><Activity className="h-4 w-4 text-gold-100" />V52 / V11 unchanged / V50</div><p className="mt-3 text-[12px] leading-6 text-white/78">AMZN・META・MSFT・NVDA・TSLAを固定したV50候補snapshotで順位1位と2位を評価します。V50はBasis ≥{v52?.minimumEntryBasisBps ?? config.v52Top2Policy.minEntryBasisBps}bps / Convergence {v52?.convergenceBps ?? config.v52Top2Policy.convergenceBps}bps / Stop {v52?.basisStopMultiple ?? config.v52Top2Policy.basisStopMultiple}x / Net Edge ≥{v52?.minimumNetEdgeBps ?? config.v52Top2Policy.minNetEdgeBps}bps / Max Cost {v52?.maximumRoundTripCostBps ?? config.v52Top2Policy.maximumRoundTripCostBps}bps / Spread {v52?.maximumSpreadBps ?? config.v52Top2Policy.maximumSpreadBps}bpsです。</p><p className="mt-2 text-[11px] leading-5 text-white/58">Rank1 {config.v52Top2Policy.rank1RequestedGross.toFixed(2)}x / Rank2 {config.v52Top2Policy.rank2RequestedGross.toFixed(2)}x / Slot上限 {(caps?.v52V50Gross ?? config.v52V50Gross).toFixed(2)}x / 最大{config.v52Top2Policy.maxConcurrentPositions}建玉 / 日次{config.v52Top2Policy.maxDailyEntries}件 / Stock {(caps?.stockGross ?? config.v52Top2Policy.stockGrossCap).toFixed(2)}x / Global {(caps?.totalGross ?? config.v52Top2Policy.globalGrossCap).toFixed(1)}x</p><p className="mt-2 text-[10px] text-white/40">Aster 5x Cross / tradingMutation=0</p></div>
          <div className="panel-gold rounded-[28px] p-4"><div className="flex items-center gap-2 text-sm font-black"><Activity className="h-4 w-4 text-amber-200" />Q102 {q102?.selectorMode ?? config.quality102Runtime.selectorMode}</div><p className="mt-3 text-[12px] leading-6 text-white/78">{q102?.selectorMode ?? config.quality102Runtime.selectorMode}の1-slot補完スリーブ。V12・PENGU・V52の注文機会を優先し、余剰Grossだけを最大 {(caps?.quality102Gross ?? config.quality102Runtime.strategyGrossCap).toFixed(2)}x 使用します。</p><p className="mt-2 text-[11px] leading-5 text-white/58">{config.quality102Runtime.symbols.join(" / ")}</p><p className="mt-2 text-[10px] leading-5 text-amber-100/65">Family Gross: HIGH_VOL {(q102?.familyGross.HIGH_VOL ?? config.quality102Runtime.familyGross.HIGH_VOL).toFixed(3)}x / MR {(q102?.familyGross.MR ?? config.quality102Runtime.familyGross.MR).toFixed(2)}x / BRK {(q102?.familyGross.BRK ?? config.quality102Runtime.familyGross.BRK).toFixed(3)}x / REV {(q102?.familyGross.REV ?? config.quality102Runtime.familyGross.REV).toFixed(2)}x / PB {(q102?.familyGross.PB ?? config.quality102Runtime.familyGross.PB).toFixed(2)}x / Crypto {(caps?.cryptoGross ?? config.quality102Runtime.cryptoGrossCap).toFixed(2)}x / Total {(caps?.totalGross ?? config.quality102Runtime.totalGrossCap).toFixed(2)}x / Aster 5x Cross</p></div>
        </section>
