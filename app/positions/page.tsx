    ...
    <main className="relative min-h-full overflow-hidden rounded-[28px] border border-gold-400/16 bg-[#04060a] p-3 text-white md:p-4">
      <div className="relative z-10 space-y-3">
        ...
        <section className="grid gap-3 xl:grid-cols-3">
          ...
          <div className="panel-gold rounded-[28px] p-4"><div className="flex items-center gap-2 text-sm font-black"><Layers3 className="h-4 w-4 text-gold-100" />PENGU V2 / Recovery V8</div><p className="mt-3 text-[12px] leading-6 text-white/78">PENGUUSDTの独立LIVE runner。通常Short V20 / Longを優先し、通常シグナル不成立時だけRecovery V8（R_BTC3、補助Long 0.50x）を評価します。V12とCryptoリスクを共有し、重複建玉・追加注文は実runnerの安全ゲートに委譲します。</p><p className="mt-2 text-[11px] leading-5 text-white/58">通常Gross上限 {config.penguGross.toFixed(2)} / V8 {config.penguRecoveryV8.recoveryGross.toFixed(2)}x / shared daily loss {config.sharedCryptoDailyLossPct}%</p></div>
