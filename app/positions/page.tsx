    ...
    <main className="relative min-h-full overflow-hidden rounded-[28px] border border-gold-400/16 bg-[#04060a] p-3 text-white md:p-4">
      <div className="relative z-10 space-y-3">
        <header className="panel-gold rounded-[30px] p-5 md:p-7">
          <div className="flex items-center gap-2 text-[10px] font-semibold uppercase tracking-[0.3em] text-gold-100/76"><ShieldCheck className="h-4 w-4" />本番ダッシュボード</div>
          <h1 className="gold-heading mt-3 text-3xl font-black tracking-tight md:text-5xl">{config.strategyLabel}</h1>
          <p className="mt-3 max-w-4xl text-sm leading-7 text-white/82">Asterの読み取り結果から、残高・実建玉・未決済注文・保護注文を表示します。未取得の値は表示しません。</p>
          <p className="mt-2 max-w-4xl rounded-2xl border border-amber-400/25 bg-amber-500/5 px-4 py-3 text-[12px] leading-6 text-amber-100/85">Quality102 Causal V4は最新データで判定する1枠の補完ロジックです。未証明の経路は安全側で停止します。上限：Q102 {config.quality102Runtime.strategyGrossCap.toFixed(2)}x / 1-slot / Crypto {config.quality102Runtime.cryptoGrossCap.toFixed(2)}x / Total {config.quality102Runtime.totalGrossCap.toFixed(2)}x。</p>
        </header>
        ...
        <section className="grid gap-3 xl:grid-cols-3">
          <div className="panel-gold rounded-[28px] p-4"><div className="flex items-center gap-2 text-sm font-black"><Activity className="h-4 w-4 text-gold-100" />V12 X1.00 ALL Top2</div><p className="mt-3 text-[12px] leading-6 text-white/78">14銘柄を監視し、ATR・リスクで配分を決めます。最大 {config.v12Gross.toFixed(1)}x、共有Crypto上限は {config.sharedCryptoGross.toFixed(1)}xです。</p><p className="mt-2 text-[11px] text-white/58">{config.v12Symbols.join(" / ")}</p></div>
          <div className="panel-gold rounded-[28px] p-4"><div className="flex items-center gap-2 text-sm font-black"><Layers3 className="h-4 w-4 text-gold-100" />PENGU Dual LS V2 / Short V20 + Recovery V8</div><p className="mt-3 text-[12px] leading-6 text-white/78">PENGUUSDTのLong・Shortを監視し、通常シグナルを優先します。条件不成立時だけRecovery V8を評価します。共有リスクと重複注文を管理します。</p><p className="mt-2 text-[11px] text-white/58">通常 {config.penguGross.toFixed(2)}x / V8 {config.penguRecoveryV8.recoveryGross.toFixed(2)}x / Crypto共有 {config.sharedCryptoGross.toFixed(1)}x</p></div>
          <div className="panel-gold rounded-[28px] p-4"><div className="flex items-center gap-2 text-sm font-black"><Activity className="h-4 w-4 text-gold-100" />V52 Top2 / Aster-only</div><p className="mt-3 text-[12px] leading-6 text-white/78">米国株5銘柄の候補を順位付けし、Gateを通過した上位2件を評価します。市場時間外は新規判定を行いません。</p><p className="mt-2 text-[11px] text-white/58">Rank1 {config.v52Top2Policy.rank1RequestedGross.toFixed(2)}x / Rank2 {config.v52Top2Policy.rank2RequestedGross.toFixed(2)}x / 最大{config.v52Top2Policy.maxConcurrentPositions}建玉 / Stock {config.v52Top2Policy.stockGrossCap.toFixed(1)}x</p><p className="mt-2 text-[10px] text-white/40">release {config.v52ProductionReleaseSha.slice(0, 8)}… / 読み取り専用</p></div>
          <div className="panel-gold rounded-[28px] p-4"><div className="flex items-center gap-2 text-sm font-black"><Activity className="h-4 w-4 text-amber-200" />Quality102 Causal V4</div><p className="mt-3 text-[12px] leading-6 text-white/78">{config.quality102Runtime.selectorMode}の1枠補完ロジックです。V12・PENGU・V52を優先し、余剰枠だけを使います。</p><p className="mt-2 text-[11px] text-white/58">対象：{config.quality102Runtime.symbols.join(" / ")}</p><p className="mt-2 text-[10px] text-amber-100/65">未証明の経路は安全側で停止 / Q102 {config.quality102Runtime.strategyGrossCap.toFixed(2)}x / Crypto {config.quality102Runtime.cryptoGrossCap.toFixed(2)}x / Total {config.quality102Runtime.totalGrossCap.toFixed(2)}x</p></div>
        </section>
