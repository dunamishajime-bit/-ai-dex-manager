    ...
    <main className="relative min-h-full overflow-hidden rounded-[28px] border border-gold-400/16 bg-[#04060a] p-3 text-white md:p-4">
      <div className="relative z-10 space-y-3">
        ...
        <section className="grid gap-3 xl:grid-cols-3">
          ...
          <div className="panel-gold rounded-[28px] p-4"><div className="flex items-center gap-2 text-sm font-black"><Activity className="h-4 w-4 text-gold-100" />V52 Stock</div><p className="mt-3 text-[12px] leading-6 text-white/78">AMZN・META・MSFT・NVDA・TSLAの参照・発注runner。米国市場時間外は待機し、参照データ異常時はFail Closedです。</p><p className="mt-2 text-[11px] leading-5 text-white/58">Stock合計上限 {config.v52StockGross.toFixed(1)}x（V11 {config.v52V11Gross.toFixed(1)}x / V50 {config.v52V50Gross.toFixed(1)}x、最大{config.v52MaxConcurrentPositions}建玉）/ daily loss {config.v52DailyLossPct}%</p></div>
        </section>
