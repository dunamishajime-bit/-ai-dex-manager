    ...
    <main className="relative min-h-full overflow-hidden rounded-[28px] border border-gold-400/16 bg-[#04060a] p-3 text-white md:p-4">
      <div className="relative z-10 space-y-3">
        <header className="panel-gold rounded-[30px] p-5 md:p-7">
          ...
          <p className="mt-2 max-w-4xl rounded-2xl border border-amber-400/25 bg-amber-500/5 px-4 py-3 text-[12px] leading-6 text-amber-100/85">Quality102 Causal V4はLIVE時点のデータだけを使うPB / MR / BRK / REV独立スリーブとして稼働中です。歴史的固定102件はLIVEへ再生せず、V12・PENGU・V52の既存LIVE経路を常に優先します。上限はQuality102 {config.quality102Runtime.strategyGrossCap.toFixed(2)}x / Crypto {config.quality102Runtime.cryptoGrossCap.toFixed(2)}x / Total {config.quality102Runtime.totalGrossCap.toFixed(2)}x。</p>
        </header>
        ...
        <section className="grid gap-3 xl:grid-cols-3">
          ...
          <div className="panel-gold rounded-[28px] p-4"><div className="flex items-center gap-2 text-sm font-black"><Activity className="h-4 w-4 text-amber-200" />Quality102 Causal V4</div><p className="mt-3 text-[12px] leading-6 text-white/78">{config.quality102Runtime.selectorMode}の独立1-slot補完スリーブ。PB / MR / BRK / REVをLIVE時点の確定データで判定し、V12・PENGU・V52の注文機会を優先して余剰Grossだけを最大 {config.quality102Runtime.strategyGrossCap.toFixed(2)}x 使用します。</p><p className="mt-2 text-[11px] leading-5 text-white/58">{config.quality102Runtime.symbols.join(" / ")}</p><p className="mt-2 text-[10px] leading-5 text-amber-100/65">歴史的固定102件はLIVEへ不使用 / Crypto {config.quality102Runtime.cryptoGrossCap.toFixed(2)}x / Total {config.quality102Runtime.totalGrossCap.toFixed(2)}x</p></div>
        </section>
