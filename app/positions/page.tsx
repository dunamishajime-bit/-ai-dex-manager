    ...
    <main className="relative min-h-full overflow-hidden rounded-[28px] border border-gold-400/16 bg-[#04060a] p-3 text-white md:p-4">
      <div className="relative z-10 space-y-3">
        <header className="panel-gold rounded-[30px] p-5 md:p-7">
          ...
          <p className="mt-2 max-w-4xl rounded-2xl border border-amber-400/25 bg-amber-500/5 px-4 py-3 text-[12px] leading-6 text-amber-100/85">Quality102 derived HIGH_VOLはLIVE時点のデータだけを使う独立スリーブとして稼働中です。歴史的102件selector parity未証明部分とBRK strength式は該当経路のみFAIL CLOSEDで、V12・PENGU・V52の既存LIVE経路には影響させません。上限はQuality102 {config.quality102Runtime.strategyGrossCap.toFixed(2)}x / Crypto {config.quality102Runtime.cryptoGrossCap.toFixed(2)}x / Total {config.quality102Runtime.totalGrossCap.toFixed(2)}x。</p>
        </header>
        ...
        <section className="grid gap-3 xl:grid-cols-3">
          <div className="panel-gold rounded-[28px] p-4"><div className="flex items-center gap-2 text-sm font-black"><Activity className="h-4 w-4 text-gold-100" />V12 X1.00 ALL</div><p className="mt-3 text-[12px] leading-6 text-white/78">14銘柄のAster Futures V3 LIVE runner。要求GrossはATR/リスクで動的に決まり、最大 {config.v12Gross.toFixed(1)}x。V12・PENGU・Quality102を含むCrypto共有上限は {config.sharedCryptoGross.toFixed(1)}xです。</p><p className="mt-2 text-[11px] leading-5 text-white/58">{config.v12Symbols.join(" / ")}</p></div>
          <div className="panel-gold rounded-[28px] p-4"><div className="flex items-center gap-2 text-sm font-black"><Layers3 className="h-4 w-4 text-gold-100" />PENGU V2 / Recovery V8</div><p className="mt-3 text-[12px] leading-6 text-white/78">PENGUUSDTの独立LIVE runner。通常Short V20 / Longを優先し、通常シグナル不成立時だけRecovery V8（R_BTC3、補助Long 0.50x）を評価します。V12・Quality102とCryptoリスクを共有し、重複建玉・追加注文は実runnerの安全ゲートに委譲します。</p><p className="mt-2 text-[11px] leading-5 text-white/58">通常Gross上限 {config.penguGross.toFixed(2)} / V8 {config.penguRecoveryV8.recoveryGross.toFixed(2)}x / Crypto共有 {config.sharedCryptoGross.toFixed(1)}x / shared daily loss {config.sharedCryptoDailyLossPct}%</p></div>
          <div className="panel-gold rounded-[28px] p-4"><div className="flex items-center gap-2 text-sm font-black"><Activity className="h-4 w-4 text-gold-100" />V52 Top2 / Aster-only</div><p className="mt-3 text-[12px] leading-6 text-white/78">AMZN・META・MSFT・NVDA・TSLAを固定したV50候補snapshotで順位1位と2位を評価します。各候補はbasis・net edge・データ品質・板・容量Gateを通過した場合だけ発注経路へ進みます。米国市場時間外はrunnerを意図的に停止します。</p><p className="mt-2 text-[11px] leading-5 text-white/58">Rank1 {config.v52Top2Policy.rank1RequestedGross.toFixed(2)}x / Rank2 {config.v52Top2Policy.rank2RequestedGross.toFixed(2)}x / 最大{config.v52Top2Policy.maxConcurrentPositions}建玉 / 日次{config.v52Top2Policy.maxDailyEntries}件 / Stock {config.v52Top2Policy.stockGrossCap.toFixed(1)}x / Global {config.v52Top2Policy.globalGrossCap.toFixed(1)}x</p><p className="mt-2 text-[10px] leading-5 text-white/40">release {config.v52ProductionReleaseSha.slice(0, 8)}… / tradingMutation=0</p></div>
          <div className="panel-gold rounded-[28px] p-4"><div className="flex items-center gap-2 text-sm font-black"><Activity className="h-4 w-4 text-amber-200" />Quality102 derived HIGH_VOL</div><p className="mt-3 text-[12px] leading-6 text-white/78">{config.quality102Runtime.selectorMode}の独立1-slot補完スリーブ。V12・PENGU・V52の注文機会を優先し、余剰Grossだけを最大 {config.quality102Runtime.strategyGrossCap.toFixed(2)}x 使用します。</p><p className="mt-2 text-[11px] leading-5 text-white/58">{config.quality102Runtime.symbols.join(" / ")}</p><p className="mt-2 text-[10px] leading-5 text-amber-100/65">歴史的selector parity未証明・BRK live式未証明部分はFAIL CLOSED / Crypto {config.quality102Runtime.cryptoGrossCap.toFixed(2)}x / Total {config.quality102Runtime.totalGrossCap.toFixed(2)}x</p></div>
        </section>
