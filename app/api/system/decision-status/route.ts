...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const [v52Top2Observability, penguRuntime, quality102Runtime, v12Heartbeat, penguHeartbeat] = await Promise.all([
      ...
      loadRunnerHeartbeatObservability(
        process.env.V12_X1_ALL_HEARTBEAT_PATH || "/var/lib/disdex/runner-health/heartbeats/v12-x1-all.json",
        "V12_X1_ALL",
      ),
      loadRunnerHeartbeatObservability(
        process.env.PENGU_DUAL_LS_V2_HEARTBEAT_PATH || "/var/lib/disdex/runner-health/heartbeats/pengu-v8.json",
        "PENGU_V8",
      ),
    ]);
