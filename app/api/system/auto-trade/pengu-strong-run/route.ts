...
function acquireRunLock() {
  try {
    ...
    fs.writeFileSync(fd, JSON.stringify({ startedAt: new Date().toISOString(), pid: process.pid, trigger: "pengu_15m" }), "utf8");
    return fd;
  } catch (error) {
    ...
    try {
      ...
      if (Date.now() - stat.mtimeMs > LOCK_TTL_MS) {
        ...
        const fd = fs.openSync(LOCK_PATH, "wx");
        fs.writeFileSync(fd, JSON.stringify({ startedAt: new Date().toISOString(), pid: process.pid, trigger: "pengu_15m", staleRecovered: true }), "utf8");
        return fd;
      }
    } catch {
...
export async function POST(req: NextRequest) {
  ...
  if (lockFd === null) {
    return NextResponse.json({ ok: false, error: "Auto trade run already in progress." }, { status: 409 });
  }
  ...
  try {
    ...
    return NextResponse.json({
      ok: false,
      error: error instanceof Error ? error.message : "Failed to run PENGU 15m strong override.",
    }, { status: 500 });
  } finally {
