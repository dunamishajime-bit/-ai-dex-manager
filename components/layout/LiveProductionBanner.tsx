...
function BannerContent() {
  return (
    <div className="mx-auto flex max-w-[1600px] flex-wrap items-center gap-x-4 gap-y-2 text-[11px] text-white/80">
      ...
      <span>V12 {config.v12SizingMode} / PENGU Short V20上限 {config.penguGross.toFixed(2)}x / Crypto共有 {config.sharedCryptoGross.toFixed(1)}x</span>
      ...
      <span className="text-amber-200">Quality102 LIVE Causal V4: ≤ {config.quality102Runtime.strategyGrossCap.toFixed(2)}x / 1-slot / Crypto ≤ {config.quality102Runtime.cryptoGrossCap.toFixed(2)}x / Total ≤ {config.quality102Runtime.totalGrossCap.toFixed(2)}x</span>
      <span className="text-amber-200">歴史的102件selector / BRK raw式: parity未証明のため該当部分のみFAIL CLOSED</span>
      <span className="text-white/60">Strict BT #{config.strictBt33404708902.sourceRun} / HPはVPS runner stateを読み取り専用で表示</span>
      <LiveRuntimeBadge />
