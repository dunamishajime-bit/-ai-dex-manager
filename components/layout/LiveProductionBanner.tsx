...
function BannerContent() {
  return (
    <div className="mx-auto flex max-w-[1600px] flex-wrap items-center gap-x-4 gap-y-2 text-[11px] text-white/80">
      ...
      <span>V12 Top2 / 1件{config.v12PerPositionGross.toFixed(2)}x / Base {config.v12BaseGross.toFixed(2)}x / Dynamic込み最大 {config.v12Gross.toFixed(2)}x / PENGU上限 {config.penguGross.toFixed(2)}x / Crypto共有 {config.sharedCryptoGross.toFixed(2)}x</span>
      ...
      <span>V52 Top2 Rank1 {config.v52Top2Policy.rank1RequestedGross.toFixed(2)}x / Rank2 {config.v52Top2Policy.rank2RequestedGross.toFixed(2)}x / Slot上限 {config.v52V50Gross.toFixed(2)}x / Stock {config.v52StockGross.toFixed(2)}x / 最大{config.v52Top2Policy.maxConcurrentPositions}建玉</span>
      ...
      <span className="text-amber-200">Q102 Causal V4: 1 Slot / ≤ {config.quality102Runtime.strategyGrossCap.toFixed(2)}x / HV {config.quality102Runtime.familyGross.HIGH_VOL.toFixed(3)}x / MR {config.quality102Runtime.familyGross.MR.toFixed(2)}x / BRK {config.quality102Runtime.familyGross.BRK.toFixed(3)}x / REV/PB {config.quality102Runtime.familyGross.REV.toFixed(2)}x / Crypto ≤ {config.quality102Runtime.cryptoGrossCap.toFixed(2)}x / Total ≤ {config.quality102Runtime.totalGrossCap.toFixed(2)}x</span>
      <LiveRuntimeBadge />
