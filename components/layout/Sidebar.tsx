...
const navItems = [
    ...
    { icon: BrainCircuit, label: "AI Council", href: "/ai-agents" },
    { icon: Clock, label: "Council History", href: "/ai-agents/history" },
    { icon: Bot, label: "TraderBrain", href: "/trader-brain" },
    { icon: Newspaper, label: "News", href: "/news" },
    { icon: Bell, label: "Notifications", href: "/notifications" },
    { icon: Crosshair, label: "Positions", href: "/positions" },
    { icon: Layers, label: "Strategy", href: "/strategy" },
    { icon: BarChart3, label: "Performance", href: "/performance" },
    { icon: Activity, label: "Trade History", href: "/history" },
    { icon: Star, label: "Watchlist", href: "/watchlist" },
    { icon: Play, label: "Demo Setup", href: "/demo" },
                    ...
                    {navItems.map((item, idx) => {
                        ...
                        return (
                            <Link
                                ...
                                onClick={() => setMobileOpen(false)}
                                className={cn(
                                    "group relative overflow-hidden rounded-lg px-3 py-1.5 text-xs transition-all duration-200 stagger-item btn-micro",
                            ...
                            >
                                ...
                                <item.icon className={cn("h-4 w-4 transition-colors", isActive ? "text-gold-500" : "text-gray-600 group-hover:text-gray-400")} />
                                <span className="truncate">{item.label}</span>
                                {isActive && <div className="ml-auto h-1.5 w-1.5 rounded-full bg-gold-500 shadow-[0_0_5px_rgba(255,215,0,0.5)]" />}
                            </Link>
                        );
                    })}
                </nav>
