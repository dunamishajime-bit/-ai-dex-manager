...
type V12Observability = {
  ...
  runnerState: {
    ...
    manualReview?: string;
    activePositions?: Array<{ symbol?: string; side?: string; quantity?: number; gross?: number; entryPrice?: number; entrySignalTs?: number; holdingBars?: number }>;
...
function V12Detail({ details }: { details?: V12Observability }) {
  ...
  const decisionLabel = decision ? (decision.candidates.length ? (decision.symbol || "候補") + " " + (decision.side || "WAIT") : "候補なし") : "候補未取得";
  ...
  const runnerActives = runner?.activePositions?.length ? runner.activePositions : runner?.active ? [runner.active] : [];
      ...
      <div className="mt-4 grid gap-4 xl:grid-cols-[1.15fr_0.85fr]">
        ...
        <div className="rounded-2xl border border-white/10 bg-black/20 p-3">
          ...
          <div className="mt-3 space-y-2 text-xs leading-5 text-white/75">
            ...
            <p>建玉：{runnerActives.length ? runnerActives.map((active, index) => `R${index + 1} ${active.symbol || "—"} ${active.side || "—"} / gross ${number(active.gross, 3)}x`).join(" / ") : "なし"}</p>
    ...
    <section className="panel-gold rounded-[28px] p-4 md:p-5">
      ...
      <div className="mt-4 grid gap-4 xl:grid-cols-[0.85fr_1.15fr]">
        ...
        <div className="rounded-2xl border border-amber-400/20 bg-amber-500/5 p-3 text-xs leading-5 text-amber-100/85">
          ...
          <p className="mt-2">上限：Q102 1.00x / 1-slot / Crypto 2.00x / Total 2.50x。主力の注文を優先します。</p>
        </div>
      </div>
    </section>
  );
