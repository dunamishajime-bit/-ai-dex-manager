          ...
          <article key={item.symbol} className="rounded-2xl border border-white/10 bg-black/20 p-3 md:p-4">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                ...
                <div><div className="font-bold text-white">{item.symbol}</div><div className="text-xs text-white/50">{item.side === "LONG" ? "ロング" : item.side === "SHORT" ? "ショート" : "待機"}</div></div>
              </div>
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-white/10 bg-white/[0.03] px-4 py-3 text-xs text-white/60">
        <span className="flex items-center gap-2"><ShieldCheck className="h-4 w-4 text-emerald-300" />注文・取消・建玉変更なし / 読み取り専用</span>
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        ...
        <Sleeve title="V52 Stock 判定状況" items={snapshot.v52.items} marketLabel={`${snapshot.v52.marketLabel} / ${snapshot.v52.marketOpen ? "取引時間内" : "対象時間外"}`} />
      </div>
    </div>
  );
}
