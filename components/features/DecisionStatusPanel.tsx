...
function candidateOrderStatus(candidate: CandidateDetail, decision: NonNullable<V12Observability["decision"]>) {
  if ((candidate.rank || 99) > 2) return "Top2外";
  if (decision.selectionConfirmed && candidate.symbol === decision.symbol && candidate.side === decision.side) return "発注Signal選定済み";
  if (candidate.signalGate?.code === "BTC_REGIME_OR_ENTRY_QUALITY_BLOCKED") return "候補順位のみ / BTC・Entry品質Gate未達";
  if (candidate.signalGate?.status === "blocked") return "候補順位のみ / Runner Signal Gate未達";
  if (candidate.signalGate?.status === "unknown") return "候補順位のみ / Runner Gate未取得";
  if (candidate.signalGate?.status === "pass") return "Runner Signal Gate通過候補 / 未選定";
  return "候補順位のみ / 発注Signal未成立";
}
...
function RuntimeSummary({ runtime }: { runtime: Snapshot["runtime"] }) {
  ...
  return <section className="panel-gold min-w-0 rounded-[28px] p-4 md:p-5"><div className="flex flex-wrap items-center justify-between gap-3"><div className="flex min-w-0 items-center gap-2 break-words text-lg font-bold text-white"><ServerCog className="h-5 w-5 text-gold-100" />VPS実稼働ロジック</div><span className={`max-w-full whitespace-normal break-words rounded-full border px-3 py-1 text-center text-xs font-semibold ${badgeClass}`}>{badge}</span></div><p className="mt-2 text-xs leading-5 text-white/55">確認時刻：{time(runtime.checkedAt)} / runnerごとに実stateの更新時刻・mode・保護状態を判定します。1つのrunnerが要確認でも、他runnerのLIVE状態は独立して表示します。ここから発注操作は行いません。</p><div className="mt-4 grid gap-3 xl:grid-cols-3">{runtime.units.map((unit) => <article key={unit.id} className="min-w-0 rounded-2xl border border-white/10 bg-black/20 p-3"><div className="flex items-start justify-between gap-2"><div className="min-w-0"><div className="break-words font-bold text-white">{unit.label}</div><div className="mt-1 text-[11px] text-white/45">{unit.venue} / {unit.timeframe}</div></div><span className={`rounded-full border px-2 py-1 text-[10px] font-semibold ${runtimeStatusClass(unit.status)}`}>{unit.status === "UNAVAILABLE" ? "未取得" : unit.status === "UNCONFIRMED" ? "未確認" : unit.status === "STALE" ? "要確認" : "LIVE"}</span></div><div className="mt-3 space-y-2 text-xs leading-5 text-white/72"><p><span className="text-white/45">判定：</span>{unit.entryPolicy}</p><p><span className="text-white/45">保護：</span>{unit.protection}</p><p className="text-white/50">{unit.note}</p><p className="break-all text-amber-100/80">状態根拠：{unit.reason || "未取得"}</p></div><div className="mt-3 break-all border-t border-white/10 pt-2 text-[10px] text-white/40">release {shortSha(unit.releaseSha)}… / state更新 {time(unit.updatedAt)}</div></article>)}</div></section>;
}
    ...
    <section className="panel-gold rounded-[28px] p-4 md:p-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="flex min-w-0 items-center gap-2 break-words text-lg font-bold text-white"><Activity className="h-5 w-5 text-gold-100" />V12 X1.00 ALL Top2 判定</div>
          <p className="mt-1 text-xs text-white/55">VPSの最新runnerデータから、候補・Gate・発注状況を表示します。V12はRunnerのsignalEligible / signalReasonを正本とし、HP側でSignal Gateを再計算しません。</p>
        </div>
      ...
      <div className="mt-4 grid gap-4 xl:grid-cols-[1.15fr_0.85fr]">
        ...
        <div className="min-w-0 rounded-2xl border border-white/10 bg-black/20 p-3">
          ...
          <div className="mt-3 break-all space-y-2 text-xs leading-5 text-white/75">
...
function V52Top2Detail({ details, marketOpen }: { details?: V52Top2Observability; marketOpen?: boolean }) {
  ...
  return (
    <section className="panel-gold rounded-[28px] p-4 md:p-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="flex min-w-0 items-center gap-2 break-words text-lg font-bold text-white"><Activity className="h-5 w-5 text-gold-100" />V52 Top2 判定</div>
      ...
      <div className="mt-4 rounded-2xl border border-gold-400/20 bg-gold-500/5 p-4">
        ...
        <p className="mt-1 text-xs leading-6 text-white/60">窓内retry対象：{policy.retryableReasons.join(" / ")}。最終拒否：{policy.finalRejectReasons.join(" / ")}。V50最大保有 {policy.maxHoldingHours}h。Stock Gross上限 {policy.stockGrossCap.toFixed(1)}x / Global Gross上限 {policy.globalGrossCap.toFixed(1)}x。</p>
      </div>
      <div className="mt-4 break-all rounded-xl border border-white/10 bg-black/20 px-3 py-3 text-xs leading-5 text-white/70">V52 runner state更新：{time(details.updatedAt)} / LIVE判定理由：{details.reason || "未取得"} / reference={details.referenceStatus || "未取得"} / reference health={details.referenceHealth ? (details.referenceHealth.ready ? "ready" : "blocked / " + details.referenceHealth.reason) : "未接続"} / orders={details.referenceOrdersAllowed === true ? "許可条件内" : "Fail Closed"}。キー未設定・state stale・reference品質未達のいずれでも実注文へ進みません。</div>
...
function Quality102Detail({ details }: { details?: Quality102RuntimeStatus }) {
  ...
  return (
    <section className="panel-gold rounded-[28px] p-4 md:p-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="flex min-w-0 items-center gap-2 break-words text-lg font-bold text-white"><Activity className="h-5 w-5 text-gold-100" />Quality102 Causal V4</div>
          <p className="mt-1 text-xs text-white/55">主力3ロジックを優先する1 slotの補完スリーブです。</p>
        </div>
        <div className="flex flex-wrap gap-2"><span className={`max-w-full whitespace-normal break-words rounded-full border px-3 py-1 text-center text-xs font-semibold ${statusClass}`}>Q102 {statusLabel}</span><span className="rounded-full border border-emerald-400/30 bg-emerald-500/10 px-3 py-1 text-xs font-semibold text-emerald-100">tradingMutation=0</span></div>
      </div>
      ...
      <div className="mt-4 grid gap-4 xl:grid-cols-[0.85fr_1.15fr]">
        <div className="min-w-0 break-all rounded-2xl border border-white/10 bg-black/20 p-3 text-xs leading-5 text-white/75">
...
function Sleeve({ title, items, marketLabel }: { title: string; items: DecisionStatusItem[]; marketLabel?: string }) {
  return <section className="panel-gold rounded-[28px] p-4 md:p-5"><div className="flex flex-wrap items-center justify-between gap-3"><div className="flex min-w-0 items-center gap-2 break-words text-lg font-bold text-white"><Activity className="h-5 w-5 text-gold-100" />{title}</div>{marketLabel ? <div className="text-xs text-white/55">{marketLabel}</div> : null}</div><p className="mt-2 text-xs leading-5 text-white/50">最新データの候補一覧です。</p><div className="mt-4 space-y-2">{items.map((item) => <article key={item.symbol} className="min-w-0 rounded-2xl border border-white/10 bg-black/20 p-3 md:p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div className="flex items-center gap-3"><span className={`flex h-8 min-w-8 items-center justify-center rounded-full border px-2 text-sm font-bold ${rankClass(item.rank)}`}>{item.rank || "-"}</span><div><div className="font-bold text-white">{item.symbol}</div><div className="text-xs text-white/50">{item.side === "LONG" ? "ロング候補" : item.side === "SHORT" ? "ショート候補" : "待機"} / {item.sleeve === "V12" ? `score ${item.score.toFixed(4)}` : `basis ${item.score.toFixed(2)}bps`}</div></div></div><span className={`max-w-full whitespace-normal break-words rounded-full border px-3 py-1 text-center text-xs font-semibold ${statusClass(item.status)}`}>{item.status}</span></div><p className="mt-3 break-words text-sm leading-6 text-white/80">理由：{item.reason}</p><div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-[11px] text-white/45"><span>データ：{time(item.dataUpdatedAt)}</span><span>確認：{time(item.checkedAt)}</span></div></article>)}</div></section>;
}
