...
type V52Details = {
  ...
  referenceHealth?: { ready: boolean; reason: string; status?: string; connected?: boolean; provider?: string; feed?: string; maximumQuoteAgeMs?: number; staleSymbols?: Array<{ symbol: string; ageMs: number }> };
  killSwitchActive: boolean;
...
function FinalGate({ trace, detail, state: stateOverride }: { trace?: Trace; detail?: string; state?: UiDecisionState }) {
  ...
  const detailState: UiDecisionState | undefined = detail && /拒否|阻止|blocked|reject|fail|失敗|insufficient|below|not[_ ]allowed|denied|gate/i.test(detail) ? "BLOCKED" : undefined;
...
function V52Detail({ details: sourceDetails }: { details?: V52Details }) {
  ...
  const referenceDetail = details.referenceHealth && !details.referenceHealth.ready
    ? `V52発注Gate停止：${details.referenceHealth.reason}${details.referenceHealth.staleSymbols?.length ? ` / stale: ${details.referenceHealth.staleSymbols.map((item) => `${item.symbol} ${item.ageMs}ms`).join(", ")}` : ""}`
    : details.killSwitchActive
      ? "V52共有Kill Switchが有効です。"
    : details.referenceStatus || "V52の参照状態は未取得です。";
  const gateBlocked = details.killSwitchActive || details.referenceOrdersAllowed === false || details.referenceHealth?.ready === false;
  return <section className="panel-gold rounded-[26px] p-4 md:p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><div className="flex items-center gap-2"><Activity className="h-5 w-5 text-gold-100" /><h2 className="text-xl font-black">V52 Top2 / STOCK</h2></div><p className="mt-1 text-[11px] text-white/45">株式候補の実telemetryと発注判断</p></div><div className="flex items-center gap-2"><RuntimeBadge status={details.status} /><span className="rounded-full border border-emerald-300/25 bg-emerald-400/8 px-3 py-1 text-[10px] font-bold text-emerald-100">tradingMutation=0</span></div></div>{details.status === "LIVE" && gateBlocked ? <div className="mt-4 rounded-xl border border-amber-300/25 bg-amber-400/8 px-3 py-2 text-xs leading-5 text-amber-100"><span className="font-bold">Runner LIVE / 発注Gate BLOCKED</span>：Runnerは稼働していますが、安全Gateが発注を停止しています。{referenceDetail}</div> : null}<div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-4"><div className="rounded-xl border border-white/10 bg-black/20 p-3"><div className="text-[10px] text-white/40">市場区分</div><div className="mt-1 font-bold text-cyan-100">EQUITY / STOCK</div></div><div className="rounded-xl border border-white/10 bg-black/20 p-3"><div className="text-[10px] text-white/40">建玉</div><div className="mt-1 font-bold text-white">{details.activeV50Slots}</div></div><div className="rounded-xl border border-white/10 bg-black/20 p-3"><div className="text-[10px] text-white/40">reference / Gate</div><div className={`mt-1 font-bold ${details.referenceHealth && !details.referenceHealth.ready ? "text-amber-200" : "text-white"}`}>{details.referenceStatus || "未取得"} / {details.referenceHealth?.ready ? "ready" : "blocked"}</div><div className="mt-1 text-[10px] leading-4 text-white/45">{details.referenceOrdersAllowed === false ? referenceDetail : "発注許可条件を確認済み"}</div></div><div className="rounded-xl border border-white/10 bg-black/20 p-3"><div className="text-[10px] text-white/40">Kill Switch</div><div className={`mt-1 font-bold ${details.status === "UNAVAILABLE" ? "text-amber-200" : details.killSwitchActive ? "text-rose-200" : "text-emerald-200"}`}>{details.status === "UNAVAILABLE" ? "未確認" : details.killSwitchActive ? "ACTIVE" : "inactive"}</div></div></div><div className="mt-4 space-y-2">{details.windows.map((window) => <details key={window.window} className="rounded-2xl border border-white/10 bg-black/20 p-3"><summary className="flex cursor-pointer list-none flex-wrap items-center gap-2 text-sm font-bold text-white"><ChevronDown className="h-4 w-4 text-gold-100" />{window.window} NY<span className="text-[11px] font-normal text-white/45">候補 {window.candidates.length} / retry {window.transientRetryCount}</span></summary><div className="mt-3 space-y-2">{window.candidates.map((candidate, index) => <div key={`${candidate.symbol || "candidate"}-${index}`} className="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-white/8 bg-black/20 px-3 py-2 text-xs"><span className="font-bold text-white">R{candidate.candidateRank ?? "—"} / {candidate.symbol || "—"}</span><span className="text-white/60">basis {number(candidate.basisBps, 2)}bps</span><StateBadge state="WATCH" /></div>)}{!window.candidates.length ? <div className="text-xs text-white/45">この窓の候補snapshotはありません。</div> : null}</div></details>)}</div><div className="mt-4"><FinalGate detail={gateBlocked ? referenceDetail : rows[rows.length - 1]?.orderBlockedReason || rows[rows.length - 1]?.orderResult || "V52の発注判断telemetryはまだありません。"} /></div>{rows.length ? <div className="mt-4 overflow-x-auto rounded-2xl border border-white/10 bg-black/20"><table className="min-w-[980px] w-full text-left text-xs"><thead className="text-white/45"><tr><th className="px-3 py-2">窓</th><th className="px-3 py-2">候補</th><th className="px-3 py-2">要求→配分</th><th className="px-3 py-2">空き/Global</th><th className="px-3 py-2">送信</th><th className="px-3 py-2">結果/阻止理由</th></tr></thead><tbody>{rows.slice(-12).reverse().map((row, index) => <tr key={`${row.window}-${row.symbol || "row"}-${index}`} className="border-t border-white/5"><td className="px-3 py-2 text-white/75">{row.window} NY</td><td className="px-3 py-2 font-bold text-white">R{row.candidateRank ?? "—"} / {row.symbol || "—"}</td><td className="px-3 py-2 text-white/70">{number(row.requestedGross, 2)}x → {number(row.allocatedGross, 2)}x</td><td className="px-3 py-2 text-white/70">{number(row.availableGrossBeforeEntry, 2)}x / {number(row.globalGrossBeforeReservation, 2)}x</td><td className="px-3 py-2 text-white/70">{row.orderSendAttempted ? "attempted" : "なし"}</td><td className="px-3 py-2 text-rose-100">{row.orderResult || row.orderBlockedReason || row.rank2RejectedReason || "—"}</td></tr>)}</tbody></table></div> : null}{details.errors.length ? <div className="mt-3 rounded-xl border border-amber-300/25 bg-amber-400/8 px-3 py-2 text-xs text-amber-100">観測上の注意：{details.errors.join(" / ")}</div> : null}</section>;
}
