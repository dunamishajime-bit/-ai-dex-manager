...
type V52Row = { candidateRank?: number; qualifiedRank?: number; symbol?: string; requestedGross?: number; allocatedGross?: number; availableGrossBeforeEntry?: number; globalGrossBeforeReservation?: number; globalGrossAfterReservation?: number; rank2Accepted?: boolean | null; rank2RejectedReason?: string | null; orderBlockedReason?: string | null; orderSendAttempted?: boolean; orderResult?: string };
...
function PenguDetail({ details }: { details?: PenguDetails }) {
  if (!details) return <section className="rounded-2xl border border-dashed border-white/12 px-4 py-7 text-center text-sm text-white/50">PENGUの実Runner詳細を取得できません。</section>;
  ...
  const directionState = (eligible?: boolean) => eligible === true ? "SIGNAL" as const : eligible === false ? "OFF" as const : "ERROR" as const;
  const directions = [
    { direction: "LONG" as const, state: directionState(signal?.decision.longEligible), stageLabel: signal?.decision.longEligible ? details.executionTrace.currentStageLabel : "条件未成立", blocker: signal?.decision.longEligible === false ? blocker : undefined, detail: signal?.decision.longEligible ? "PENGU runnerのLong条件が成立しています。" : "PENGU runnerからLong成立を確認できません。" },
    { direction: "SHORT" as const, state: directionState(signal?.decision.shortEligible), stageLabel: signal?.decision.shortEligible ? details.executionTrace.currentStageLabel : "条件未成立", blocker: signal?.decision.shortEligible === false ? blocker : undefined, detail: signal?.decision.shortEligible ? "PENGU runnerのShort条件が成立しています。" : "PENGU runnerからShort成立を確認できません。" },
  ];
...
function V52Detail({ details }: { details?: V52Details }) {
  if (!details) return <section className="rounded-2xl border border-dashed border-white/12 px-4 py-7 text-center text-sm text-white/50">V52の実Runner詳細を取得できません。</section>;
  const rows = details.windows.flatMap((window) => [...window.entries, ...window.rejections].map((row) => ({ ...row, window: window.window })));
