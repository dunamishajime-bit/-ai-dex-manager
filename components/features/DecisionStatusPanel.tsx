  ...
  return (
    <section className="panel-gold rounded-[28px] p-4 md:p-5">
      ...
      <div className="mt-4 rounded-2xl border border-gold-400/20 bg-gold-500/5 p-4">
        ...
        <p className="mt-1 text-xs leading-6 text-white/60">V50単一契約：Basis ≥{bps(details.thresholds.basisBps)} / Convergence {bps(details.thresholds.convergenceBps)} / Stop {details.thresholds.stopMultiple.toFixed(2)}x / Net Edge ≥{bps(details.thresholds.netEdgeBps)} / Max Cost {bps(details.thresholds.maxCostBps)} / Spread {bps(details.thresholds.maxSpreadBps)}。窓内retry対象：{policy.retryableReasons.join(" / ")}。最終拒否：{policy.finalRejectReasons.join(" / ")}。Stock Gross上限 {policy.stockGrossCap.toFixed(1)}x / Global Gross上限 {policy.globalGrossCap.toFixed(1)}x。</p>
      </div>
    ...
    <section className="panel-gold rounded-[28px] p-4 md:p-5">
      ...
      <div className="mt-4 grid gap-4 xl:grid-cols-[0.85fr_1.15fr]">
        ...
        <div className="rounded-2xl border border-amber-400/20 bg-amber-500/5 p-3 text-xs leading-5 text-amber-100/85">
          ...
          <p className="mt-2">Q102はCausal V4・最大1.50x・1 Slot、Crypto最大3.00x、Total最大3.50x。主力発火時はQ102だけを残余Grossまで縮小し、主力の注文機会をblockしません。</p>
        </div>
      </div>
    </section>
  );
