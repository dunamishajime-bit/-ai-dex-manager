...
function PenguDetail({ details }: { details?: PenguRuntimeStatus }) {
  ...
  return <section className="panel-gold rounded-[28px] p-4 md:p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><div className="flex items-center gap-2 text-lg font-bold text-white"><Activity className="h-5 w-5 text-gold-100" />PENGU V2 / Short V20 発火経路</div><p className="mt-1 text-xs text-white/55">実PENGU runner-live.jsonの確定1時間足・Long/Short判定・共有Gate・注文Windowを読み取り表示</p></div><div className="flex flex-wrap gap-2"><span className={`rounded-full border px-3 py-1 text-xs font-semibold ${penguStatusClass(details.status)}`}>PENGU {details.status === "LIVE" ? "稼働確認済み" : details.status === "STALE" ? "要確認" : "状態未取得"}</span><span className="rounded-full border border-emerald-400/30 bg-emerald-500/10 px-3 py-1 text-xs font-semibold text-emerald-100">tradingMutation=0</span></div></div><div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-8">{[["判定段階", trace.currentStageLabel], ["確定足", time(signal?.referenceTs)], ["Side", signal?.side === 1 ? "LONG" : signal?.side === -1 ? "SHORT" : "WAIT"], ["Long", boolLabel(decision?.longEligible)], ["Short", boolLabel(decision?.shortEligible)], ["target gross", signal?.targetGross === undefined ? "—" : `${number(signal.targetGross, 3)}x`], ["volumeRatio", number(feature.volumeRatio6OverPrior36, 3)], ["BTC 24h", `${number(feature.btcReturn24h, 2)}%`]].map(([label, value]) => <div key={label} className="rounded-xl border border-white/10 bg-black/20 px-3 py-2"><div className="text-[10px] uppercase tracking-wide text-white/45">{label}</div><div className={`mt-1 break-words text-sm font-semibold ${value === "未成立" || value === "WAIT" ? "text-rose-200" : "text-white"}`}>{value}</div></div>)}</div><div className="mt-4 rounded-2xl border border-gold-400/20 bg-gold-500/5 p-4"><div className="text-sm font-bold text-gold-100">現在の配分仕様</div><p className="mt-2 text-xs leading-6 text-white/75">Longは基準0.75x × 1.25 = 最大0.9375x、Short V20は基準0.75x × 1.00 = 最大0.75x。確定1時間足でLong/Short条件が成立し、共有risk・建玉容量・entry window・account lock・最小Notional・同一注文防止を通過した場合だけAsterへ進みます。</p><p className="mt-1 text-xs leading-6 text-white/60">PENGU/BTC足の時刻ずれ・取得失敗はfail closedで発注しません。直近失敗履歴：{details.failures.length ? `${details.failures.length}件` : "なし"}</p></div><div className="mt-4 grid gap-4 xl:grid-cols-[1.15fr_0.85fr]"><div className="rounded-2xl border border-white/10 bg-black/20 p-3"><div className="text-sm font-bold text-white">発火から約定まで</div><p className="mt-2 text-sm leading-6 text-white/80">{trace.summary}</p><p className="mt-2 text-xs leading-5 text-gold-100">次の判定：{trace.nextAction}</p><div className="mt-3 space-y-2">{trace.steps.map((step) => <div key={step.key} className={`flex items-start gap-2 rounded-xl border px-3 py-2 text-xs ${penguTraceStateClass(step.state)}`}><TraceIcon state={step.state === "pass" ? "pass" : step.state === "blocked" ? "blocked" : step.state === "pending" ? "pending" : "unknown"} /><div><div className="font-semibold">{step.label}</div><div className="mt-0.5 leading-5 opacity-85">{step.detail}</div></div></div>)}</div></div><div className="rounded-2xl border border-white/10 bg-black/20 p-3"><div className="text-sm font-bold text-white">実データ詳細</div><div className="mt-3 space-y-2 text-xs leading-5 text-white/75"><p>runner更新：{time(details.updatedAt)}</p><p>最新判定理由：{signal?.reason || "未取得"}</p><p>BTC EMA168距離：{number(feature.btcEma168Distance, 4)}</p><p>relativeReturn24h：{number(feature.relativeReturn24h, 4)}</p><p>ATR24 ratio：{number(feature.atr24Ratio, 4)}</p><p>RSI14：{number(feature.rsi14, 2)}</p><p>建玉：{details.position ? `side ${details.position.side ?? "—"} qty ${number(details.position.quantity, 6)}` : "なし"}</p><p>pending：{details.pending ? `${details.pending.phase || "ORDER"} / ${details.pending.reason || "—"}` : "なし"}</p></div></div></div>{details.failures.length ? <div className="mt-4 overflow-x-auto rounded-2xl border border-white/10 bg-black/20"><div className="border-b border-white/10 px-3 py-2 text-sm font-bold text-white">直近fail-closed履歴</div><table className="min-w-[760px] w-full text-left text-xs"><thead className="text-white/45"><tr><th className="px-3 py-2">時刻</th><th className="px-3 py-2">停止理由</th></tr></thead><tbody>{details.failures.slice().reverse().map((failure, index) => <tr key={`${failure.occurredAt}-${index}`} className="border-t border-white/5"><td className="px-3 py-2 text-white/70">{time(failure.occurredAt)}</td><td className="px-3 py-2 text-rose-200">{failure.message}</td></tr>)}</tbody></table></div> : null}{details.reason ? <div className="mt-3 rounded-xl border border-amber-400/25 bg-amber-500/10 px-3 py-2 text-xs leading-5 text-amber-100">観測理由：{details.reason}</div> : null}</section>;
}
