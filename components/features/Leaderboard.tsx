                ...
                {leaderboard.map((user) => (
                    <div key={`${user.rank}-${user.name}`} className="flex items-center justify-between rounded-lg border border-white/5 bg-black/20 px-4 py-3">
                        <div>
                            ...
                            <div className="text-[10px] text-gray-500">Daily PnL {user.dailyProfit.toLocaleString("ja-JP")}</div>
                        </div>
                        <div className={`text-sm font-bold ${user.dailyChange >= 0 ? "text-emerald-400" : "text-rose-400"}`}>
