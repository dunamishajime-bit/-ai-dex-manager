...
function safeDecisionSnapshot(value: unknown) {
  ...
  const selectionConfirmed = typeof row.selectionConfirmed === "boolean" ? row.selectionConfirmed : typeof row.symbol === "string" && typeof row.side === "string";
...
export function sanitizeV12RunnerState(value: unknown) {
  const row = asObject(value);
  if (!row) return null;
  const active = asObject(row.active);
  const activePositions = (Array.isArray(row.activePositions) ? row.activePositions : active ? [active] : [])
    .map(asObject).filter((position): position is JsonObject => Boolean(position)).map((position) => ({
      symbol: typeof position.symbol === "string" ? position.symbol : undefined,
      side: typeof position.side === "string" ? position.side : undefined,
      quantity: Number.isFinite(Number(position.quantity)) ? Number(position.quantity) : undefined,
      gross: Number.isFinite(Number(position.gross)) ? Number(position.gross) : undefined,
      entryPrice: Number.isFinite(Number(position.entryPrice)) ? Number(position.entryPrice) : undefined,
      entrySignalTs: Number.isFinite(Number(position.entrySignalTs)) ? Number(position.entrySignalTs) : undefined,
      holdingBars: Number.isFinite(Number(position.holdingBars)) ? Number(position.holdingBars) : undefined,
    }));
...
function buildExecutionTrace(
  ...
  runnerState: ReturnType<typeof sanitizeV12RunnerState>,
  ...
  if (decision && decision.candidates.length === 0) {
    return {
      currentStage: "candidate-none",
      currentStageLabel: "この確定2時間足では候補なし",
      summary: "V12 decision snapshotは正常取得済みですが、この確定2時間足ではSignal候補がありません。",
      nextAction: "次の確定2時間足でTop2候補を再評価します。",
      steps: [{ key: "candidate", label: "1. 候補選定", state: "blocked" as const, detail: "snapshot正常 / 候補0件" }],
    };
  }
  if (!decision?.symbol) {
    return {
      ...
      nextAction: "snapshot配線または次回のV12確定足評価を確認します。",
...
export async function loadV12DecisionObservability() {
  ...
  const runnerState = sanitizeV12RunnerState(runnerFile.value);
