...
function isFresh(value: unknown, now: number): boolean {
  const parsed = parseRuntimeTimestamp(value);
  return parsed !== null && parsed <= now + 60_000 && now - parsed <= SNAPSHOT_MAX_AGE_MS;
}
...
function buildV52Items(
  ...
  if (!market.open) {
    return config.stockSymbols.map((symbol) => outsideMarketItem(symbol, checkedAt, market.label));
  }
  if (!serviceActive) {
    return config.stockSymbols.map((symbol) => stoppedItem(symbol, "V52", checkedAt));
  }
  ...
  if (!decision || !isFresh(decisionUpdatedAt, now) || !decisionItems) {
    return config.stockSymbols.map((symbol) => unavailableItem(
      symbol,
      "V52",
      checkedAt,
      "V52実Runnerの判定スナップショットが欠損または古いため、発火候補を推測表示しません。",
      iso(decisionUpdatedAt),
    ));
  }
...
export async function loadDecisionStatus(): Promise<DecisionStatusSnapshot> {
  ...
  const v52Items = buildV52Items(service.active, checkedAt, now, market);
  v96Items.forEach((item, index) => { item.rank = index + 1; });
  v52Items.forEach((item, index) => { item.rank = index + 1; });
