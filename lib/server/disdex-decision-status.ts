    ...
    units: [
      {
        ...
        note: "VPSの最新stateを確認できた場合だけLIVE表示します。過去データから推測表示しません。",
      ...
      },
      {
        ...
        entryPolicy: "確定1時間足でShort V20 / Base Longを優先し、通常シグナル不成立時だけRecovery V8補助Longを評価",
        protection: "Recovery V8 hard stop -6%・24h partial/cooldown・trail・max hold 72h。Crypto 2.00x / Total 2.50x",
        note: "VPSの最新stateを確認できた場合だけLIVE表示します。過去データから推測表示しません。",
        reason: "PENGU runnerの状態を確認中です。",
      },
      {
        id: "QUALITY102_CAUSAL_V1",
        label: "Quality102 Causal V4 1.0x / 1-slot",
        status: "UNCONFIRMED",
        releaseSha: config.vpsObservedReleases.quality102,
        venue: "Aster Futures crypto sleeve",
        timeframe: "LIVE時点の利用可能データのみ",
        entryPolicy: "Causal V4で判定する1.00x / 1-slot補完ロジック。主力を優先し、余剰枠だけを使用",
...
export function v12ItemsFromSnapshot(state: JsonObject, checkedAt: string): DecisionStatusItem[] {
  ...
  if (!candidates.length) {
    ...
    return config.v12Symbols.map((symbol) => ({
      symbol, sleeve: "V12" as const, rank: 0, score: 0, scoreMax: 1,
      status: "条件不足" as const, side: "WAIT" as const,
      reason: "この確定2時間足では候補なし。snapshot取得自体は正常です。次の確定2時間足で再評価します。",
      checkedAt, source: "VPS V12 sanitized decision snapshot",
