/**
 * Account Store - アカウント別データ管理
 * localStorage/IndexedDB ベース。アカウントごとに完全独立したAI状態を保持。
 */
...
function emitAuthChanged() {
    if (!isBrowser) return;
    window.dispatchEvent(new CustomEvent("jdex-auth-changed"));
}
...
export interface UserProfile {
    ...
    securitySettings?: {
        ...
        updatedAt?: number;
        requireAllMethods?: boolean; // legacy
    };
