...
export async function reserveAsterGlobalRateSlot(options: AsterGlobalRateSlotOptions) {
  ...
  try {
    ...
    if (current.schema && current.schema !== ASTER_GLOBAL_RATE_BUDGET_SCHEMA) {
    ...
    const state: BudgetState = {
      ...
      nextAllowedAt: permitAt + minIntervalMs,
      updatedAt: now,
      pid: process.pid,
    };
...
export async function waitForAsterGlobalRateSlot() {
  ...
  const slot = await reserveAsterGlobalRateSlot({ path, minIntervalMs, maxQueueMs });
