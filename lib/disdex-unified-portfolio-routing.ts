...
export function planUnifiedPortfolio(intents: PortfolioIntent[], active: ActivePortfolioPosition[] = []): PortfolioPlan {
    ...
    for (const intent of [...intents].sort((a, b) => sortPriority(a) - sortPriority(b) || a.symbol.localeCompare(b.symbol))) {
        ...
        const sleeveCap = intent.sleeve === "V12" ? STRICT_BT33404708902.v12PerPositionGrossCap : intent.sleeve === "PENGU_DUAL_LS_V2" ? V12_PENGU_V52_PORTFOLIO_POLICY.penguMaximumGross : 1;
        ...
        const v12Residual = intent.sleeve === "V12" ? Math.max(0, V12_PENGU_V52_PORTFOLIO_POLICY.v12MaximumGross - v12Gross) : Number.POSITIVE_INFINITY;
        const planned = { ...intent, gross: Math.min(gross, remaining, v12Residual) };
        if (!(planned.gross > 0)) { rejected.push({ intent, reason: "V12_AGGREGATE_CAPACITY_BLOCKED" }); continue; }
