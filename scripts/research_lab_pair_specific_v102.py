...
# V102 mechanisms intentionally differ from V100/V101 families.
CANDIDATES={
...
def signal(mech,s,candles,idx,ts,cfg):
    ...
    # explicit no-trade state for chaotic, low-efficiency extremes
    ...
    if mech=='tail_state_normalization':
        ...
        # after a heavy-tail downside state normalizes, require local recovery and non-collapsing breadth
        if ko>1.0 and so<-.35 and kn<.65 and sn>-.20 and r3>0 and r24<0 and br>=.34:
            return (1,24,.78)
        # symmetric euphoric-tail normalization for short
        if ko>1.0 and so>.35 and kn<.65 and sn<.20 and r3<0 and r24>0 and br<=.66:
            return (-1,18,.68)

    elif mech=='breadth_divergence_router':
        # pair relative divergence from six-asset median, with state-dependent continuation/reversion choice
        m6=median_move(candles,idx,ts,6); m24=median_move(candles,idx,ts,24)
        d6=r6-m6; d24=r24-m24
        rp=range_position(c,i,96)
        ...
        rp=range_position(c,i,96); prev=range_position(c,i-12,96)
        vr=v24/max(v96,1e-9)
        # escape from a mature range only when path efficiency and vol expansion agree
        ...
        if prev>.18 and rp<.08 and r6<0 and eff>.30 and .85<vr<2.25 and br<=.55:
            return (-1,24,.82)
        # failed escape enters opposite side only after immediate reversal
        if rp>.90 and r3<-.45 and eff<.22:return (-1,12,.50)
        if rp<.10 and r3>.45 and eff<.22:return (1,12,.50)

    elif mech=='vol_cluster_reversal':
        # identify persistent high-vol cluster then trade stabilization reversal, not static oversold
        q=[]
        for k in range(8):
            j=i-k*12
            if j<96:break
        ...
        persistent=sum(x>1.25 for x in q)>=5
        calming=q[0]<q[1] and q[0]<1.20
        ...
        old=rr[:-24]; recent=rr[-24:]; z=sum(recent[-6:])/(sd(old)*math.sqrt(6)+1e-9)
        compression=sd(recent)/max(sd(old),1e-9)
        # dislocation after residual variance expansion, fade only if pair impulse reverses
...
def portfolio(chosen,candles,idx,start,end,cost,delay,cfg):
    ...
    for ts in sorted(byts):
        ...
        if not xs:continue
        # portfolio allocator caps simultaneous concentration
...
def main():
    ...
    if (dm.get('pf') or 0)<1.05 or (dm.get('returnPct') or 0)<=0 or (vm.get('pf') or 0)<1.05 or (vm.get('returnPct') or 0)<=0:
        ...
        if not gate(cm,cs):result.update(status='FAIL',reason='CONFIRMATION')
            ...
            shares=[abs(v) for v in yc.values()]; concentration=max(shares)/sum(shares) if sum(shares)>1e-9 else 1.0
    ...
    out=Path(os.environ.get('RESEARCH_AUTONOMOUS_STATE_DIR','.research-state')); out.mkdir(parents=True,exist_ok=True); stem=f"pair-specific-v102-{a.arch.lower()}"; txt=json.dumps(result,indent=2)
