...
This fetcher therefore requires 100% daily archive/header availability, never
interpolates missing metrics, and gates on >=99.5% hourly coverage with no gap
longer than 12 hours. Each symbol runs in its own GitHub job to avoid the
...
def fetch_day(symbol: str, day: dt.date) -> dict[str,Any]:
    ...
    for attempt in range(RETRIES):
        try:
            ...
            if not raw_rows: raise RuntimeError('EMPTY_METRICS_DAY')
            hourly={}
            for row in sorted(raw_rows,key=lambda x:x['ts']):
                hour=int(row['ts']//3_600_000*3_600_000)
                hourly[hour]=row
            return {'day':day.isoformat(),'available':True,'rawRows':len(raw_rows),'hourly':[dict(v,hourTs=k) for k,v in sorted(hourly.items())]}
...
def build(symbol: str, out_root: Path) -> dict[str,Any]:
    ...
    hourly={}; partial=[]
    for r in rows:
        if int(r['rawRows'])!=288: partial.append({'day':r['day'],'rawRows':int(r['rawRows'])})
    ...
    manifest={
      ...
      'partialDays':partial,'hourlyRule':'last available 5m metrics snapshot inside each UTC candle hour; no interpolation',
