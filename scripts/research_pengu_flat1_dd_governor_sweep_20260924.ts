...
async function fetchArray(url:URL){
  ...
  for(let a=0;a<6;a++){
    try{
      const res=await fetch(url,{headers:{accept:"application/json","user-agent":"DisDex-PENGU-Flat1-DD-Governor-Sweep/20260924"}});
...
function routeHardstopHours(){
  if(ddMode==="ROUTE_HARDSTOP_Q72")return 72;
...
function replay(rows:PenguDualLsV2EvaluationRow[],points:FundingPoint[],v:Variant,mode:Mode){
  ...
  while(i<rows.length-2){
    ...
    const routeHours=routeHardstopHours();
...
async function main(){
  ...
  const variants:DDMode[]=[
    ...
    "HARDSTOP_COOLDOWN48",
    "COMBINED_PROTECT_COOLDOWN48",
    "ROUTE_HARDSTOP_Q72","ROUTE_HARDSTOP_Q120","ROUTE_HARDSTOP_Q168",
    "SIDE_HARDSTOP_Q72","SIDE_HARDSTOP_Q120",
    "LOSS2_GLOBAL_Q72","LOSS2_GLOBAL_Q120",
    "ROUTE2LOSS14D_Q168",
    "DD8_Q168","DD10_Q168","DD12_Q168",
    "ROUTE120_LOSS2_Q72","ROUTE120_DD10_Q168","SMART_GOVERNOR"
  ];
  const result:any={
    schema:"pengu-flat1-dd-governor-sweep/v2",
  ...
  await fs.mkdir(".research-state/pengu-flat1-dd-governor-sweep",{recursive:true});
  await fs.writeFile(".research-state/pengu-flat1-dd-governor-sweep/result.json",JSON.stringify(result,null,2)+"\n");
  ...
  console.log("PENGU_FLAT1_DD_GOVERNOR_SWEEP="+JSON.stringify(compact));
  console.log("PENGU_FLAT1_DD_GOVERNOR_CONTRACT_PASS="+JSON.stringify({gross:1,logic:"COMBINED_FILTERED",variants:variants.length,baselineParity:true,researchOnly:true}));
}
