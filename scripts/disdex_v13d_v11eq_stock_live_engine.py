...
REFERENCE_QUALITY_ERROR_CODES = {
    ...
    "reference_http_unavailable",
    "reference_transport_unavailable",
}


class ReferenceQualityError(RuntimeError):
    """A reference-data rejection that blocks one tick but is retryable."""
    ...
    def __init__(self, code: str, detail: Optional[dict] = None):
        ...
        allowed = {
            "symbol", "ageMs", "maximumAgeMs", "confidenceBps",
            "maximumConfidenceBps", "crossSourceDifferenceBps",
            "maximumCrossSourceBps", "httpStatus",
        }
        self.detail = {key: value for key, value in (detail or {}).items() if key in allowed}
        super().__init__(self.summary())

    def summary(self) -> str:
        return json.dumps({"error": self.code, **self.detail}, ensure_ascii=False, separators=(",", ":"))
    ...
    try:
        ...
        if reference_request:
            raise ReferenceQualityError(
                "reference_transport_unavailable",
                {"httpStatus": getattr(getattr(error, "reason", None), "errno", None)},
            ) from error
        raise
