...
function route(sig:any,idx:number){
 ...
 if(slice.length!==V12_X1_ALL.btcRegimeSmaBars||!sb?.[idx])return "UNKNOWN";
...
async function main(){
 for(const sym of V12_X1_ALL.universe){
   try{universe[sym]=resampleV12H1ToH2(await h1(sym));}
   catch(e){console.error("FETCH_FAIL",sym,String(e));universe[sym]=[];}
 }
