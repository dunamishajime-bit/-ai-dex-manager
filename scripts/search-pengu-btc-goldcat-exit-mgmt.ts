...
const CACHE_DIR = path.join(process.cwd(), ".cache", "pengu-goldcat-btc15m");
const REPORT_DIR = path.join(process.cwd(), "reports", "pengu-btc-goldcat-exit-mgmt");
...
function runTrade(candles1m: Candle[], side: "long" | "short", signalTs: number, variant: Variant) {
  const entry = executionPrice(candles1m, signalTs);
  if (!entry) return null;
  const stop = side === "long" ? entry.price * (1 - variant.stopLossPct) : entry.price * (1 + variant.stopLossPct);
  const take = side === "long" ? entry.price * (1 + variant.takeProfitPct) : entry.price * (1 - variant.takeProfitPct);
...
function buildVariants() {
  ...
  const holdMinutes = [12, 15, 20, 25, 30];
  const stopLossPct = [0.003, 0.004, 0.005, 0.006];
  const takeProfitPct = [0.01, 0.012, 0.015, 0.018, 0.02, 0.025];
  const trailActivationPct = [0.006, 0.008, 0.01, 0.012];
  const trailRetracePct = [0.003, 0.004, 0.005, 0.006];
      ...
      for (const take of takeProfitPct) {
        for (const trailAct of trailActivationPct) {
          for (const trailRet of trailRetracePct) {
            ...
            variants.push({
              key: `h${hold}_sl${stop}_tp${take}_ta${trailAct}_tr${trailRet}`,
              holdMinutes: hold,
              stopLossPct: stop,
              takeProfitPct: take,
...
async function main() {
  ...
  const candles1m = await loadCachedBinance(EXECUTION_SYMBOL, "1m", startMs, endMs);

  const results = buildVariants().map((variant) => simulate(variant, rows, candles1m)).sort((a, b) => b.pnlUsd - a.pnlUsd);
  ...
  await fs.writeFile(path.join(REPORT_DIR, "result.md"), [
    "# PENGU BTC GoldCat Exit Mgmt Search",
    "",
