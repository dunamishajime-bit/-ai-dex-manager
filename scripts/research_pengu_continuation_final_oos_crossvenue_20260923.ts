...
const CROSS_VENUE = String(process.env.PENGU_CROSS_VENUE || "OKX").toUpperCase();
...
let CONTINUATION_TRAIL_ACTIVATION = PENGU_DUAL_LS_V2.long.trailingActivationPct;
let CONTINUATION_TRAIL_RETRACE = PENGU_DUAL_LS_V2.long.trailingRetracePct;
...
interface Trade {
  ...
  partialDefense?:boolean; partialAccountReturn?:number; openAtWindowEnd?:boolean; mfePct?:number; maePct?:number;
...
function replay(rows:PenguDualLsV2EvaluationRow[],points:FundingPoint[],v:Variant,mode:Mode){
  ...
  while(i<rows.length-2){
    ...
    if(route==="RECOVERY_V8"){
      ...
      if(route.startsWith("TRAIL_")&&side==="L"){
        let previousBest=entry.open;
        for(let j=entryIndex;j<=last;j++){
          ...
          const trailing=previousBest*(1-CONTINUATION_TRAIL_RETRACE);
          if(previousBest/entry.open-1>=CONTINUATION_TRAIL_ACTIVATION&&ff.low<=trailing){exitIndex=j;exitPrice=trailing;exitReason="LONG_TRAILING_STOP";break;}
          previousBest=Math.max(previousBest,ff.high);
          if(j===naturalLast){exitIndex=j;exitPrice=rows[j].candle.close;exitReason="LONG_MAX_HOLD";break;}
        }
      }else{
    ...
    const pathBars=rows.slice(entryIndex,exitIndex+1).map(x=>x.candle);
    const mfePct=pathBars.length?Math.max(...pathBars.map(x=>x.high))/entry.open-1:0;
    const maePct=pathBars.length?Math.min(...pathBars.map(x=>x.low))/entry.open-1:0;
    const trade:Trade={variant:v,mode,route,side,signalTs:rows[i].candle.openTime,entryTs:entry.openTime,exitTs:rows[exitIndex].candle.openTime,entryPrice:entry.open,exitPrice,requestedGross:gross,accountReturn,rawUnitReturn,fundingUnitReturn,costUnitReturn,exitReason,entryFeatures:{...f},partialDefense,partialAccountReturn,openAtWindowEnd:exitReason==="WINDOW_END",mfePct:mfePct*100,maePct:maePct*100,continuationSourceExitTs:usedArm?.exitTs,continuationSourceExitPrice:usedArm?.exitPrice};
    trades.push(trade);
    arm=undefined;
...
async function main(){
  ...
  const windows=[
    {label:"DISCOVERY",start:Date.parse("2024-08-10T00:00:00Z"),end:Date.parse("2025-06-01T00:00:00Z")},
    {label:"OOS_2025H2",start:Date.parse("2025-06-01T00:00:00Z"),end:Date.parse("2026-01-01T00:00:00Z")},
    {label:"OOS_2026_TO_AUG10",start:Date.parse("2026-01-01T00:00:00Z"),end:Date.parse("2026-08-10T00:00:00Z")},
    {label:"RECENT_HOLDOUT",start:Date.parse("2026-08-10T00:00:00Z"),end:RECENT_END},
  ];
  const result:any={schema:"pengu-continuation-final-candidate-oos/v1",venue:CROSS_VENUE,candidate:{armOnlyAfter:"profitable BASE_V64_LONG LONG_TRAILING_STOP",armHours:24,ret6Min:.03,distanceAtrMax:.50,ret24Min:.10,ret72Min:.15,ret72Max:.40,relative24Min:.05,btc24Min:0,rsiMin:55,rsiMax:90,trailActivation:.06,trailRetrace:.03,hardStop:PENGU_DUAL_LS_V2.long.hardStopPct,maxHoldHours:PENGU_DUAL_LS_V2.long.maxHoldHours},availableRange:[new Date(pengu[0].openTime).toISOString(),new Date(pengu.at(-1)!.openTime+HOUR).toISOString()],windows:{}};
  for(const mode of ["NORMAL","SEVERE"] as Mode[]){
    const base=replay(rows,fp,"BASELINE",mode),candidate=replay(rows,fp,"TRAIL_MOMENTUM",mode);
    for(const w of windows){
      const btr=between(base,w.start,w.end),ctr=between(candidate,w.start,w.end);
      result.windows[w.label]??={start:new Date(w.start).toISOString(),end:new Date(w.end).toISOString()};
      result.windows[w.label][mode]={
        baseline:metrics(btr),candidate:metrics(ctr),continuation:continuationLedger(ctr),
        deltaReturnPct:metrics(ctr).returnPct-metrics(btr).returnPct,
        deltaMarkedReturnPct:metrics(ctr).markToWindowReturnPct-metrics(btr).markToWindowReturnPct,
        deltaWinRatePct:(metrics(ctr).winRatePct??0)-(metrics(btr).winRatePct??0),
        deltaPf:(metrics(ctr).profitFactor??0)-(metrics(btr).profitFactor??0),
        deltaDdPct:metrics(ctr).maxDrawdownPct-metrics(btr).maxDrawdownPct,
      };
    }
  }
  const outDir=`.research-state/pengu-continuation-final-oos-cross-${CROSS_VENUE.toLowerCase()}`;await fs.mkdir(outDir,{recursive:true});await fs.writeFile(path.join(outDir,"result.json"),JSON.stringify(result,null,2)+"\n");
  console.log("PENGU_CONTINUATION_FINAL_OOS="+JSON.stringify(result));
}
