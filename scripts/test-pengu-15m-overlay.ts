...
const REPORT_DIR = path.join(process.cwd(), "reports", "pengu-15m-overlay");
...
async function main() {
  ...
  const variants: Array<{ key: string; memo: string; options: HybridVariantOptions }> = [
    {
      key: "baseline_2023_current",
      memo: "Current production-equivalent baseline over the 15m-available window.",
      options: { ...production, label: "baseline_2023_current" },
    },
    {
      key: "pengu_15m_score_rising_priority",
      memo: "PENGU score-rising logic using 15m strict-extra decision checks.",
      options: {
        ...production,
        ...
        strictExtraTrendRotationScoreGapBySymbol: { PENGU: 10 },
        strictExtraTrendRotationCurrentMomAccelMax: 0.01,
        strictExtraTrendRotationCurrentMom20Max: 0.12,
        strictExtraTrendRotationCandidateMinScore: 24,
        ...
        strictExtraTrendRotationRequireConsecutiveBarsBySymbol: { PENGU: 2 },
        strictExtraTrendRotationMinHoldBars: 1,
        label: "pengu_15m_score_rising_priority",
      },
    },
    {
      key: "pengu_15m_idle_breakout",
      memo: "USDT waiting only: PENGU 15m breakout early pick.",
      options: {
        ...production,
        ...
        idleBreakoutMinVolumeRatio: 1.25,
        ...
        idleBreakoutProfitTrailActivationPct: 0.10,
        idleBreakoutProfitTrailRetracePct: 0.045,
        idleBreakoutMaxHoldBars: 96,
        label: "pengu_15m_idle_breakout",
      },
    },
    {
      key: "pengu_15m_hold_extension",
      memo: "PENGU holding only: use 15m exit checks and strict guard to avoid weak rotations.",
      options: {
        ...production,
        strictExtraTrendDecisionTimeframe: "15m",
        strictExtraTrendExitCheckTimeframe: "15m",
        ...
        strictExtraTrendSwitchGuardMinCurrentScore: 28,
        strictExtraTrendSwitchGuardMinCurrentMom20: 0.06,
        strictExtraTrendSwitchGuardMinCurrentMomAccel: 0.001,
        strictExtraTrendSwitchGuardMinCurrentEfficiencyRatio: 0.18,
        strictExtraTrendSwitchGuardRequiredScoreGap: 12,
        strictExtraTrendSwitchGuardMode: "all",
        label: "pengu_15m_hold_extension",
      },
    },
  ];
  ...
  const markdown = [
    "# PENGU 15m Overlay Backtest",
    "",
    "## Setup",
    "",
    `- Start: ${new Date(START_TS).toISOString()}`,
    `- End: ${new Date(END_TS).toISOString()}`,
    "- Method: engine-connected 15m PENGU overlay. Binance 15m remote cache is used.",
    "",
    "## Summary",
    "",
