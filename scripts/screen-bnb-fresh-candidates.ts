...
const REPORT_DIR = path.join(process.cwd(), "reports", "bnb-fresh-candidates");
const BASE_SYMBOLS = ["ETH", "SOL", "AVAX"] as const;
const CANDIDATES = ["SHIB", "PEPE", "CAKE", "TWT", "FTM", "EOS", "AXS", "ALPACA", "DODO", "BCH", "MATIC"] as const;

async function main() {
    await fs.mkdir(REPORT_DIR, { recursive: true });

    const baseline = await runExpandedUniverseBacktest({
        label: "current_production_reference",
