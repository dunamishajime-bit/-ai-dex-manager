...
WARM=pd.Timestamp('2025-07-20T00:00:00Z')
EVAL_START=pd.Timestamp('2025-08-10T00:00:00Z')
EVAL_END=pd.Timestamp('2026-08-10T00:00:00Z')
OUT='research/pengu-v2-final-bybit-frozen'
FEE=.0006

# Frozen before Bybit is opened. Do not tune from this run.
SHORT=dict(impulse=-.07,expiry=24,bounce_min=.0125,bounce_max=.06,p24_floor=-.12,
           btcema_floor=-.04,rsi_min=30,vol_min=.25,vol_max=3,rel_max=-.02,btc24_max=.04)
LONG=dict(look=18,p24_min=.10,rel_min=.01,btc24_min=0.,rsi_min=48,rsi_max=78,
          vol_min=.25,vol_max=3,atr_max=.05)
EXIT=dict(short_hold=72,short_hard=.08,short_trig=.15,short_trail=.04,
          long_hold=120,long_hard=.08,long_trig=.10,long_trail=.03,cooldown=6)
...
def get(path,params,retries=6):
    ...
    for k in range(retries):
        try:
            with urlopen(Request(u,headers={'User-Agent':'Mozilla/5.0 PENGU-V2-FinalFrozen'}),timeout=30) as r:
...
def klines(symbol,st,en):
    rows=[];cur=en-1
    while cur>=st:
        j=get('/v5/market/kline',{'category':'linear','symbol':symbol,'interval':'60','start':st,'end':cur,'limit':1000})
        x=j.get('result',{}).get('list',[])
        if not x: break
        rows+=x; oldest=min(int(v[0]) for v in x); new=oldest-1
        if new>=cur: break
        cur=new
        if oldest<=st: break
        time.sleep(.03)
    ...
    lo=pd.Timestamp(st,unit='ms',tz='UTC');hi=pd.Timestamp(en,unit='ms',tz='UTC')
    return d[(d.t>=lo)&(d.t<hi)][['t','open','high','low','close','volume']].drop_duplicates('t').sort_values('t').reset_index(drop=True)
...
def prep(p,b):
    ...
    d['ema72']=c.ewm(span=72,adjust=False,min_periods=72).mean();d['ema168']=c.ewm(span=168,adjust=False,min_periods=168).mean()
    ...
    d['hi18']=d.high.shift(1).rolling(18,min_periods=18).max()
...
def short_sig(d):
    ...
    for i in range(180,len(d)):
        ...
        if active:
            ...
            if armed:
                ok=(d.close.iat[i]<d.low.iat[i-1] and d.close.iat[i]<d.ema72.iat[i] and
                    d.ema72.iat[i]<d.ema168.iat[i] and d.rel24.iat[i]<=s['rel_max'] and
                    s['vol_min']<=d.volr.iat[i]<=s['vol_max'] and d.btc24.iat[i]<=s['btc24_max'] and
                    d.p24.iat[i]>=s['p24_floor'] and d.btcema168.iat[i]>=s['btcema_floor'] and d.rsi.iat[i]>=s['rsi_min'])
                if ok: sig[i]=True;active=False;armed=False;lowv=0.
    return sig

def long_sig(d):
    s=LONG
    x=(d.close>d.hi18)&(d.p24>=s['p24_min'])&(d.rel24>=s['rel_min'])&(d.btc24>=s['btc24_min'])&(
       d.rsi>=s['rsi_min'])&(d.rsi<=s['rsi_max'])&(d.volr>=s['vol_min'])&(d.volr<=s['vol_max'])&(
       d.atr24<=s['atr_max'])&(d.close>d.ema168)
...
def sim(d,stress=0.,delay=0):
    ...
    while i<len(d)-2:
        ...
        if ei>=len(d): break
        et=d.t.iat[ei]
        entry=float(d.open.iat[ei]);gross=float(np.clip(.75*RISK['target']/float(d.atr24.iat[i]),RISK['gross_floor'],RISK['gross_cap']))
        ...
        if EVAL_START<=et<EVAL_END:
            tr.append({'side':side,'entryTime':str(et),'exitTime':str(d.t.iat[ex]),'pnl':pnl,'gross':gross,'reason':reason})
        cd=ex+EXIT['cooldown'];i=ex+1
    return tr

def met(t):
    if not t:return dict(trades=0,ret=0,pf=0,dd=0,win=0,long=0,short=0)
    v=np.array([x['pnl'] for x in t]);e=np.cumprod(1+v);pk=np.maximum.accumulate(e);gp=v[v>0].sum();gl=-v[v<0].sum()
    return dict(trades=len(v),long=sum(x['side']=='L' for x in t),short=sum(x['side']=='S' for x in t),ret=float((e[-1]-1)*100),pf=float(gp/gl if gl else 999),dd=float(np.min(e/pk-1)*100),win=float((v>0).mean()*100))
...
def folds(t,n=4):
    ...
    for k in range(n):
        ...
        out.append({'start':str(a),'end':str(b),**met([x for x in t if a<=pd.Timestamp(x['entryTime'])<b])})
    return out

def main():
    os.makedirs(OUT,exist_ok=True);st=ms(WARM);en=ms(EVAL_END)
    p=klines('PENGUUSDT',st,en);b=klines('BTCUSDT',st,en)
    if len(p)==0 or len(b)==0: raise RuntimeError('Bybit returned no candles')
    d=prep(p,b);base=sim(d);res={'strategy':'PENGU_DUAL_LS_V2_FINAL_FROZEN','source':'Bybit linear USDT perpetual untouched external validation','eval':{'start':str(EVAL_START),'endExclusive':str(EVAL_END)},'short':SHORT,'long':LONG,'exit':EXIT,'risk':RISK,'data':{'rows':len(d),'first':str(d.t.iloc[0]),'last':str(d.t.iloc[-1])},'base':met(base),'stress35bpsSide':met(sim(d,.0035)),'delay1h':met(sim(d,0,1)),'stress35_delay1h':met(sim(d,.0035,1)),'waves':waves(d,base),'folds':folds(base,4)}
    print('RESULT_JSON');print(json.dumps(res,indent=2));json.dump(res,open(OUT+'/result.json','w'),indent=2);pd.DataFrame(base).to_csv(OUT+'/trades.csv',index=False)
