...
OUT='research/pengu-v2-final-bitget-frozen';FEE=.0006
SHORT=dict(ret72_max=0.0,impulse=-.07,expiry=24,bounce_min=.0125,bounce_max=.06,p24_floor=-.12,btcema_floor=-.04,rsi_min=30,vol_min=.25,vol_max=3,rel_max=-.02,btc24_max=.04)
LONG=dict(ret72_min=.15,look=18,p24_min=.10,rel_min=.01,btc24_min=0.,rsi_min=48,rsi_max=78,vol_min=.25,vol_max=3,atr_max=.05)
...
def get(path,params,retries=6):
 ...
 for k in range(retries):
  try:
   with urlopen(Request(u,headers={'Accept':'application/json','User-Agent':'Mozilla/5.0 PENGU-V2-FinalFrozen'}),timeout=30) as r:j=json.loads(r.read().decode())
...
def klines(symbol,start,end):
 rows=[];cur=int(start.timestamp()*1000);endms=int(end.timestamp()*1000);step=199*3600*1000
 while cur<endms:
  to=min(endms-1,cur+step)
  x=get('/api/v2/mix/market/history-candles',{'symbol':symbol,'productType':'usdt-futures','granularity':'1H','startTime':cur,'endTime':to,'limit':200})
  rows.extend(x or []);cur=to+1;time.sleep(.04)
 if not rows:return pd.DataFrame()
 d=pd.DataFrame(rows,columns=['t','open','high','low','close','volume','quoteVolume'])
...
def prep(p,b):
 ...
 d['rsi']=rsi(c);d['btc24']=d.btc/d.btc.shift(24)-1;d['p24']=c/c.shift(24)-1;d['rel24']=d.p24-d.btc24;d['ret72']=c/c.shift(72)-1
 ...
 prev=c.shift(1);tr=pd.concat([d.high-d.low,(d.high-prev).abs(),(d.low-prev).abs()],axis=1).max(axis=1);d['atr24']=tr.rolling(24,min_periods=24).mean()/c;d['hi18']=d.high.shift(1).rolling(18,min_periods=18).max();return d.reset_index(drop=True)
def short_sig(d):
 ...
 for i in range(180,len(d)):
  ...
  if active:
   ...
   if armed:
    ok=(d.ret72.iat[i]<=s['ret72_max'] and d.close.iat[i]<d.low.iat[i-1] and d.close.iat[i]<d.ema72.iat[i] and d.ema72.iat[i]<d.ema168.iat[i] and d.rel24.iat[i]<=s['rel_max'] and s['vol_min']<=d.volr.iat[i]<=s['vol_max'] and d.btc24.iat[i]<=s['btc24_max'] and d.p24.iat[i]>=s['p24_floor'] and d.btcema168.iat[i]>=s['btcema_floor'] and d.rsi.iat[i]>=s['rsi_min'])
    if ok:sig[i]=True;active=False;armed=False;lowv=0.
 return sig
def long_sig(d):
 s=LONG;x=(d.ret72>=s['ret72_min'])&(d.close>d.hi18)&(d.p24>=s['p24_min'])&(d.rel24>=s['rel_min'])&(d.btc24>=s['btc24_min'])&(d.rsi>=s['rsi_min'])&(d.rsi<=s['rsi_max'])&(d.volr>=s['vol_min'])&(d.volr<=s['vol_max'])&(d.atr24<=s['atr_max'])&(d.close>d.ema168)
...
def folds(t,n=4):
 ...
 return out
def main():
 os.makedirs(OUT,exist_ok=True);p=klines('PENGUUSDT',WARM,EVAL_END);b=klines('BTCUSDT',WARM,EVAL_END)
 if len(p)==0 or len(b)==0:raise RuntimeError('Bitget returned no candles')
 d=prep(p,b);base=sim(d);res={'strategy':'PENGU_DUAL_LS_V2_FINAL_FROZEN','source':'Bitget USDT perpetual untouched external validation','eval':{'start':str(EVAL_START),'endExclusive':str(EVAL_END)},'short':SHORT,'long':LONG,'exit':EXIT,'risk':RISK,'data':{'pRows':len(p),'bRows':len(b),'rows':len(d),'first':str(d.t.iloc[0]),'last':str(d.t.iloc[-1])},'base':met(base),'stress35bpsSide':met(sim(d,.0035)),'delay1h':met(sim(d,0,1)),'stress35_delay1h':met(sim(d,.0035,1)),'waves':waves(d,base),'folds':folds(base,4)};print('RESULT_JSON');print(json.dumps(res,indent=2));json.dump(res,open(OUT+'/result.json','w'),indent=2);pd.DataFrame(base).to_csv(OUT+'/trades.csv',index=False)
