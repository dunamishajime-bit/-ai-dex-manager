...
OUT='research/pengu-v2-final-kucoin-frozen';FEE=.0006

# FINAL candidate frozen before KuCoin data is opened. Do not tune from this run.
...
def get(path,params,retries=6):
    ...
    for k in range(retries):
        try:
            ...
            if j.get('code')!='200000':raise RuntimeError(j)
            return j.get('data',[])
...
def folds(t,n=4):
    ...
    return out
def main():
    os.makedirs(OUT,exist_ok=True);p=klines('PENGUUSDTM',WARM,EVAL_END);b=klines('XBTUSDTM',WARM,EVAL_END)
    if len(p)==0 or len(b)==0:raise RuntimeError('KuCoin returned no candles')
    d=prep(p,b);base=sim(d);res={'strategy':'PENGU_DUAL_LS_V2_FINAL_FROZEN','source':'KuCoin USDT perpetual untouched external validation','eval':{'start':str(EVAL_START),'endExclusive':str(EVAL_END)},'short':SHORT,'long':LONG,'exit':EXIT,'risk':RISK,'data':{'rows':len(d),'first':str(d.t.iloc[0]),'last':str(d.t.iloc[-1])},'base':met(base),'stress35bpsSide':met(sim(d,.0035)),'delay1h':met(sim(d,0,1)),'stress35_delay1h':met(sim(d,.0035,1)),'waves':waves(d,base),'folds':folds(base,4)};print('RESULT_JSON');print(json.dumps(res,indent=2));json.dump(res,open(OUT+'/result.json','w'),indent=2);pd.DataFrame(base).to_csv(OUT+'/trades.csv',index=False)
