...
const variants:Variant[]=[
  ...
  {name:"HC175_ER_CHOP_BLOCK",...LOCKED,marketMode:"ER_CHOP"},
  {name:"HC175_STATE_BLOCK",...LOCKED,marketMode:"STATE_BLOCK"},
  {name:"HC175_STATE_SIZE075",...LOCKED,marketMode:"STATE_SIZE075"},
  {name:"HC175_CLUSTER_4H",...LOCKED,clusterWindowBars:2,clusterPauseBars:2},
  {name:"HC175_CLUSTER_6H",...LOCKED,clusterWindowBars:2,clusterPauseBars:3},
  {name:"HC175_STATE_CLUSTER_4H",...LOCKED,marketMode:"STATE_BLOCK",clusterWindowBars:2,clusterPauseBars:2},
  {name:"HC175_STATE_SIZE075_CLUSTER_4H",...LOCKED,marketMode:"STATE_SIZE075",clusterWindowBars:2,clusterPauseBars:2},
  {name:"HC175_STATE_SIZE075_CLUSTER_4H_RECOVER",...LOCKED,marketMode:"STATE_SIZE075",clusterWindowBars:2,clusterPauseBars:2,clusterTrendRecovery:true},
  {name:"HC175_ER_EXHAUST_BLOCK",...LOCKED,narrowMode:"ER_BLOCK"},
  {name:"HC175_ER_EXHAUST_HALF",...LOCKED,narrowMode:"ER_HALF"},
  {name:"HC175_LAG6_BLOCK",...LOCKED,narrowMode:"LAG6_BLOCK"},
  {name:"HC175_LAG6_HALF",...LOCKED,narrowMode:"LAG6_HALF"},
  {name:"HC175_WEAK_FLOW_BLOCK",...LOCKED,narrowMode:"WEAK_BLOCK"},
  {name:"HC175_WEAK_FLOW_HALF",...LOCKED,narrowMode:"WEAK_HALF"},
  {name:"HC175_ERLAG_BLOCK",...LOCKED,narrowMode:"ERLAG_BLOCK"},
  {name:"HC175_ERLAG_HALF",...LOCKED,narrowMode:"ERLAG_HALF"},
  {name:"HC175_ANY_WEAK_BLOCK",...LOCKED,narrowMode:"ANY_BLOCK"},
  {name:"HC175_ANY_WEAK_HALF",...LOCKED,narrowMode:"ANY_HALF"},
];
const modes: Mode[] = [
...
async function main(){
  ...
  for(const v of variants){removed[v.name]={};for(const m of modes){
    ...
    removed[v.name][m.name]={removedTrades:gone.length,removedWins:gone.filter((x:any)=>x.net>0).length,removedLosses:gone.filter((x:any)=>x.net<0).length,removedNetPnl:gone.reduce((a:number,x:any)=>a+x.net,0)};
  }}
  const out={status:"PASS_RESEARCH_ONLY_HC175_EXHAUSTION_WEAKFLOW",limitations:["V12 research simulator only; no PENGU/Q102/V52/FET shared gross replay","Market-state thresholds are research candidates and require robustness/holdout validation","Locked HC condition and 1.75x multiplier remain unchanged; HC bypasses market-state and loss-cluster gates"],period:{start:new Date(START).toISOString(),end:new Date(END).toISOString()},conditions:{initialJpy:10000,monthlyJpy:10000,monthlyCount:12,totalContributionJpy:130000},productionConstants:V12_X1_ALL,dataSource:d.source,variants,results,removed,rankingNormal:[...rows].sort((a,b)=>b.NORMAL.finalEquity-a.NORMAL.finalEquity).map(x=>x.variant)};
  const focus=variants.filter(v=>v.name==="HC175_LOCKED"||v.name.includes("EXHAUST")||v.name.includes("LAG6")||v.name.includes("WEAK_FLOW")||v.name.includes("ERLAG")||v.name.includes("ANY_WEAK")).map(v=>({variant:v.name,NORMAL:{finalEquity:results[v.name].NORMAL.finalEquity,winRatePct:results[v.name].NORMAL.winRatePct,profitFactor:results[v.name].NORMAL.profitFactor,maxDrawdownPct:results[v.name].NORMAL.maxDrawdownPct,tradeCount:results[v.name].NORMAL.tradeCount,clusterTrips:results[v.name].NORMAL.clusterTrips,...removed[v.name].NORMAL},SEVERE:{finalEquity:results[v.name].SEVERE.finalEquity,winRatePct:results[v.name].SEVERE.winRatePct,profitFactor:results[v.name].SEVERE.profitFactor,maxDrawdownPct:results[v.name].SEVERE.maxDrawdownPct,tradeCount:results[v.name].SEVERE.tradeCount,clusterTrips:results[v.name].SEVERE.clusterTrips,...removed[v.name].SEVERE}}));
