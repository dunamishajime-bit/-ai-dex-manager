...
async function fetchArray(url:URL){
  ...
  for(let a=0;a<6;a++){
    try{
      const res=await fetch(url,{headers:{accept:"application/json","user-agent":"DisDex-PENGU-Wave-Cover-Stage2/20260924"}});
...
async function main(){
  ...
  const variants:Variant[]=["BASELINE","LONG_PAIR","SHORT_BLOWOFF_RAW","SHORT_BLOWOFF_FILTERED","COMBINED_RAW","COMBINED_FILTERED"];
  const windows=[{name:"FORMAL",start:FORMAL_START,end:FORMAL_END},{name:"ROLLING365",start:rollingStart,end:RECENT_END}];
  const result:any={
    schema:"pengu-wave-cover-stage2/v1",
    source:{venue:"ASTER_FUTURES_V3",productionSourceSha:process.env.PRODUCTION_SOURCE_SHA||null},
    selectedFromStage1:{
      long:["LONG_SHORT_EXIT_FLIP","LONG_CONT"],
      shortRaw:"SHORT_BLOWOFF_REVERSAL",
      shortFiltered:"SHORT_BLOWOFF_REVERSAL + BTC24<=2% + PENGU24<=6%",
      rejected:["LONG_DEEP_RECLAIM","SHORT_DIRECT_BREAKDOWN","SHORT_CONT(no signals)"]
    },
    priority:"SHORT_V20 > supplemental Short > BASE_V64_LONG > supplemental Long > Recovery V8",
    windows:{},
  ...
  for(const w of windows){
    ...
    const bn=metrics(baseN),bs=metrics(baseS);
    const out:any={start:new Date(w.start).toISOString(),end:new Date(w.end).toISOString(),baseline:{NORMAL:bn,SEVERE:bs},variants:{}};
    for(const v of variants.filter(x=>x!=="BASELINE")){
      ...
      const nm=metrics(n),sm=metrics(sv);
      out.variants[v]={
        NORMAL:nm,SEVERE:sm,
        delta:{
          normalReturnPct:nm.returnPct-bn.returnPct,severeReturnPct:sm.returnPct-bs.returnPct,
          normalWinRatePct:(nm.winRatePct??0)-(bn.winRatePct??0),severeWinRatePct:(sm.winRatePct??0)-(bs.winRatePct??0),
          normalPf:(nm.profitFactor??0)-(bn.profitFactor??0),severePf:(sm.profitFactor??0)-(bs.profitFactor??0),
          normalDdPct:nm.maxDrawdownPct-bn.maxDrawdownPct,severeDdPct:sm.maxDrawdownPct-bs.maxDrawdownPct
        },
        wave15:dualWaveSummary(rows,baseN,n,w.start,w.end),
        addedTrades:supplementalTrades(n),
        folds:w.name==="FORMAL"?{NORMAL:formalFolds(n),SEVERE:formalFolds(sv)}:undefined,
      };
    }
    result.windows[w.name]=out;
  }
  await fs.mkdir(".research-state/pengu-wave-cover-stage2",{recursive:true});
  await fs.writeFile(".research-state/pengu-wave-cover-stage2/result.json",JSON.stringify(result,null,2)+"\n");
  console.log("PENGU_WAVE_COVER_STAGE2="+JSON.stringify(result));
}
