...
STRATEGY_ID = "DISDEX_ASTER_ONLY_V24_V19_PLUS_LATE_FALLBACK"
SCENARIOS = v14.SCENARIOS
MAX_DEVELOPMENT_SURVIVORS = 30
MIN_VALIDATION_ROUTER_TRADES = 8
MIN_VALIDATION_SECONDARY_TRADES = 3
SECONDARY_CANDIDATES = tuple(
    candidate for candidate in v15.CANDIDATES
    if candidate.entry_policy == "SLOT_1330"
)
...
def build_secondary_rows(
    ...
    return [
        {
            **row,
            "strategy": "V24_LATE_SECONDARY_FALLBACK",
            "secondaryCandidateId": candidate.candidate_id,
        }
...
def layered_route(
    ...
    by_secondary = {
        str(row["day"]): row for row in secondary_rows if str(row["day"]) in allowed
    }
    events: List[dict] = []
    ...
    for day in sorted(allowed):
        ...
        secondary = by_secondary.get(day)
        if secondary is not None:
            value = v22.trade_value(secondary, cost_bps)
            if value is not None:
                events.append({
                    **secondary,
                    "netReturn": value,
                    "route": "V24_LATE_SECONDARY_FALLBACK",
                })
                stats["V24_LATE_SECONDARY_SELECTED"] += 1
            else:
                stats["V24_LATE_SECONDARY_COST_GATE_REJECTED"] += 1
...
def scenario_set(
    ...
    for name, cost in SCENARIOS.items():
        events, stats = layered_route(v11_rows, v19_rows, secondary_rows, cost, days)
...
def route_component(
    ...
    events, _stats = layered_route(v11_rows, v19_rows, secondary_rows, cost_bps, days)
...
def audit(
    ...
    full, routing = scenario_set(v11_rows, v19_rows, secondary_rows, target)
    dev, _ = scenario_set(v11_rows, v19_rows, secondary_rows, development)
    val, _ = scenario_set(v11_rows, v19_rows, secondary_rows, validation)
    fin, _ = scenario_set(v11_rows, v19_rows, secondary_rows, final)
    hol, _ = scenario_set(v11_rows, v19_rows, secondary_rows, holdout)

    normal_events, _ = layered_route(
        v11_rows, v19_rows, secondary_rows, SCENARIOS["NORMAL"], target
    )
    p95_events, _ = layered_route(
        v11_rows, v19_rows, secondary_rows, SCENARIOS["P95"], target
    )
...
def development_pass(result: dict, routing: dict, baseline: dict) -> bool:
    ...
    secondary_count = int(routing["NORMAL"].get("V24_LATE_SECONDARY_SELECTED", 0))
    return bool(
        secondary_count >= 5
...
def validation_pass(result: dict, routing: dict, baseline: dict) -> bool:
    ...
    return bool(
        ...
        and secondary_count >= MIN_VALIDATION_SECONDARY_TRADES
        and normal["compoundedReturnPct"] > baseline["NORMAL"]["compoundedReturnPct"]
        and p95["compoundedReturnPct"] > baseline["P95"]["compoundedReturnPct"]
...
def improvement_checks(
    ...
    return {
        "selectedPassesV22StrictHurdles": selected["allStrictHurdlesPassed"],
        ...
        "secondaryNormalPositive": secondary_component["NORMAL"]["compoundedReturnPct"] > 0,
        "secondaryP95Positive": secondary_component["P95"]["compoundedReturnPct"] > 0,
...
def analyze(cache_root: Path) -> dict:
    ...
    empty_secondary: List[dict] = []

    baseline_audit = audit(
        v11_rows, v19_rows, empty_secondary, target, development_days,
        validation_days, final_days, holdout,
    )
    baseline_dev, _ = scenario_set(
        v11_rows, v19_rows, empty_secondary, development_days
    )
    baseline_val, _ = scenario_set(
        v11_rows, v19_rows, empty_secondary, validation_days
    )

    development_survivors = []
    for candidate in SECONDARY_CANDIDATES:
        rows = build_secondary_rows(candidate, warmup, features)
    ...
    base_result = {
        ...
        "candidateUniverse": len(SECONDARY_CANDIDATES),
        ...
        "selectionDiscipline": {
            "secondaryCandidateGridPredeclared": True,
            "v19AlwaysEvaluatedBeforeSecondary": True,
        ...
        "architecture": {
            ...
            "priority": [
                "V11_EQ_PRIMARY_1030",
                "V19_FALLBACK_1230",
                "V24_LATE_SECONDARY_1330_ONLY_IF_BOTH_PRIOR_ROUTES_ABSENT",
            ],
            "maximumOneStockPositionPerDay": True,
            "maximumGross": 1.0,
            "maximumSecondaryHoldingHours": 2,
        },
    ...
    secondary_component = {
        name: route_component(
            ...
            "V24_LATE_SECONDARY_FALLBACK",
        )
        for name, cost in SCENARIOS.items()
    }
    checks = improvement_checks(selected_audit, baseline_audit, secondary_component)
    all_checks = all(checks.values())
    status = (
        "ASTER_ONLY_V24_LATE_FALLBACK_STRICT_IMPROVEMENT_SHADOW_ONLY"
        if all_checks
        else "ASTER_ONLY_V24_VALIDATED_LATE_FALLBACK_NOT_STRICT_IMPROVEMENT"
    )
    return v14.rounded({
        **base_result,
        "status": status,
        ...
        "selectedSecondaryComponent": secondary_component,
        ...
        "limitations": [
            ...
            "Any passing result remains Forward-Shadow only.",
        ],
    })
...
def report(result: dict) -> str:
    ...
    lines = [
        "# Aster-only V24 V19 Plus Late Fallback",
        "",
    ...
    if "selectedCandidate" in result:
        ...
        lines.append("No late fallback candidate passed chronological Validation.")
    lines += [
        "",
