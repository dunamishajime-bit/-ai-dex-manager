...
async function main() {
  ...
  const generatedPath = path.join(process.cwd(), "scripts", ".research_current_top2_generated.ts");
  ...
  const payload = {
    schema: "v12-top2-current-ledger/v1",
  ...
  const out = process.env.V12_TOP2_LEDGER_OUT || ".research-state/current-top2-dca/v12-top2-ledger.json";
  await fs.mkdir(path.dirname(out), { recursive: true });
  await fs.writeFile(out, JSON.stringify(payload, null, 2) + "\n", "utf8");
  console.log(JSON.stringify({
    status: "V12_TOP2_LEDGER_PASS",
    period: payload.period,
    normal: payload.modes.normal.metrics,
    stress: payload.modes.stress.metrics,
    normalTrades: normal.trades.length,
    stressTrades: stress.trades.length,
  }, null, 2));
}
