...
async function fetchArray(url:URL){
  ...
  for(let a=0;a<6;a++){
    try{
      const res=await fetch(url,{headers:{accept:"application/json","user-agent":"DisDex-PENGU-Flat1-DD-Governor-Neighborhood/20260924"}});
...
function routeHardstopHours(route?:EntryRoute){
  ...
  if(ddMode==="ROUTE_HARDSTOP_Q60")return 60;
...
async function main(){
  ...
  const variants:DDMode[]=[
    "BASELINE_FLAT1",
    "HARDSTOP_COOLDOWN48",
    "ROUTE_HARDSTOP_Q48","ROUTE_HARDSTOP_Q60","ROUTE_HARDSTOP_Q72","ROUTE_HARDSTOP_Q84","ROUTE_HARDSTOP_Q96",
    "ROUTE72_RECOVERY96","ROUTE72_RECOVERY120","ROUTE72_SHORT96",
    "ROUTE_LOSS4_Q72","ROUTE_ANYLOSS_Q72",
    "ROUTE72_LOSS2_Q48","ROUTE72_DD15_Q72","ROUTE72_DD18_Q72",
    "COMBINED_PROTECT_COOLDOWN48"
  ];
  const result:any={
    schema:"pengu-flat1-dd-governor-neighborhood/v3",
  ...
  await fs.mkdir(".research-state/pengu-flat1-dd-governor-neighborhood",{recursive:true});
  await fs.writeFile(".research-state/pengu-flat1-dd-governor-neighborhood/result.json",JSON.stringify(result,null,2)+"\n");
  ...
  console.log("PENGU_FLAT1_DD_NEIGHBORHOOD="+JSON.stringify(compact));
  console.log("PENGU_FLAT1_DD_NEIGHBORHOOD_CONTRACT_PASS="+JSON.stringify({gross:1,logic:"COMBINED_FILTERED",variants:variants.length,baselineParity:true,researchOnly:true}));
}
