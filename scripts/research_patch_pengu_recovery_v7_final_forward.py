...
tail=r'''  const forwardDerivation=deriveV57Thresholds(historicalBaselineNormal);
  ...
  assert.equal(histV64m.longTrades,13); assert.equal(histV64m.shortTrades,28);
  ...
  const bN=summarize(baseN),bS=summarize(baseS),r2N=summarize(v2N),r2S=summarize(v2S),r7N=summarize(v7N),r7S=summarize(v7S);
  ...
  const result={schema:"pengu-recovery-v7-final-observed-forward/v1",start:new Date(forwardStart).toISOString(),endExclusive:new Date(EVAL_END).toISOString(),alreadyObserved:true,notFreshAfterV7Development:true,v57Frozen:forwardDerivation,v64Frozen,v2Frozen,v7Frozen,baseline:{normal:bN,stress:bS},v2:{normal:r2N,stress:r2S},v7:{normal:r7N,stress:r7S},deltas:{v7VsV64:{normal:delta(bN,r7N),stress:delta(bS,r7S)},v7VsV2:{normal:delta(r2N,r7N),stress:delta(r2S,r7S)}},historicalFreeze:{v64:histV64m},safety:{mode:"RESEARCH_ONLY",ordersSent:false,liveChanged:false,vpsChanged:false,productionChanged:false}};
  await fs.mkdir(OUTPUT_DIR,{recursive:true});
  await fs.writeFile(path.join(OUTPUT_DIR,"v7-final-forward.json"),JSON.stringify(result,null,2)+"\n","utf8");
  console.log("V7_FINAL_FORWARD="+JSON.stringify(result));
'''
src=src[:start]+tail+src[end:]
TARGET.write_text(src)
print(f'PATCHED_V7_FINAL_FORWARD={TARGET}')
