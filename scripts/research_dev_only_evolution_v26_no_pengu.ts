...
// PENGU intentionally excluded because it is already handled by an independent live strategy.
const UNIVERSE = [
...
async function main() {
  if (UNIVERSE.includes("PENGU")) throw new Error("PENGU_EXCLUSION_FAIL");
  ...
  const out = {
    researchLine: "V26_NO_PENGU_DEVELOPMENT_ONLY_EVOLUTION_1X",
    researchOnly: true,
  ...
  await fs.writeFile(path.join(root, "dev-only-evolution-v26-no-pengu.json"), JSON.stringify(out, null, 2), "utf8");
