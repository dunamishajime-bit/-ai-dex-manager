...
async function main(){
  ...
  console.log(`PENGU_CROSS_AVAILABLE_RANGE venue=${CROSS_VENUE} ${new Date(pengu[0].openTime).toISOString()}..${new Date(pengu.at(-1)!.openTime+HOUR).toISOString()} rows=${pengu.length}`);
  ...
  const outDir=`.research-state/pengu-continuation-cross-${CROSS_VENUE.toLowerCase()}`;
