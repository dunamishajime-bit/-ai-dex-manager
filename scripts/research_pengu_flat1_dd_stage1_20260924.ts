...
const PROFILES:RiskProfile[]=[
  {id:"BASELINE"},
  {id:"LONG_HARD6",longHard:.06},
  {id:"LONG_TRAIL7_025",longTrailAct:.07,longTrailRet:.025},
  {id:"LONG_PROTECT6_6_025",longHard:.06,longTrailAct:.06,longTrailRet:.025},
  {id:"SHORT_HARD6",shortHard:.06},
  {id:"SHORT_TRAIL10_03",shortTrailAct:.10,shortTrailRet:.03},
  {id:"SHORT_PROTECT6_8_03",shortHard:.06,shortTrailAct:.08,shortTrailRet:.03},
  {id:"REC_HARD5",recoveryHard:.05},
  {id:"REC_EARLY_HALF3_12_HARD5",recoveryHard:.05,recoveryPartialStop:.03,recoveryPartialAfterHours:12},
  {id:"REC_PROTECT5_5_025",recoveryHard:.05,recoveryPartialStop:.03,recoveryPartialAfterHours:12,recoveryTrailAct:.05,recoveryTrailRet:.025},
  {id:"REC_TREND_FAIL",recoveryTrendFail:true},
  {id:"LONG_TREND_FAIL",longTrendFail:true},
  {id:"SHORT_TREND_FAIL",shortTrendFail:true},
  {id:"LOSS_CD48",lossCooldownHours:48},
  {id:"LOSS_CD72",lossCooldownHours:72},
  {id:"HARD6_ALL",longHard:.06,shortHard:.06,recoveryHard:.05},
  {id:"TRAIL_EARLY_ALL",longTrailAct:.07,longTrailRet:.025,shortTrailAct:.10,shortTrailRet:.03,recoveryTrailAct:.05,recoveryTrailRet:.025},
  {id:"TREND_FAIL_ALL",longTrendFail:true,shortTrendFail:true,recoveryTrendFail:true},
];
...
function replay(rows:PenguDualLsV2EvaluationRow[],points:FundingPoint[],v:Variant,mode:Mode){
  ...
  while(i<rows.length-2){
    ...
    }
    {
      ...
      cooldownUntilTs=rows[exitIndex].candle.openTime+Math.max(baseCd,lossCd)*HOUR;
      i=exitIndex+1;
    }
  }
  return trades;
...
async function main(){
  ...
  const result:any={schema:"pengu-flat1-dd-deep-search/stage1",fixed:{logic:"COMBINED_FILTERED",maximumGross:1,allEntriesGross:1},profiles:{},safety:{researchOnly:true,ordersSent:false,liveChanged:false}};
  ...
  await fs.writeFile(".research-state/pengu-flat1-dd-deep-search/stage1.json",JSON.stringify(result,null,2)+"\n");
  console.log("PENGU_FLAT1_DD_STAGE1="+JSON.stringify({ranked:ranked.map((x:any)=>({id:x.id,score:x.score,formal:{N:x.formal.NORMAL,S:x.formal.SEVERE},rolling:{N:x.rolling.NORMAL,S:x.rolling.SEVERE}})),baselineDd:{formal:base.windows.FORMAL.ddEpisodeNormal,rolling:base.windows.ROLLING365.ddEpisodeNormal}}));
}
