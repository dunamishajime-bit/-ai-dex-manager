...
async function fetchArray(url:URL){
  ...
  for(let a=0;a<6;a++){
    try{
      const res=await fetch(url,{headers:{accept:"application/json","user-agent":"DisDex-PENGU-Flat1-Cooldown-Sweep/20260924"}});
...
type CooldownMode =
  | "BASE_24"
  | "ALL_30" | "ALL_36" | "ALL_42" | "ALL_48" | "ALL_60" | "ALL_72"
  | "BASELONG_48" | "RECOVERY_48" | "SHORT_48" | "LONG_RECOVERY_48";
...
function hardStopCooldownFor(route:EntryRoute,exitReason:string,base:number){
  ...
  const all=Number(cooldownMode.match(/^ALL_(\d+)$/)?.[1]||0);
  if(all>0)return Math.max(base,all);
  if(cooldownMode==="BASELONG_48"&&route==="BASE_V64_LONG")return Math.max(base,48);
  if(cooldownMode==="RECOVERY_48"&&route==="RECOVERY_V8")return Math.max(base,48);
  if(cooldownMode==="SHORT_48"&&route==="SHORT_V20")return Math.max(base,48);
  if(cooldownMode==="LONG_RECOVERY_48"&&(route==="BASE_V64_LONG"||route==="RECOVERY_V8"))return Math.max(base,48);
  return base;
}
...
async function main(){
  ...
  const variants:CooldownMode[]=["BASE_24","ALL_30","ALL_36","ALL_42","ALL_48","ALL_60","ALL_72","BASELONG_48","RECOVERY_48","SHORT_48","LONG_RECOVERY_48"];
  const result:any={
    schema:"pengu-flat1-hardstop-cooldown-sweep/v1",
    source:{venue:"ASTER_FUTURES_V3",productionSourceSha:process.env.PRODUCTION_SOURCE_SHA||null,logic:"COMBINED_FILTERED fixed"},
    fixedContract:{penguMaximumGross:1.0,everyEntryGross:1.0,entryLogic:"COMBINED_FILTERED unchanged",exitLogic:"unchanged; cooldown risk overlay only"},
    candidates:{BASE_24:"existing hard-stop cooldown 24h",ALL_30:"30h after all hard stops",ALL_36:"36h",ALL_42:"42h",ALL_48:"48h",ALL_60:"60h",ALL_72:"72h",BASELONG_48:"48h after BASE_V64_LONG hard stop only",RECOVERY_48:"48h after Recovery V8 hard stop only",SHORT_48:"48h after Short V20 hard stop only",LONG_RECOVERY_48:"48h after Base Long or Recovery hard stop"},
  ...
  approx(formal.NORMAL.returnPct,1307.9130447891462,"formal flat1 baseline parity");
  approx(formal.SEVERE.returnPct,769.7792392486292,"formal flat1 severe parity");
  approx(rolling.NORMAL.returnPct,978.2039888080867,"rolling flat1 baseline parity");
  approx(rolling.SEVERE.returnPct,550.8813814669275,"rolling flat1 severe parity");
  await fs.mkdir(".research-state/pengu-flat1-hardstop-cooldown-sweep",{recursive:true});
  await fs.writeFile(".research-state/pengu-flat1-hardstop-cooldown-sweep/result.json",JSON.stringify(result,null,2)+"\n");
  ...
  console.log("PENGU_FLAT1_COOLDOWN_SWEEP="+JSON.stringify(compact));
  console.log("PENGU_FLAT1_COOLDOWN_CONTRACT_PASS="+JSON.stringify({gross:1,logic:"COMBINED_FILTERED",variants:variants.length,baselineParity:true,researchOnly:true}));
}
