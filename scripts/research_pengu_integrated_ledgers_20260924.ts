...
interface Trade {
  ...
  partialDefense?:boolean; partialAccountReturn?:number; partialDefenseTs?:number; partialDefensePrice?:number; partialDefenseGross?:number; openAtWindowEnd?:boolean;
...
function replay(rows:PenguDualLsV2EvaluationRow[],points:FundingPoint[],v:Variant,mode:Mode){
  ...
  while(i<rows.length-2){
    ...
    let exitIndex=entryIndex,exitPrice=entry.open,exitReason="WINDOW_END",partialDefense=false,partialAccountReturn=0,partialDefenseTs:number|undefined,partialDefensePrice:number|undefined,partialDefenseGross:number|undefined,accountReturn=0,rawUnitReturn=0,fundingUnitReturn=0,costUnitReturn=0;
    if(route==="RECOVERY_V8"){
      ...
      for(let j=entryIndex;j<=last;j++){
        ...
        if(ev.events.includes("PARTIAL_DEFENSE")&&!partialDefense){
          ...
          partialDefense=true;partialDefenseTs=rows[j].candle.openTime;partialDefensePrice=pp;partialDefenseGross=pg;remainingGross=Math.max(0,gross-pg);
        }
...
async function main(){
  ...
  const modes:AllocationMode[]=["CAP1_CURRENT_DESIGN","CAP1_EVERY_ENTRY_FLAT"];
  const payload:any={
    schema:"pengu-combined-filtered-integrated-ledgers/v1",
    strategyId:"PENGU_DUAL_LS_V2_COMBINED_FILTERED",
    period:{startInclusive:new Date(FORMAL_START).toISOString(),endExclusive:new Date(FORMAL_END).toISOString()},
    source:{venue:"ASTER_FUTURES_V3",logic:"COMBINED_FILTERED",productionSourceSha:process.env.PRODUCTION_SOURCE_SHA||null},
    allocationModes:{},
    safety:{researchOnly:true,ordersSent:false,liveChanged:false,vpsChanged:false,productionChanged:false}
  };
  for(const allocation of modes){
    allocationMode=allocation;
    const normal=between(replay(rows,fp,"COMBINED_FILTERED","NORMAL"),FORMAL_START,FORMAL_END);
    const stress=between(replay(rows,fp,"COMBINED_FILTERED","SEVERE"),FORMAL_START,FORMAL_END);
    const mapTrade=(t:Trade)=>({
      symbol:"PENGUUSDT",side:t.side==="L"?1:-1,route:t.route,signalTs:t.signalTs,entryTs:t.entryTs,exitTs:t.exitTs,
      entryPrice:t.entryPrice,exitPrice:t.exitPrice,requestedGross:t.requestedGross,
      netUnitReturn:t.requestedGross>0?t.accountReturn/t.requestedGross:0,
      accountReturn:t.accountReturn,exitReason:t.exitReason,
      partialDefense:Boolean(t.partialDefense),
      partialDefenseTs:t.partialDefenseTs??null,
      partialDefensePrice:t.partialDefensePrice??null,
      partialDefenseGross:t.partialDefenseGross??0,
      partialAccountReturn:t.partialAccountReturn??0,
      partialNetUnitReturn:(t.partialDefenseGross??0)>0?(t.partialAccountReturn??0)/(t.partialDefenseGross??1):0,
      remainingGrossAfterPartial:t.partialDefense?(t.requestedGross-(t.partialDefenseGross??0)):t.requestedGross,
      remainingAccountReturn:t.accountReturn-(t.partialAccountReturn??0),
      remainingNetUnitReturn:(t.partialDefense?(t.requestedGross-(t.partialDefenseGross??0)):t.requestedGross)>0
        ?(t.accountReturn-(t.partialAccountReturn??0))/(t.partialDefense?(t.requestedGross-(t.partialDefenseGross??0)):t.requestedGross):0
    });
    payload.allocationModes[allocation]={
      modes:{
        normal:{metrics:metrics(normal),trades:normal.map(mapTrade)},
        stress:{metrics:metrics(stress),trades:stress.map(mapTrade)}
      }
    };
  }
  await fs.mkdir(".research-state/pengu-integrated-ledgers",{recursive:true});
  await fs.writeFile(".research-state/pengu-integrated-ledgers/pengu-combined-filtered-ledgers.json",JSON.stringify(payload,null,2)+"\n");
  for(const allocation of modes){
    const one={schema:payload.schema,strategyId:payload.strategyId,period:payload.period,source:payload.source,allocationMode:allocation,...payload.allocationModes[allocation],safety:payload.safety};
    await fs.writeFile(`.research-state/pengu-integrated-ledgers/${allocation.toLowerCase()}.json`,JSON.stringify(one,null,2)+"\n");
  }
  const checks:any={};
  for(const allocation of modes){
    const row=payload.allocationModes[allocation].modes;
    checks[allocation]={
      normalTrades:row.normal.trades.length,stressTrades:row.stress.trades.length,
      normalPartials:row.normal.trades.filter((x:any)=>x.partialDefense).length,
      stressPartials:row.stress.trades.filter((x:any)=>x.partialDefense).length,
      maxGross:Math.max(...row.normal.trades.map((x:any)=>x.requestedGross),0)
    };
  }
  assert.equal(checks.CAP1_CURRENT_DESIGN.maxGross,1);
  assert.equal(checks.CAP1_EVERY_ENTRY_FLAT.maxGross,1);
  console.log("PENGU_INTEGRATED_LEDGERS="+JSON.stringify(checks));
}
