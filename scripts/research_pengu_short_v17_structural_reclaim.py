...
PRE_SHA = '9b02e7ec708d02c712d7577cbd50cc548492d311'
V16_PRE_SHA = '1528d182ad3593460d325e2c1e1f6bacf0b07795'
V16_NAME = 'COUNTERWIND_RELATIVE_THESIS_PROBATION'
V17_NAME = 'COUNTERWIND_STRUCTURAL_RECLAIM_PROBATION'

spec = importlib.util.spec_from_file_location('v16', 'scripts/research_pengu_short_v16_relative_thesis.py')
v16 = importlib.util.module_from_spec(spec)
spec.loader.exec_module(v16)


def source_for(venue):
    temp = v16.source_for(venue)
    text = temp.read_text()
    temp.unlink(missing_ok=True)

    if text.count(V16_PRE_SHA) < 1:
        raise RuntimeError('Frozen V16 preregistration SHA not found')
    text = text.replace(V16_PRE_SHA, PRE_SHA)
    text = text.replace(V16_NAME, V17_NAME)

    old = 'if (bar.close >= costCoverPrice && features.relativeReturn24h >= 0) {'
    new = 'if (bar.close >= costCoverPrice && features.relativeReturn24h >= 0 && bar.close >= features.ema72) {'
    if text.count(old) != 1:
        raise RuntimeError(f'Expected exactly one V16 cost-floor condition, got {text.count(old)}')
    text = text.replace(old, new, 1)
    text = text.replace('pengu-short-v16-', 'pengu-short-v17-')
...
def main():
    v16.v15.v12.v11runner.load_binance_klines('PENGUUSDT')
    v16.v15.v12.v11runner.load_binance_klines('BTCUSDT')
    v16.v15.v12.v11runner.load_binance_funding()
    okx = run('OKX')
    binance = run('Binance')

    gp = v16.v15.v12final.load_gate_klines('PENGU_USDT')
    gb = v16.v15.v12final.load_gate_klines('BTC_USDT')
    v16.v15.v12final.write_gate_cache(gp, gb, [])
    gate = run('Gate')

    bitget_data = {
        'pengu': v16.v15.dense_bitget_candles('PENGUUSDT'),
        'btc': v16.v15.dense_bitget_candles('BTCUSDT'),
        'funding': v16.v15.v13.load_bitget_funding(),
    }
    bitget = run('Bitget')
    ...
    result = {
        ...
        'ruleChangeCount': 1,
        'ruleChange': 'cost-floor exit additionally requires bar.close >= features.ema72',
