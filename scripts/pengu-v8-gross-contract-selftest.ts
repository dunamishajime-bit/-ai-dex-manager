...
assert.deepEqual(contract, { requestedGross: 1.0, intentGross: 1.0, intentNotionalUsd: 1_000 });
...
assert.equal(plan.accepted[0]?.gross, 1.0);
assert.equal((plan.accepted[0] as any)?.requestedGross, 1.0);
