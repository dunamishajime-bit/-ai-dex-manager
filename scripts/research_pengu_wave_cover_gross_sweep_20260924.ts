...
async function fetchArray(url:URL){
  ...
  for(let a=0;a<6;a++){
    try{
      const res=await fetch(url,{headers:{accept:"application/json","user-agent":"DisDex-PENGU-Wave-Cover-Gross-Sweep/20260924"}});
...
let RESEARCH_PENGU_MAX_GROSS = 0.85;
function researchTargetGrossForAtr(atr24Ratio:number){
  const targetVolatility=PENGU_DUAL_LS_V2.sizing.targetVolatility;
  const grossFloor=PENGU_DUAL_LS_V2.sizing.grossFloor;
  if(!Number.isFinite(atr24Ratio)||atr24Ratio<=0)return 0;
  return Math.min(RESEARCH_PENGU_MAX_GROSS,Math.max(grossFloor,RESEARCH_PENGU_MAX_GROSS*targetVolatility/atr24Ratio));
}
function researchV64RequestedLongGross(f:PenguDualLsV2Features){
  const multiplier=PENGU_V8_V64_BASE.longMultiplier;
  const floor=PENGU_DUAL_LS_V2.sizing.grossFloor*multiplier;
  const target=RESEARCH_PENGU_MAX_GROSS*PENGU_DUAL_LS_V2.sizing.targetVolatility/f.atr24Ratio*multiplier;
...
function acceptedGross(route:EntryRoute,f:PenguDualLsV2Features){
  ...
  return Math.min(RESEARCH_PENGU_MAX_GROSS,researchV64RequestedLongGross(f));
}
...
async function main(){
  ...
  const grossCaps=[0.85,1.0,1.3,1.5];
  const result:any={
    schema:"pengu-wave-cover-gross-sweep/v1",
    ...
    sizingInterpretation:{
      changed:"penguMaximumGross equivalent used by normal/short/V64/supplemental sizing",
      unchanged:["grossFloor=0.60","Recovery V8 initialGross=0.50","V64 lowGross=0.1875","all entry/exit logic","fees/funding","cooldowns"],
      formula:"Production-equivalent ATR sizing with candidate maximum gross"
    },
    windows:{},
    safety:{researchOnly:true,ordersSent:false,liveChanged:false,vpsChanged:false,productionChanged:false}
  };
  for(const w of windows){
    const out:any={start:new Date(w.start).toISOString(),end:new Date(w.end).toISOString(),grossCases:{}};
    for(const gross of grossCaps){
      RESEARCH_PENGU_MAX_GROSS=gross;
      const n=between(replay(rows,fp,"COMBINED_FILTERED","NORMAL"),w.start,w.end);
      const sv=between(replay(rows,fp,"COMBINED_FILTERED","SEVERE"),w.start,w.end);
      out.grossCases[String(gross)]={
        NORMAL:metrics(n),SEVERE:metrics(sv),
        folds:w.name==="FORMAL"?{NORMAL:formalFolds(n),SEVERE:formalFolds(sv)}:undefined,
        maxRequestedGrossNormal:Math.max(...n.map(t=>t.requestedGross),0),
        maxRequestedGrossSevere:Math.max(...sv.map(t=>t.requestedGross),0),
        routeNormal:routeMetrics(n),
        routeSevere:routeMetrics(sv),
      };
    }
    const base=out.grossCases["0.85"];
    for(const gross of grossCaps.filter(x=>x!==0.85)){
      const row=out.grossCases[String(gross)];
      row.deltaVs085={
        normalReturnPct:row.NORMAL.returnPct-base.NORMAL.returnPct,
        severeReturnPct:row.SEVERE.returnPct-base.SEVERE.returnPct,
        normalWinRatePct:(row.NORMAL.winRatePct??0)-(base.NORMAL.winRatePct??0),
        severeWinRatePct:(row.SEVERE.winRatePct??0)-(base.SEVERE.winRatePct??0),
  ...
  await fs.mkdir(".research-state/pengu-wave-cover-gross-sweep",{recursive:true});
  await fs.writeFile(".research-state/pengu-wave-cover-gross-sweep/result.json",JSON.stringify(result,null,2)+"\n");
  console.log("PENGU_WAVE_COVER_GROSS_SWEEP="+JSON.stringify(result));
}
