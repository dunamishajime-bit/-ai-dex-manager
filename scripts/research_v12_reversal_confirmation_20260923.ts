...
type Variant = {
  ...
  noTrend?: {er12Max:number; er24Max?:number; requireBtc12Adverse?:boolean; requireBtc6OrSymbolWeak?:boolean; symbol6Max?:number; includeBurst?:boolean; combineAnyWeak?:boolean; minScore?:number};
  confirmReversal?: {er12Max:number; minScore?:number};
};
...
const variants:Variant[]=[
  ...
  {name:"HC175_ANY_WEAK_BLOCK",...LOCKED,narrowMode:"ANY_BLOCK"},
  {name:"CONF_REV065_ALL",...LOCKED,confirmReversal:{er12Max:.065}},
  {name:"CONF_REV080_ALL",...LOCKED,confirmReversal:{er12Max:.080}},
  {name:"CONF_REV065_S050",...LOCKED,confirmReversal:{er12Max:.065,minScore:.50}},
  {name:"CONF_REV080_S050",...LOCKED,confirmReversal:{er12Max:.080,minScore:.50}},
  {name:"ANY_CONF_REV065_ALL",...LOCKED,narrowMode:"ANY_BLOCK",confirmReversal:{er12Max:.065}},
  {name:"ANY_CONF_REV080_ALL",...LOCKED,narrowMode:"ANY_BLOCK",confirmReversal:{er12Max:.080}},
  {name:"ANY_CONF_REV065_S050",...LOCKED,narrowMode:"ANY_BLOCK",confirmReversal:{er12Max:.065,minScore:.50}},
  {name:"ANY_CONF_REV080_S050",...LOCKED,narrowMode:"ANY_BLOCK",confirmReversal:{er12Max:.080,minScore:.50}},
  {name:"ANY_CONF_REV065_S055",...LOCKED,narrowMode:"ANY_BLOCK",confirmReversal:{er12Max:.065,minScore:.55}},
  {name:"ANY_CONF_REV080_S055",...LOCKED,narrowMode:"ANY_BLOCK",confirmReversal:{er12Max:.080,minScore:.55}},
];
const modes: Mode[] = [
...
function confirmReversalRisk(s:RoutedSignal,opt:NonNullable<Variant["confirmReversal"]>){
  const f=s.features;
  return f.btcEr12<opt.er12Max
    && f.btc12h<0
    && (f.btc6h<0 || f.ret6h<0)
    && (opt.minScore==null || f.score>=opt.minScore);
}
function simulate(d:PerpMarketData,p:Prepared,v:Variant,m:Mode){
  ...
  const pos=new Map<string,Position>(); const pending=new Map<string,RoutedSignal>(); const cool=new Map<string,number>(); const nonHcCooldown=new Map<string,number>(); const confirm=new Map<string,{side:"LONG"|"SHORT";ts:number}>();
  const pnls:number[]=[]; const tradeRows:any[]=[]; const recentNonHcLossTs:number[]=[]; let marketPauseUntil=0; let clusterTrips=0; let confirmTriggered=0,confirmPassed=0; let entries=0, rank2Entries=0, filtered=0, winsFiltered=0, maxEntryGross=0, hcEntries=0,nonHcEntries=0;
  ...
  for(const t of times){
    ...
    for(const [sym,c] of [...confirm])if(c.ts<t-V12_X1_ALL.timeframeHours*H)confirm.delete(sym);
    for(const s of ss){
      if(slots<=0)break;
      const pauseBlocks=marketPauseUntil>t&&!highConfidence(s)&&!(v.clusterTrendRecovery&&marketState(s)==="TREND");
      if(pos.has(s.symbol)||pending.has(s.symbol)||(cool.get(s.symbol)||0)>t||((nonHcCooldown.get(s.symbol)||0)>t&&!highConfidence(s))||pauseBlocks)continue;
      if(s.route==="EARLY_FAST_BTC"&&[...pos.values()].some(q=>q.side!==s.side))continue;
      if(v.confirmReversal&&!highConfidence(s)){
        const prior=confirm.get(s.symbol), prevTs=t-V12_X1_ALL.timeframeHours*H;
        if(prior&&prior.side===s.side&&prior.ts===prevTs){
          confirm.delete(s.symbol);confirmPassed++;pending.set(s.symbol,s);slots--;continue;
        }
        if(confirmReversalRisk(s,v.confirmReversal)){
          confirm.set(s.symbol,{side:s.side,ts:t});confirmTriggered++;continue;
        }
      }
      confirm.delete(s.symbol);pending.set(s.symbol,s);slots--;
    }
  }
  ...
  return {
    hcStats:subgroup(hcRows),nonHcStats:subgroup(nonHcRows),maxEntryGross,hcEntries,nonHcEntries,clusterTrips,confirmTriggered,confirmPassed,
...
async function main(){
  ...
  const out={status:"PASS_RESEARCH_ONLY_HC175_REVERSAL_CONFIRMATION",limitations:["V12 research simulator only","HC condition and 1.75x multiplier frozen","today audit uses OKX 2h public candles as a venue proxy; historical BT uses repository research data source"],period:{start:new Date(START).toISOString(),end:new Date(END).toISOString()},conditions:{initialJpy:10000,monthlyJpy:10000,monthlyCount:12,totalContributionJpy:130000},productionConstants:V12_X1_ALL,dataSource:d.source,variants,results,removed,todayAudit,rankingNormal:[...rows].sort((a,b)=>b.NORMAL.finalEquity-a.NORMAL.finalEquity).map(x=>x.variant)};
  const focus=variants.map(v=>({variant:v.name,NORMAL:{finalEquity:results[v.name].NORMAL.finalEquity,winRatePct:results[v.name].NORMAL.winRatePct,profitFactor:results[v.name].NORMAL.profitFactor,maxDrawdownPct:results[v.name].NORMAL.maxDrawdownPct,tradeCount:results[v.name].NORMAL.tradeCount,hc:results[v.name].NORMAL.hcStats,confirmTriggered:results[v.name].NORMAL.confirmTriggered,confirmPassed:results[v.name].NORMAL.confirmPassed,...removed[v.name].NORMAL},SEVERE:{finalEquity:results[v.name].SEVERE.finalEquity,winRatePct:results[v.name].SEVERE.winRatePct,profitFactor:results[v.name].SEVERE.profitFactor,maxDrawdownPct:results[v.name].SEVERE.maxDrawdownPct,tradeCount:results[v.name].SEVERE.tradeCount,hc:results[v.name].SEVERE.hcStats,confirmTriggered:results[v.name].SEVERE.confirmTriggered,confirmPassed:results[v.name].SEVERE.confirmPassed,...removed[v.name].SEVERE},today:todayAudit.find(x=>x.variant===v.name)?.blocked}));
