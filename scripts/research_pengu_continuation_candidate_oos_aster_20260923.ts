...
async function main(){
  ...
  const result:any={schema:"pengu-continuation-candidate-oos/v1",venue:"ASTER",candidate:{ret6Min:.03,distanceAtrMax:.50,ret72Min:.15,ret72Max:.40,armHours:24},availableRange:[new Date(pengu[0].openTime).toISOString(),new Date(pengu.at(-1)!.openTime+HOUR).toISOString()],windows:{}};
  ...
  const outDir=".research-state/pengu-continuation-oos-aster";
