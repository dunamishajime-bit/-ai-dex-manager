...
import research_lab_v96_core_profit_capture_round2 as r2
...
class Round3Config:
    name: str
    family: str
    risk_lock_trigger_pct: Optional[float] = None
    risk_lock_reduction: float = 0.25
    boost_pyramid_trigger_pct: Optional[float] = None
    boost_pyramid_add: float = 0.0


CANDIDATES = (
    Round3Config("BASELINE", "BASELINE"),
    Round3Config("RISK_LOCK25_T8", "RISK_LOCK", risk_lock_trigger_pct=8.0),
    Round3Config("RISK_LOCK25_T12", "RISK_LOCK", risk_lock_trigger_pct=12.0),
    Round3Config("RISK_LOCK25_T16", "RISK_LOCK", risk_lock_trigger_pct=16.0),
    Round3Config("BOOST_PYRAMID2P5_T4", "BOOST_PYRAMID2P5", boost_pyramid_trigger_pct=4.0, boost_pyramid_add=0.025),
    Round3Config("BOOST_PYRAMID2P5_T6", "BOOST_PYRAMID2P5", boost_pyramid_trigger_pct=6.0, boost_pyramid_add=0.025),
    Round3Config("BOOST_PYRAMID2P5_T8", "BOOST_PYRAMID2P5", boost_pyramid_trigger_pct=8.0, boost_pyramid_add=0.025),
    Round3Config("BOOST_PYRAMID5_T4", "BOOST_PYRAMID5", boost_pyramid_trigger_pct=4.0, boost_pyramid_add=0.05),
    Round3Config("BOOST_PYRAMID5_T6", "BOOST_PYRAMID5", boost_pyramid_trigger_pct=6.0, boost_pyramid_add=0.05),
    Round3Config("BOOST_PYRAMID5_T8", "BOOST_PYRAMID5", boost_pyramid_trigger_pct=8.0, boost_pyramid_add=0.05),
)
...
def risk_deteriorated(frame: dict) -> bool:
    feature = frame.get("feature", {})
    return bool(
        bool(frame.get("whipsaw"))
        or int(frame.get("ddStage", 0)) > 0
        or int(frame.get("breadth", 0)) < 2
        or r1.finite(feature.get("mom3")) < 0
        or r1.finite(feature.get("shock")) < -4.0
        or r1.finite(feature.get("skew"), 1.0) > 1.35
    )
...
def candidate_weights(
    ...
    for symbol in SYMBOLS:
        ...
        multiplier = 1.0
        if config.risk_lock_trigger_pct is not None:
            if (
                r1.finite(state["peak"]) * 100.0 >= config.risk_lock_trigger_pct
                and int(state["negativeStreak"]) >= 2
                and risk_deteriorated(frame)
            ):
                state["locked"] = True
        if bool(state["locked"]):
            multiplier *= 1.0 - config.risk_lock_reduction

        if config.boost_pyramid_trigger_pct is not None and not bool(state["pyramided"]):
            if (
                bool(frame.get("boost"))
                and not bool(frame.get("whipsaw"))
                and int(frame.get("ddStage", 0)) == 0
                and r1.finite(state["cumulative"]) * 100.0 >= config.boost_pyramid_trigger_pct
                and r1.finite(state["lastSignedReturn"]) > 0
            ):
                state["pyramided"] = True
        if bool(state["pyramided"]):
            multiplier *= 1.0 + config.boost_pyramid_add
...
def simulate(
    ...
    for position, ts in enumerate(times):
        ...
        frame = r2.frame_at(ts, base_core, features, context, controlled_map)
...
def main() -> None:
    ...
    for candidate in CANDIDATES:
        normal = simulate(
            candidate, raw, targets, times, base_core, features, context,
            normal_controlled, 10.0, 0.0, 0
        )
        severe = simulate(
            candidate, raw, targets, times, severe_core, features, context,
    ...
    for candidate in CANDIDATES:
        if candidate.name == "BASELINE":
            continue
        result = evaluations[candidate.name]["periods"]
        full = result["full"]
        validation = result["validation2025"]
        diagnostic = result["diagnostic2026H1"]
        loo_wins = sum(
            leave_one_out[candidate.name][symbol]["normal"]["compoundedReturnPct"]
            >= baseline_loo[symbol]["normal"]["compoundedReturnPct"]
            and leave_one_out[candidate.name][symbol]["severe"]["compoundedReturnPct"]
            >= baseline_loo[symbol]["severe"]["compoundedReturnPct"]
            for symbol in SYMBOLS
        )
        passed = bool(
        ...
        screen.append({
            ...
            "screenPass": passed,
            "leaveOneOutImprovements": int(loo_wins),
    ...
    for candidate in CANDIDATES:
        ...
        family = families.setdefault(candidate.family, {"tested": 0, "passed": 0, "members": []})
        family["tested"] += 1
        family["passed"] += int(bool(row["screenPass"]))
        family["members"].append(candidate.name)

    result = r1.rounded({
        "strategyId": "V96_CORE_PROFIT_CAPTURE_ROUND3",
        ...
        "limitations": [
            "Round 3 is still a frozen-controller screening overlay.",
            "The reused 2025 and 2026H1 data are not pristine forward evidence.",
            "Only a neighboring family with exact state feedback may advance to shadow.",
        ],
    })
    state_dir.mkdir(parents=True, exist_ok=True)
    (state_dir / "v96-core-profit-capture-round3.json").write_text(
        json.dumps(result, ensure_ascii=False, indent=2), encoding="utf-8"
    )
    report = [
        "# V96 Core Profit Capture Round 3",
        "",
        "- Production changed: **NO**",
        "- Families: risk-conditioned profit lock and Strong-Boost-conditioned pyramiding",
        "",
        "| Candidate | Family | Pass | Full normal | Full severe | 2025 N | 2025 S | 2026 N | 2026 S | LOO |",
        "| --- | --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
    ]
    ...
    report.append("")
    report.append("No screen result is eligible for Production without exact controller feedback and forward shadow evidence.")
    (state_dir / "v96-core-profit-capture-round3.md").write_text(
