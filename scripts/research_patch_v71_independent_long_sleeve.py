...
insert = r'''
type V71Family = "FAILED_BREAKDOWN_RECLAIM";
const V71_NEW_LONG_GROSS = 0.25;
const V71_MATERIAL_GAIN_MULTIPLE = 1.05;
const V71_NEW_LONG_SLEEVE = "PENGU_V71_NEW_LONG_SLEEVE";
const MIN_TRADES_PER_FOLD = 2;
const V71_FOLD_BOUNDARIES = [
  "2025-08-10T00:00:00Z",
  "2025-12-09T16:00:00Z",
  "2026-04-10T08:00:00Z",
  "2026-08-10T00:00:00Z",
].map(x=>Date.parse(x));
const V71_FROZEN_V64_CONFIG:V64Config={
...
function v71PriorLow(rows:PenguDualLsV2EvaluationRow[],index:number) {
  let low=Number.POSITIVE_INFINITY;
  for(let i=index-7;i<=index-2;i+=1) low=Math.min(low,rows[i].candle.low);
  return low;
}
function v71Raw(rows:PenguDualLsV2EvaluationRow[],index:number) {
  if(index<8) return false;
  const row=rows[index], f=row.features, prev=rows[index-1].candle;
  if(!f || row.shortSignal || longRawForMode(row,"V64_DYNAMIC")) return false;
  const priorLow=v71PriorLow(rows,index);
  return (
    f.penguReturn24h<=-0.02
    && f.relativeReturn24h<=0.01
    && f.btcReturn24h>=-0.03
    && f.volumeRatio6OverPrior36>=0.80
    && f.rsi14>=25 && f.rsi14<=52
    && prev.low<priorLow
    && row.candle.close>priorLow
    && row.candle.close>prev.open
  );
}
function v71NewLongSignal(rows:PenguDualLsV2EvaluationRow[],index:number) {
  const current=v71Raw(rows,index);
  const previous=index>0?v71Raw(rows,index-1):false;
  return current&&!previous;
}
function replayV71NewLongSleeve(rows:PenguDualLsV2EvaluationRow[],funding:FundingPoint[],mode:Mode) {
  ...
  while(index<rows.length-2) {
    ...
    if(!features || !v71NewLongSignal(rows,index)) { index+=1; continue; }
    const entryIndex=index+1, entry=rows[entryIndex].candle;
    let position:PenguDualLsV2Position={
      side:1,entryTs:entry.openTime,entryPrice:entry.open,quantity:1,gross:V71_NEW_LONG_GROSS,
    ...
    if(entry.openTime>=EVAL_START && entry.openTime<EVAL_END) {
      ...
      trades.push({side:"L",signalTs:rows[index].candle.openTime,entryTs:entry.openTime,exitTs,entryPrice:entry.open,exitPrice,
        requestedGross:V71_NEW_LONG_GROSS,rawUnitReturn,fundingUnitReturn,costUnitReturn,netUnitReturn,
        accountReturn:V71_NEW_LONG_GROSS*netUnitReturn,exitReason,engineExitReason,entryFeatures:{...features},mfeUnit:bestFavorable,maeUnit:worstAdverse});
    }
...
function evaluateV71(rows:PenguDualLsV2EvaluationRow[],funding:FundingPoint[],baselineV56Normal:RichTrade[]) {
  const derivation=deriveV57Thresholds(baselineV56Normal); v57Thresholds=derivation.thresholds;
  v64ActiveConfig=V71_FROZEN_V64_CONFIG;
  ...
  const normal=replayV71NewLongSleeve(rows,funding,"normal");
  const stress=replayV71NewLongSleeve(rows,funding,"stress");
  const folds:Array<any>=[];
  for(let i=0;i<3;i+=1) {
    const a=V71_FOLD_BOUNDARIES[i], b=V71_FOLD_BOUNDARIES[i+1];
    const n=sliceByTime(normal,a,b), s=sliceByTime(stress,a,b);
    const mn=metrics(n), ms=metrics(s);
    const normalPass=n.length>=MIN_TRADES_PER_FOLD && mn.returnPct>0 && (mn.profitFactor??0)>=1.0;
    const stressPass=s.length>=MIN_TRADES_PER_FOLD && ms.returnPct>0 && (ms.profitFactor??0)>=1.0;
  ...
  const strictPass=allFoldPass&&fullNormalPositive&&stressRobust;
  return {schema:"pengu-v71-independent-long-sleeve/v1",family:"FAILED_BREAKDOWN_RECLAIM" as V71Family,frozenV64Config:V71_FROZEN_V64_CONFIG,newLongGross:V71_NEW_LONG_GROSS,materialGainThresholdMultiple:V71_MATERIAL_GAIN_MULTIPLE,incumbent:{normal:incNM,stress:incSM},folds,allFoldPass,fullMetrics:{normal:fmN,stress:fmS},fullNormalPositive,stressRobust,strictPass,decision:strictPass?"ADOPT_V71_RESEARCH_CANDIDATE":"KEEP_V64_RESEARCH_CANDIDATE",reason:strictPass?"Fixed FAILED_BREAKDOWN_RECLAIM 0.25x independent Long sleeve passed every predeclared Normal/Stress chronological fold and full-period guards.":"Fixed V71 sleeve failed at least one predeclared chronological fold or full-period guard; no tuning permitted."};
}
'''
...
tail = r'''  const v71=evaluateV71(rows,funding,baselineNormal);
  v64ActiveConfig=V71_FROZEN_V64_CONFIG;
  ...
  const newLongNormal=replayV71NewLongSleeve(rows,funding,"normal");
  const newLongStress=replayV71NewLongSleeve(rows,funding,"stress");
  const resultPayload={status:"PASS_RESEARCH_ONLY",period:{startInclusive:new Date(EVAL_START).toISOString(),endExclusive:new Date(EVAL_END).toISOString()},source:{productionLogicSha:SOURCE_SHA,venue:"Aster perpetual public REST V3"},v71,final:{v64Preserved:true,newLongSleeveSelected:"FAILED_BREAKDOWN_RECLAIM",newLongSleevePromoted:v71.strictPass},safety:{mode:"RESEARCH_ONLY",ordersSent:false,liveChanged:false,vpsChanged:false,productionChanged:false}};
  ...
  function mkNewLongLedger(tradesN:RichTrade[],tradesS:RichTrade[]) { return {
    schema:"pengu-v71-new-long-sleeve-ledger/v1",strategyId:V71_NEW_LONG_SLEEVE,researchOnly:true,selectedFamily:"FAILED_BREAKDOWN_RECLAIM",requestedGross:V71_NEW_LONG_GROSS,
  ...
  await fs.writeFile(path.join(OUTPUT_DIR,"v71-result.json"),JSON.stringify(resultPayload,null,2)+"\n","utf8");
  ...
  console.log("V71_RESULT="+JSON.stringify({decision:v71.decision,family:v71.family,allFoldPass:v71.allFoldPass,fullNormalPositive:v71.fullNormalPositive,stressRobust:v71.stressRobust,strictPass:v71.strictPass,folds:v71.folds}));
'''
src = src[:start] + tail + src[end:]
TARGET.write_text(src)
print(f'PATCHED_V71={TARGET} bytes={TARGET.stat().st_size}')
