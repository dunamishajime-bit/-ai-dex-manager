...
SOL_CID='sol_wrong_wave_retest_release'
LINK_CID='link_vol_midpoint_checkpoint_owner'
...
def simulate_new(cid,pair,candles,idx,start,end,cost_bps,delay,model):
    ...
    th=model['threshold'];c=candles[pair];state=0;life='CASH';side=0;mark_ts=None;retest_seen=False;rearm=True
    entry=peak=trough=None;entry_i=entry_ts=signal_ts=None;entry_pred=None;vals=[];recs=[];prev_ctx=None
    def reset():
        nonlocal state,life,side,mark_ts,retest_seen,entry,peak,trough,entry_i,entry_ts,signal_ts,entry_pred
        state=0;life='CASH';side=0;mark_ts=None;retest_seen=False;entry=peak=trough=None;entry_i=entry_ts=signal_ts=None;entry_pred=None
    ...
    for row in c:
        ...
        if state:
            ...
            if pair=='LINK' and held>=base.HORIZON//2 and held<base.HORIZON:
                breadth_dir=1 if ctx['breadth']>.5 else -1 if ctx['breadth']<.5 else 0
                fast_ok=ctx['r3']*state>0 and ctx['r6']*state>0
                medium_ok=ctx['r12']*state>=0
                if not (breadth_dir==state and fast_ok and medium_ok):flags=['MIDPOINT_OWNERSHIP_RELEASE']
        ...
        if pair=='SOL':
            ...
            if life=='LATCHED_SHADOW':
                elapsed=(ts-mark_ts)//HOUR;pred_against=pr*side<0;medium_against=ctx['r12']*side<0;slow_against=ctx['r24']*side<0
                if slow_against and (medium_against or pred_against):reset();continue
                if elapsed>=2 and ctx['r12']*side>=0 and ctx['r24']*side>=0:life='RETEST_WAIT';mark_ts=ts;retest_seen=False;continue
                if elapsed>12:reset();continue
            elif life=='RETEST_WAIT':
                pred_against=pr*side<0;medium_against=ctx['r12']*side<0;slow_against=ctx['r24']*side<0
                if (pred_against and medium_against) or slow_against:reset();continue
                fast_against=ctx['r3']*side<0 or ctx['r6']*side<0
                if fast_against and not slow_against:retest_seen=True
                release=ctx['r3']*side>0 and ctx['r6']*side>0 and ctx['r12']*side>=0
                if retest_seen and release:
                    ...
                    continue
                if (ts-mark_ts)//HOUR>12:reset();continue
        else:
...
def run(pair):
    ...
    out={'researchLine':'FROZEN_V109_SOL_LINK_NEXT_STRUCTURAL_V3','pair':pair,'candidateChain':[res],'selectedForNextStage':cid,'selectedStatus':res['status'],'nextCandidateGeneration':None if res['status']=='FROZEN_SURVIVOR' else {'sourceDiagnosis':res['diagnosis'],'policy':'NEXT_RUN_NEW_STRUCTURAL_MECHANISM_ONLY_NO_NUMERIC_RETUNE','structuralDirection':('wrong-wave retest/release mechanism only' if pair=='SOL' else 'volatility-aware Cash plus sponsor handshake, causal midpoint ownership release, and 48h ownership only')},'periods':{'development':ps['development'],'validation':ps['validation'],'confirmation':'UNTOUCHED','holdout':'UNTOUCHED'},'frozenV109Changed':False,'frozenThreshold':model['threshold'],'frozenRisk':v109.RISK[pair],'frozenTrailPct':v109.TRAIL[pair],'researchMultiplicity':{'evaluatedThisRun':1,'priorCatalog':8 if pair=='SOL' else 6,'cumulative':9 if pair=='SOL' else 7},'antiOverfit':{'denseSweep':False,'thresholdRetune':False,'riskRetune':False,'trailRetune':False,'sideHardcode':False,'confirmationRead':False,'holdoutRead':False,'designEvidence':['development','validation'],'strictPeriodIsolation':True},'productionChanged':False,'realTradingEnabled':False}
