...
class HoldoutDataBlock(RuntimeError): pass
class HoldoutAdapterBlock(RuntimeError): pass
...
def load_klines(symbol):
    ...
    while cursor < EVAL_END:
        ...
        calls += 1
        if not isinstance(data, list): raise HoldoutDataBlock(f'KuCoin {symbol} kline payload is not a list')
        for row in data:
            if not isinstance(row, list) or len(row) < 6: continue
            try:
                ts = norm_ts(row[0])
                if WARM_START <= ts < EVAL_END:
                    by_ts[ts] = {'openTime':ts,'open':float(row[1]),'high':float(row[2]),'low':float(row[3]),'close':float(row[4]),'volume':float(row[5]),'closeTime':ts+HOUR-1}
            except (TypeError, ValueError): continue
        cursor = end + HOUR
        time.sleep(0.08)
    expected = list(range(WARM_START, EVAL_END, HOUR)); actual = sorted(by_ts)
    missing = sorted(set(expected)-set(actual)); extra = sorted(set(actual)-set(expected))
    if missing or extra or actual != expected:
        raise HoldoutDataBlock(f'KuCoin {symbol} H1 grid incomplete expected={len(expected)} actual={len(actual)} missing={missing[:20]} extra={extra[:20]}')
    rows = [by_ts[ts] for ts in actual]
    (ROOT/f'kucoin-{symbol}.json').write_text(json.dumps(rows,separators=(',',':')))
    return {'rows':len(rows),'calls':calls,'first':rows[0]['openTime'],'last':rows[-1]['openTime']}

def load_funding():
    symbol='PENGUUSDTM'; by_ts={}; cursor=WARM_START; calls=0
    while cursor < EVAL_END:
        end=min(EVAL_END-1,cursor+7*DAY-1)
        data=kucoin_get('/api/v1/contract/funding-rates',{'symbol':symbol,'from':str(cursor),'to':str(end)})
        calls += 1
        if not isinstance(data,list): raise HoldoutDataBlock('KuCoin funding payload is not a list')
        for item in data:
            if not isinstance(item,dict): continue
            try: ts=norm_ts(item.get('timepoint')); rate=float(item.get('fundingRate'))
            except (TypeError,ValueError): continue
            if WARM_START <= ts < EVAL_END: by_ts[ts]={'fundingTime':ts,'fundingRate':rate}
        cursor=end+1; time.sleep(0.1)
    rows=[by_ts[k] for k in sorted(by_ts)]
    if not rows: raise HoldoutDataBlock('KuCoin PENGU funding history is empty')
    if rows[0]['fundingTime'] > EVAL_START: raise HoldoutDataBlock(f'KuCoin funding begins after first eligible evaluation: first={rows[0]["fundingTime"]} evalStart={EVAL_START}')
    if rows[-1]['fundingTime'] < EVAL_END-12*HOUR: raise HoldoutDataBlock(f'KuCoin funding ends too early: last={rows[-1]["fundingTime"]} cutoff={EVAL_END}')
    gaps=[]
    for a,b in zip(rows,rows[1:]):
        delta=b['fundingTime']-a['fundingTime']
        if delta > 24*HOUR: gaps.append({'left':a['fundingTime'],'right':b['fundingTime'],'hours':delta/HOUR})
    if gaps: raise HoldoutDataBlock(f'KuCoin funding contains >24h gaps: {gaps[:20]}')
...
def kucoin_source():
    temp=v20.source_for('Binance'); text=temp.read_text(); temp.unlink(missing_ok=True)
    old_warm='const WARM_START = Date.parse("2024-12-17T00:00:00Z");'; old_eval='const EVAL_START = Date.parse("2024-12-24T00:00:00Z");'
    if text.count(old_warm)!=1 or text.count(old_eval)!=1: raise HoldoutAdapterBlock('Frozen source warm/eval constants changed')
    text=text.replace(old_warm,'const WARM_START = Date.parse("2025-01-01T00:00:00Z");',1).replace(old_eval,'const EVAL_START = Date.parse("2025-01-08T00:00:00Z");',1)
    begin=text.index('async function downloadCandles('); end=text.index('function fundingBetween',begin)
    text=text[:begin]+KUCOIN_FUNCS+text[end:]
    if 'venue: "Binance"' not in text: raise HoldoutAdapterBlock('Frozen Binance venue marker missing')
    text=text.replace('venue: "Binance"','venue: "KuCoin"').replace('pengu-short-v20-binance','pengu-short-v20-kucoin-holdout')
    out=Path('scripts/.pengu_v20_kucoin_holdout.ts'); out.write_text(text); return out

def run_kucoin():
    temp=kucoin_source(); out=ROOT/'kucoin.json'; env=dict(os.environ); env['PENGU_V11_OUT']=str(out)
    try:
        cp=subprocess.run(['npx','tsx',str(temp)],env=env,text=True,stdout=subprocess.PIPE,stderr=subprocess.STDOUT)
        (ROOT/'kucoin.log').write_text(cp.stdout); print(cp.stdout)
        if cp.returncode!=0: raise HoldoutAdapterBlock(f'V20 KuCoin evaluator failed code={cp.returncode}')
...
def main():
    ...
    try:
        contracts={'pengu':verify_contract('PENGUUSDTM'),'btc':verify_contract('XBTUSDTM')}; market_data_observed=True
        data={'pengu':load_klines('PENGUUSDTM'),'btc':load_klines('XBTUSDTM'),'funding':load_funding(),'contracts':contracts}
    except HoldoutDataBlock as exc:
        payload=blocked_payload('BLOCKED_HOLDOUT_DATA',exc,market_data_observed); (ROOT/'holdout-result.json').write_text(json.dumps(payload,ensure_ascii=False,indent=2)+'\n'); print(json.dumps(payload,ensure_ascii=False,indent=2)); return
    try: result=run_kucoin()
