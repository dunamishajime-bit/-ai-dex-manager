...
RECENT_SHADOW_WINDOW = 8
MIN_RECENT_SHADOW_TRADES = 5
RECENT_SHADOW_GATED_MODE = "SOL_LINK_RECENT_SHADOW_GATED"
...
def _recent_shadow_stats(
    records: list[dict[str, Any]],
    now: int,
) -> dict[str, Any]:
    completed = [r for r in records if int(r["exitTs"]) <= now]
    recent = completed[-RECENT_SHADOW_WINDOW:]
    ytd_values = [float(r["netReturnPct"]) for r in completed]
    recent_values = [float(r["netReturnPct"]) for r in recent]
    return {
        "completedTrades": len(completed),
        "recentTrades": len(recent),
        "ytdReturnPct": compound(ytd_values),
        "ytdPf": profit_factor(ytd_values),
        "ytdPfWithoutBest": profit_factor(_without_best(ytd_values)),
        "recentReturnPct": compound(recent_values),
        "recentPf": profit_factor(recent_values),
        "recentPfWithoutBest": profit_factor(_without_best(recent_values)),
        "recentSampleSufficient": len(recent) >= MIN_RECENT_SHADOW_TRADES,
        "signalStrength": float(completed[-1]["signalStrength"]) if completed else 0.0,
    }


def _recent_shadow_gate(
    records: list[dict[str, Any]],
    now: int,
) -> tuple[bool, str, dict[str, Any]]:
    stats = _recent_shadow_stats(records, now)
    if stats["recentTrades"] < MIN_RECENT_SHADOW_TRADES:
        return True, "INSUFFICIENT_RECENT_SHADOW_SAMPLE_SIGNAL_STRENGTH", stats
    if stats["ytdReturnPct"] <= 0:
        return False, "SHADOW_CUMULATIVE_RETURN_NON_POSITIVE", stats
    if (stats["ytdPfWithoutBest"] or 0.0) < 1.0:
        return False, "YTD_PF_WITHOUT_BEST_BELOW_1", stats
    if stats["recentReturnPct"] <= 0:
        return False, "RECENT_SHADOW_RETURN_NON_POSITIVE", stats
    if (stats["recentPfWithoutBest"] or 0.0) < 1.0:
        return False, "RECENT_PF_WITHOUT_BEST_BELOW_1", stats
    return True, "RECENT_SHADOW_QUALITY_PASS", stats
...
def _rank_complements(
    ...
    ranked, _ = _rank_complements_with_decisions(
        candidates, mode, shadow_records, now
    )
    return ranked


def _rank_complements_with_decisions(
    candidates: list[dict[str, Any]],
    mode: str,
    shadow_records: dict[str, list[dict[str, Any]]],
    now: int,
    ...
    decision_by_key: dict[tuple[str, int, str], dict[str, Any]] = {}
    eligible: list[dict[str, Any]] = []

    def candidate_key(record: dict[str, Any]) -> tuple[str, int, str]:
        return (str(record["symbol"]), int(record["entryTs"]), str(record["side"]))

    for record in candidates:
        key = candidate_key(record)
        if mode == RECENT_SHADOW_GATED_MODE:
            allowed, reason, stats = _recent_shadow_gate(
                shadow_records[record["symbol"]], now
            )
            decision = {
                "symbol": record["symbol"],
                "entryTs": int(record["entryTs"]),
                "side": record["side"],
                "allowed": allowed,
                "reason": reason,
                "shadowStats": stats,
            ...
            stats = (
                ...
                else _shadow_stats(shadow_records[record["symbol"]], now)
            )
            # A single completed shadow win must not lock the router's ranking.
            if mode != "SIGNAL_STRENGTH_ONLY" and not stats["sampleSufficient"]:
                stats = {
                    "cumulativeReturnPct": 0.0,
                    "pf": 0.0,
                    "tradeCount": 0,
    ...
    def key(record: dict[str, Any]) -> tuple[float, float, float, float, str]:
        decision = decision_by_key[candidate_key(record)]
        stats = decision["shadowStats"]
        if mode == RECENT_SHADOW_GATED_MODE:
            if stats["recentTrades"] < MIN_RECENT_SHADOW_TRADES:
                # Until the recent sample is sufficient, signal strength is
                # deliberately the primary ordering key.
                return (
                    1.0,
                    float(record["signalStrength"]),
                    float(stats["completedTrades"]),
                    float(stats["ytdReturnPct"]),
                    str(record["symbol"]),
                )
            return (
                2.0,
                float(stats["recentReturnPct"]),
                float(stats["recentPfWithoutBest"] or 0.0),
                float(stats["ytdReturnPct"]),
                str(record["symbol"]),
            )
        return (
            ...
            float(stats["tradeCount"]),
            float(record["signalStrength"]),
            str(record["symbol"]),
        )

    return sorted(eligible, key=key, reverse=True), decisions
...
def run_router(
    ...
    if mode not in (
        "SHADOW_YTD_RANK",
        "SIGNAL_STRENGTH_ONLY",
        "SOL_LINK_FIXED_50_50",
        RECENT_SHADOW_GATED_MODE,
    ):
    ...
    complement_decisions: dict[tuple[str, int, str], dict[str, Any]] = {}
    ...
    for ts in times:
        ...
        complement_events_at_ts = [x for x in at_ts if x["symbol"] in COMPLEMENTS]
        active_complement_universe = (
            ACTIVE_COMPLEMENTS
            if mode == RECENT_SHADOW_GATED_MODE
            else COMPLEMENTS
        )
        complements_at_ts = [
            x for x in complement_events_at_ts
            if x["symbol"] in active_complement_universe
            and x["symbol"] not in by_symbol_open
        ]
        for candidate in complement_events_at_ts:
            key = candidate_key(candidate)
            if candidate["symbol"] not in active_complement_universe:
                complement_decisions[key] = {
                    "symbol": candidate["symbol"],
                    "entryTs": int(candidate["entryTs"]),
                    "side": candidate["side"],
                    "allowed": False,
                    "reason": "BNB_EXCLUDED_FROM_COMPLEMENT_CANDIDATES",
                }
            elif candidate["symbol"] in by_symbol_open:
                complement_decisions[key] = {
                    "symbol": candidate["symbol"],
                    "entryTs": int(candidate["entryTs"]),
                    "side": candidate["side"],
            ...
            elif mode == "SOL_LINK_FIXED_50_50":
                complement_decisions[key] = {
                    "symbol": candidate["symbol"],
                    "entryTs": int(candidate["entryTs"]),
                    "side": candidate["side"],
                    "allowed": False,
                    "reason": "FIXED_PRIORITY_SLEEVES",
                }
        ...
        rank_mode = (
            RECENT_SHADOW_GATED_MODE
            if mode == RECENT_SHADOW_GATED_MODE
            else ("SIGNAL_STRENGTH_ONLY" if mode != "SHADOW_YTD_RANK" else mode)
        )
        ranked, rank_decisions = _rank_complements_with_decisions(
            complements_at_ts,
            rank_mode,
            shadow_records,
            ts,
        ...
        for rank, candidate in enumerate(ranked):
            ...
            if candidate["symbol"] in by_symbol_open:
                ...
                complement_decisions[key]["allowed"] = False
                complement_decisions[key]["reason"] = "COMPLEMENT_ALREADY_HELD"
                continue
            if available <= 0:
                skipped["complement_no_slot"] += 1
                complement_decisions[key]["allowed"] = False
                complement_decisions[key]["reason"] = "NO_FREE_SLOT"
                continue
            slot = slot_for_candidate(candidate)
            if slot is None:
                skipped["complement_no_slot"] += 1
            ...
            available -= 1
            complement_decisions[key]["allowed"] = True
            complement_decisions[key]["reason"] = "ADOPTED"
            selected_complement_events.append(
                {
                    ...
                    "signalStrength": candidate["signalStrength"],
                    "shadowStats": (
                        _recent_shadow_stats(shadow_records[candidate["symbol"]], ts)
                        if mode == RECENT_SHADOW_GATED_MODE
                        else _shadow_stats(shadow_records[candidate["symbol"]], ts)
                    ),
                }
            )
    ...
    return {
        ...
        "complementDecisions": list(complement_decisions.values()),
        ...
        "policy": {
            ...
            "activeComplementCandidates": list(
                ACTIVE_COMPLEMENTS
                if mode == RECENT_SHADOW_GATED_MODE
                else COMPLEMENTS
            ),
            ...
            "recentShadowWindow": RECENT_SHADOW_WINDOW,
            "minimumRecentShadowTrades": MIN_RECENT_SHADOW_TRADES,
            "qualityGates": [
                "YTD_SHADOW_CUMULATIVE_RETURN_GT_0",
                "YTD_PF_WITHOUT_BEST_GTE_1",
                "RECENT_SHADOW_RETURN_GT_0",
                "RECENT_PF_WITHOUT_BEST_GTE_1",
            ] if mode == RECENT_SHADOW_GATED_MODE else [],
        },
    }
...
def missed_shadow_summary(
    ...
    decisions = {
        (str(x["symbol"]), int(x["entryTs"]), str(x["side"])): x
        for x in router.get("complementDecisions", [])
    }
    out = {}
    for symbol in COMPLEMENTS:
        ...
        for record in missed:
            ...
            decision = decisions.get(
                (symbol, int(record["entryTs"]), str(record["side"]))
            )
            enriched["notAdoptedReason"] = (
                decision.get("reason") if decision else "NOT_ADOPTED_REASON_UNAVAILABLE"
            )
            if decision and decision.get("shadowStats"):
                enriched["routerShadowStats"] = decision["shadowStats"]
...
def run_selftest() -> None:
    ...
    candidates = [
        {"symbol": "ETH", "signalStrength": 1.0, "entryTs": 2, "side": "LONG"},
        {"symbol": "BNB", "signalStrength": 4.0, "entryTs": 2, "side": "LONG"},
    ]
    ...
    good = [
        {"netReturnPct": 1.0, "exitTs": i, "signalStrength": 1.0}
        for i in range(1, MIN_RECENT_SHADOW_TRADES + 1)
    ]
    allowed, reason, _ = _recent_shadow_gate(good, 99)
    assert allowed and reason == "RECENT_SHADOW_QUALITY_PASS"
    bad = [
        {"netReturnPct": -1.0, "exitTs": i, "signalStrength": 1.0}
        for i in range(1, MIN_RECENT_SHADOW_TRADES + 1)
    ]
    allowed, reason, _ = _recent_shadow_gate(bad, 99)
    assert not allowed and reason == "SHADOW_CUMULATIVE_RETURN_NON_POSITIVE"
    allowed, reason, _ = _recent_shadow_gate(good[:2], 99)
    assert allowed and reason == "INSUFFICIENT_RECENT_SHADOW_SAMPLE_SIGNAL_STRENGTH"
...
def _run() -> dict[str, Any]:
    ...
    gated = run_router(
        RECENT_SHADOW_GATED_MODE, candles, index, periods, candidates
    )
    result = {
        "researchLine": "SOL_LINK_PRIORITY_COMPLEMENT_SHADOW_ROUTER_1Y_V2",
        ...
        "portfolios": {
            ...
            RECENT_SHADOW_GATED_MODE: gated,
        },
        "perSymbol": {
        ...
        "comparison": {
            ...
            "PRIORITY_RECENT_SHADOW_GATED": {
                "returnPct": gated["metrics"]["oneYearReturnPct"],
                "maxDDPct": gated["metrics"]["maxDrawdownHourlyMtmPct"],
                "returnToDD": gated["metrics"]["returnToAbsDrawdown"],
                "pf": gated["metrics"]["pf"],
            },
        },
        "diagnostics": {
            ...
            "recentShadowWindow": RECENT_SHADOW_WINDOW,
            "minimumRecentShadowTrades": MIN_RECENT_SHADOW_TRADES,
            "activeComplementCandidates": list(ACTIVE_COMPLEMENTS),
            "excludedComplementCandidates": ["BNB"],
            "qualityGates": [
                "YTD_SHADOW_CUMULATIVE_RETURN_GT_0",
                "YTD_PF_WITHOUT_BEST_GTE_1",
                "RECENT_SHADOW_RETURN_GT_0",
