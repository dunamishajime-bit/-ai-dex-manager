...
async function fetchArray(url:URL){
  ...
  for(let a=0;a<6;a++){
    try{
      const res=await fetch(url,{headers:{accept:"application/json","user-agent":"DisDex-PENGU-Flat1-DD-Reduction/20260924"}});
...
async function main(){
  ...
  const variants:DDMode[]=["BASELINE_FLAT1","V64_PROTECT_6_8_3","RECOVERY_PROTECT_5_12_3","SHORT_PROTECT_6_10_3","HARDSTOP_COOLDOWN48","COMBINED_PROTECT","COMBINED_PROTECT_COOLDOWN48"];
  const result:any={
    schema:"pengu-flat1-dd-reduction/v1",
    ...
    candidates:{
      V64_PROTECT_6_8_3:"BASE_V64_LONG only: hard stop 6%, trailing activates +8%, 3% retrace",
      RECOVERY_PROTECT_5_12_3:"Recovery V8 only: 50% partial at -3% after12h, hard stop -5%, trailing activates +5% with 2.5% retrace",
      SHORT_PROTECT_6_10_3:"SHORT_V20 only: hard stop 6%, trailing activates +10%, 3% retrace",
      HARDSTOP_COOLDOWN48:"entry signals unchanged; after any hard stop extend no-entry cooldown to 48h",
      COMBINED_PROTECT:"all three route-specific protections",
      COMBINED_PROTECT_COOLDOWN48:"all three protections plus 48h hard-stop cooldown"
    },
  ...
  await fs.mkdir(".research-state/pengu-flat1-dd-reduction",{recursive:true});
  await fs.writeFile(".research-state/pengu-flat1-dd-reduction/result.json",JSON.stringify(result,null,2)+"\n");
  ...
  console.log("PENGU_FLAT1_DD_REDUCTION="+JSON.stringify(compact));
  console.log("PENGU_FLAT1_DD_CONTRACT_PASS="+JSON.stringify({gross:1,logic:"COMBINED_FILTERED",variants:variants.length,baselineParity:true,researchOnly:true}));
}
