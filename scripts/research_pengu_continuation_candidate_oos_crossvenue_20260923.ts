...
async function main(){
  ...
  const result:any={schema:"pengu-continuation-candidate-oos/v1",venue:CROSS_VENUE,candidate:{ret6Min:.03,distanceAtrMax:.50,ret72Min:.15,ret72Max:.40,armHours:24},availableRange:[new Date(pengu[0].openTime).toISOString(),new Date(pengu.at(-1)!.openTime+HOUR).toISOString()],windows:{}};
  for(const mode of ["NORMAL","SEVERE"] as Mode[]){
    ...
    for(const w of windows){
      ...
      result.windows[w.label][mode]={
        baseline:metrics(btr),candidate:metrics(ctr),
        continuation:continuationLedger(ctr),
  ...
  const outDir=`.research-state/pengu-continuation-oos-cross-${CROSS_VENUE.toLowerCase()}`;
  await fs.mkdir(outDir,{recursive:true});await fs.writeFile(path.join(outDir,"result.json"),JSON.stringify(result,null,2)+"\n");
  console.log("PENGU_CONTINUATION_OOS="+JSON.stringify(result));
}
