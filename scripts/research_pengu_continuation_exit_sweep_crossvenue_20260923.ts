...
async function main(){
  ...
  const venue=CROSS_VENUE;
  const exits=[
    {label:"A04_R02",activation:.04,retrace:.02},
    {label:"A05_R02",activation:.05,retrace:.02},
    {label:"A05_R03",activation:.05,retrace:.03},
    {label:"A05_R04",activation:.05,retrace:.04},
    {label:"A06_R03",activation:.06,retrace:.03},
    {label:"A07_R03",activation:.07,retrace:.03},
    {label:"A08_R03",activation:.08,retrace:.03},
    {label:"CURRENT_A10_R03",activation:.10,retrace:.03},
  ];
  const out:any={schema:"pengu-continuation-exit-sweep/v1",venue,entryCandidate:{ret6Min:.03,distanceAtrMax:.50,ret72Min:.15,ret72Max:.40},exits:{}};
  for(const e of exits){
    CONTINUATION_TRAIL_ACTIVATION=e.activation;CONTINUATION_TRAIL_RETRACE=e.retrace;
    const row:any={activation:e.activation,retrace:e.retrace};
    for(const mode of ["NORMAL","SEVERE"] as Mode[]){
      ...
      const bf=between(base,FORMAL_START,FORMAL_END),cf=between(candidate,FORMAL_START,FORMAL_END);
      const br=between(base,FORMAL_END,RECENT_END),cr=between(candidate,FORMAL_END,RECENT_END);
      row[mode]={
        baselineFormal:metrics(bf),candidateFormal:metrics(cf),continuationFormal:continuationLedger(cf),
        baselineRecent:metrics(br),candidateRecent:metrics(cr),continuationRecent:continuationLedger(cr),
        deltaFormalReturnPct:metrics(cf).returnPct-metrics(bf).returnPct,
        deltaFormalWinRatePct:(metrics(cf).winRatePct??0)-(metrics(bf).winRatePct??0),
        deltaFormalPf:(metrics(cf).profitFactor??0)-(metrics(bf).profitFactor??0),
        deltaFormalDdPct:metrics(cf).maxDrawdownPct-metrics(bf).maxDrawdownPct,
        deltaRecentReturnPct:metrics(cr).returnPct-metrics(br).returnPct,
        deltaRecentMarkedReturnPct:metrics(cr).markToWindowReturnPct-metrics(br).markToWindowReturnPct
      };
    }
    out.exits[e.label]=row;
  }
  const outDir=`.research-state/pengu-continuation-exit-cross-${CROSS_VENUE.toLowerCase()}`;await fs.mkdir(outDir,{recursive:true});await fs.writeFile(path.join(outDir,"result.json"),JSON.stringify(out,null,2)+"\n");
  console.log("PENGU_CONTINUATION_EXIT_SWEEP="+JSON.stringify(out));
}
