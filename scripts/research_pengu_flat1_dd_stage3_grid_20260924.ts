...
for(const longHard of [.04,.045,.05,.055,.06]){
  for(const shortHard of [.045,.05,.055,.06]){
    for(const recoveryHard of [.04,.045,.05]){
      PROFILES.push({
        id:`GRID_L${Math.round(longHard*1000)}_S${Math.round(shortHard*1000)}_R${Math.round(recoveryHard*1000)}`,
...
async function main(){
  ...
  const result:any={schema:"pengu-flat1-dd-deep-search/stage3-grid",fixed:{logic:"COMBINED_FILTERED",maximumGross:1,allEntriesGross:1},profiles:{},safety:{researchOnly:true,ordersSent:false,liveChanged:false}};
