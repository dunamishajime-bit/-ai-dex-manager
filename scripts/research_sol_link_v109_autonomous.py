...
# Frozen V109 remains reference-only. All successors keep the frozen model,
# threshold, risk and trail and change architecture only.
CATALOG = {
    'SOL': [
...
def _ctx(pair, candles, idx, ts):
    ...
    v96 = v109.b.vol(c, i, 96)
    v168 = v109.b.vol(c, i, 168)
    v336 = v109.b.vol(c, i, 336)
    if min(v96, v168, v336) <= 1e-12:
        return None
    br = v109.b.breadth(candles, idx, ts, 24)
    r3 = v109.ret(c, i, 3) or 0.0
    r6 = v109.ret(c, i, 6) or 0.0
    r12 = v109.ret(c, i, 12) or 0.0
    r24 = v109.ret(c, i, 24) or 0.0
    return {
        'i': i, 'close': float(c[i]['close']), 'v24': v24, 'v96': v96,
        'v168': v168, 'v336': v336, 'vr': v24 / v96, 'breadth': br,
        'r3': r3, 'r6': r6, 'r12': r12, 'r24': r24,
    }
...
def simulate(cid, pair, candles, idx, start, end, cost_bps, delay, model):
    ...
    entry_i = entry_ts = signal_ts = None
    entry_pred = None
    vals, recs = [], []
    ...
    def close(ts, i, flags):
        ...
        recs.append({
            ...
            'exitExecTs': int(c[xi]['ts']), 'entryPredictor': entry_pred,
            'threshold': th, 'entryPrice': entry, 'exitPrice': xp,
            'heldHours': (ts - signal_ts) / HOUR, 'netPnlPct': pnl,
            'grossReturnPct': gross, 'costContributionPct': raw_cost * v109.RISK[pair],
            'mfePct': mfe, 'maePct': mae, 'pnl0To48hPct': p0_48,
            'pnl48hToExitPct': pnl - p0_48, 'exitReason': '+'.join(flags),
            'volRatio24_96': regs.get('vr'), 'breadth24': regs.get('breadth'),
            'entryLifecycle': 'ACCEPTED_CORE',
        })
        vals.append(pnl)
    ...
    for row in c:
        ...
        if ctx is None:
            continue
        i = ctx['i']
        pr = v109.predict(KIND, pair, candles, idx, ts, model)
        px = ctx['close']
        ...
        if pair == 'SOL' and state == 0:
            if life == 'CASH' and d and frozen_gate:
                probe_side = d; probe_ts = ts; life = 'SHADOW_PROBE'
                continue
            if life == 'SHADOW_PROBE':
                ...
                # Structural acceptance uses existing V109 fast horizons; no sweep.
                ...
                if elapsed >= min_h and _entry_ok(cid, probe_side, ctx, prev):
                    ei = i + 1 + delay
                    if ei < len(c):
                        state = probe_side; signal_ts = ts; entry_i = ei
                        entry_ts = int(c[ei]['ts']); entry = float(c[ei]['open'])
                        peak = entry; trough = entry; entry_pred = pr; life = 'CORE'
                    continue
                if elapsed > 12:
                    life = 'CASH'; probe_side = 0; probe_ts = None
            continue
        ...
        if pair == 'LINK' and state == 0 and armed and d and frozen_gate:
            ...
            if _entry_ok(cid, d, ctx, prev):
                ei = i + 1 + delay
                if ei < len(c):
                    state = d; signal_ts = ts; entry_i = ei
                    entry_ts = int(c[ei]['ts']); entry = float(c[ei]['open'])
                    peak = entry; trough = entry; entry_pred = pr; life = 'CORE'
...
def summary(recs):
    ...
    m.update({
        'netSumPct': sum(vals),
        'medianMfePct': med([r['mfePct'] for r in recs]),
...
def diagnose(pair, result):
    d, v, s = result['development'], result['validation'], result['validationStress']
    if pair == 'SOL':
        if (v.get('worstTradePct') or 0) < -4:
            return 'TAIL_LOSS_REMAINS'
        if v.get('trades', 0) < 6:
            return 'TOO_SELECTIVE'
        if (v.get('pf') or 0) < 1:
            return 'WRONG_WAVE_REJECTION_INSUFFICIENT'
        return 'WRONG_WAVE_REJECTION_IMPROVED'
    if (v.get('sumPnl48hToExitPct') or 0) < 0:
...
def candidate_result(cid, pair, candles, idx, ps, model):
    ...
    res = {
        'candidate': cid, 'pair': pair, 'development': summary(dr),
        'validation': summary(vr), 'validationStress': summary(sr),
        'walkForward': {
            'development': folds(cid, pair, candles, idx, ps['development'], model),
...
def run(pair):
    ...
    # Frozen parity baseline uses only Development/Validation; Confirmation/Holdout are never read.
    frozen = {}
    ...
    for cid in CATALOG[pair]:
        r = candidate_result(cid, pair, candles, idx, ps, model)
        chain.append(r)
        # A robust survivor is frozen immediately; no further variants are generated after success.
        if r['status'] == 'FROZEN_SURVIVOR':
            break

    best = max(chain, key=lambda x: (
        min(x['development'].get('pf') or 0, x['validation'].get('pf') or 0),
        x['validation'].get('returnPct', -999),
    ))
    next_spec = None if best['status'] == 'FROZEN_SURVIVOR' else {
        'sourceDiagnosis': best['diagnosis'],
        'policy': 'NEXT_RUN_MUST_USE_NEW_STRUCTURAL_MECHANISM_NOT_NUMERIC_RETUNE',
        'SOL': 'wrong-wave rejection: change causal acceptance/rejection mechanism only',
        'LINK': 'volatility-aware cash plus 48h forecast ownership/re-arm mechanism only',
    }

    out = {
        'researchLine': 'FROZEN_V109_SOL_LINK_AUTONOMOUS_STRUCTURAL',
        'pair': pair, 'frozenV109Changed': False, 'kind': KIND,
        'frozenThreshold': model['threshold'], 'frozenRisk': v109.RISK[pair],
        'frozenTrailPct': v109.TRAIL[pair], 'forecastHorizonHours': HORIZON,
        ...
        'antiOverfit': {
            'denseSweep': False, 'thresholdRetune': False, 'sideHardcode': False,
            'confirmationRead': False, 'holdoutRead': False,
            'designEvidence': ['development', 'validation'],
        },
    ...
    lines = [f'# {pair} Frozen V109 Autonomous Structural Research', '',
             f'- Frozen V109 changed: NO', f'- selected: {best["candidate"]}', f'- status: {best["status"]}',
             f'- diagnosis: {best["diagnosis"]}', '']
    for r in chain:
...
if __name__ == '__main__':
    ap = argparse.ArgumentParser(); ap.add_argument('--pair', choices=PAIRS, required=True)
