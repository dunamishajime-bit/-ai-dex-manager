...
const PROFILES:RiskProfile[]=[{id:"BASELINE"}];
for(const longHard of [.035,.04,.045]){
  for(const shortHard of [.04,.045,.05]){
    for(const recoveryHard of [.035,.04,.045]){
      PROFILES.push({
        id:`LOWGRID_L${Math.round(longHard*1000)}_S${Math.round(shortHard*1000)}_R${Math.round(recoveryHard*1000)}`,
        longHard,shortHard,recoveryHard
      });
    }
  }
}
PROFILES.push(
  {id:"OPT_L40_S45_R40",longHard:.04,shortHard:.045,recoveryHard:.04},
  {id:"OPT_CD48",longHard:.04,shortHard:.045,recoveryHard:.04,hardStopCooldownHours:48},
  {id:"OPT_CD72",longHard:.04,shortHard:.045,recoveryHard:.04,hardStopCooldownHours:72},
  {id:"OPT_STREAK2_72",longHard:.04,shortHard:.045,recoveryHard:.04,lossStreakThreshold:2,lossStreakCooldownHours:72},
  {id:"OPT_STREAK2_96",longHard:.04,shortHard:.045,recoveryHard:.04,lossStreakThreshold:2,lossStreakCooldownHours:96},
  {id:"BAL_L40_S50_R40",longHard:.04,shortHard:.05,recoveryHard:.04},
  {id:"BAL_STREAK2_72",longHard:.04,shortHard:.05,recoveryHard:.04,lossStreakThreshold:2,lossStreakCooldownHours:72},
  {id:"PROFIT_L50_S55_R40",longHard:.05,shortHard:.055,recoveryHard:.04},
  {id:"PROFIT_STREAK2_72",longHard:.05,shortHard:.055,recoveryHard:.04,lossStreakThreshold:2,lossStreakCooldownHours:72}
);
...
async function main(){
  ...
  const result:any={schema:"pengu-flat1-dd-deep-search/stage4-final",fixed:{logic:"COMBINED_FILTERED",maximumGross:1,allEntriesGross:1},profiles:{},safety:{researchOnly:true,ordersSent:false,liveChanged:false}};
  ...
  await fs.writeFile(".research-state/pengu-flat1-dd-deep-search/stage4-final.json",JSON.stringify(result,null,2)+"\n");
  console.log("PENGU_FLAT1_DD_STAGE4="+JSON.stringify({ranked:ranked.slice(0,20).map((x:any)=>({id:x.id,profile:result.profiles[x.id].profile,score:x.score,formal:{N:x.formal.NORMAL,S:x.formal.SEVERE},rolling:{N:x.rolling.NORMAL,S:x.rolling.SEVERE},folds:x.formal.folds})),baselineDd:{formal:base.windows.FORMAL.ddEpisodeNormal,rolling:base.windows.ROLLING365.ddEpisodeNormal}}));
}
