...
const STRATEGIES = [
  {
    key: "ranking_only_bio_zbt_runner72",
    rules: [
      { minWindowHours: 0, maxWindowHours: Infinity, symbols: ["BIO", "ZBT"] as const, variant: VARIANTS.runner72 },
    ],
  },
  {
...
function simulate(candlesBySymbol: Map<string, Candle1h[]>, windows: readonly Window[], strategy: typeof STRATEGIES[number]) {
  ...
  const sortedTs = [...tsSet].sort((left, right) => left - right);
  const trades: Trade[] = [];
  let open: null | (Trade & { peakPrice: number; troughPrice: number; maxExitTs: number; activeWindowEndTs: number; variantRef: Variant }) = null;
  let windowIndex = 0;
...
function summarizeTrades(trades: Trade[], baselineEndEquity: number, periodHours: number, baselineCashPct: number) {
  ...
  const reductionPt = Math.min(baselineCashPct, (addedDays * 24 / periodHours) * 100);
  return {
    ...
    cap300Pnl: round(pnl),
    cap300End: round(baselineEndEquity + pnl),
    addedDays: round(addedDays, 2),
    cashAfterPct: round(Math.max(0, baselineCashPct - reductionPt), 2),
...
async function main() {
  ...
  for (const period of PERIODS) {
    const baseline = await runHybridBacktest("RETQ22", { ...baseOptions(period), label: `v7_ranking_window_${period.key}` });
    const windows = cashWindowsFromBaseline(baseline);
    const candles = await loadCandles(period.startTs, period.endTs);
    const baselineCashPct = 100 - baseline.summary.exposure_pct;
    const periodHours = (period.endTs - period.startTs + 1) / HOUR_MS;
    for (const strategy of STRATEGIES) {
      const trades = simulate(candles, windows, strategy);
      const summary = summarizeTrades(trades, baseline.summary.end_equity, periodHours, baselineCashPct);
      rows.push({
        period: period.key,
        strategy: strategy.key,
        v7End: round(baseline.summary.end_equity),
        v7MaxDdPct: round(baseline.summary.max_drawdown_pct),
        v7Trades: baseline.summary.trade_count,
        v7CashPct: round(baselineCashPct, 2),
        cashWindows: windows.length,
        ...summary,
      });
      tradeRows.push(...trades.map((trade) => ({ period: period.key, strategy: strategy.key, ...trade })));
      console.log(`${period.key} ${strategy.key}: trades=${summary.trades} cap300=${summary.cap300Pnl} cash ${round(baselineCashPct, 2)} -> ${summary.cashAfterPct}`);
    }
  }
  ...
  const md = [
    ...
    "",
    "- method: engine-direct V7 cash windows + ranked 1h sidecar simulation",
    "- cap: 300 USDT",
    "- ranking_only: BIO/ZBT runner_72h on every USDT window",
    "- window_regime: switch ranking rules by idle-window length",
