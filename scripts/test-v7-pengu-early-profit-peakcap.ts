...
const REPORT_DIR = path.join(process.cwd(), "reports", "v7-pengu-early-profit-peakcap");
...
async function main() {
  ...
  for (const maxPeakProfitPct of [0.04, 0.05, 0.06, 0.08, 0.1, 0.12, 0.15]) {
    variants.push({
      key: `peakcap${maxPeakProfitPct}`,
      options: earlyProtect({ maxPeakProfitPct }),
    });
    for (const maxMom80 of [0.05, 0.08]) {
      variants.push({
        key: `peakcap${maxPeakProfitPct}_m80${maxMom80}`,
        options: earlyProtect({ maxPeakProfitPct, maxMom80 }),
      });
    }
    for (const minLongHighDrawdownPct of [0.2, 0.3]) {
      variants.push({
        key: `peakcap${maxPeakProfitPct}_longdd${minLongHighDrawdownPct}`,
        options: earlyProtect({
          maxPeakProfitPct,
          minLongHighDrawdownPct,
          longHighDrawdownLookbackBars: 960,
        }),
      });
    }
  }
  ...
  for (const period of periods) {
    ...
    for (const variant of variants) {
      ...
      const result = await runHybridBacktest("RETQ22", {
        ...
        label: `v7_peng_early_peakcap_${period.key}_${variant.key}`,
      });
  ...
  const lines = [
    "# V7 PENGU Early Profit Peak Cap",
    "",
