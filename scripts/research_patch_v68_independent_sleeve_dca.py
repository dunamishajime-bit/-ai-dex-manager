...
def once(old: str, new: str, label: str) -> None:
    global src
    if old not in src:
        raise SystemExit(f'V68 DCA marker missing: {label}')
...
once(
    '"limits": {"v12Positions": V12_MAX_POSITIONS, "v12Gross": V12_GROSS_CAP, "penguGross": PENGU_MAX_GROSS, "cryptoGross": CRYPTO_GROSS_CAP,',
    '"limits": {"v12Positions": V12_MAX_POSITIONS, "v12Gross": V12_GROSS_CAP, "penguGross": PENGU_MAX_GROSS + PENGU_NEW_LONG_MAX_GROSS, "penguCoreGross": PENGU_MAX_GROSS, "penguNewLongGross": PENGU_NEW_LONG_MAX_GROSS, "cryptoGross": CRYPTO_GROSS_CAP,',
    'gross verification limits',
)
once(
...
once(
    ...
    '        new_trades = [] if pengu_new is None else pengu_new["modes"][mode]["trades"]\n        results[scenario] = simulate(v12["modes"][mode]["trades"], pengu["modes"][mode]["trades"], new_trades, v11_rows, v50_rows, finite(assumptions["stockCostBps"]))\n',
    'simulate call',
)
once(
...
once(
    '"penguGrossCap": PENGU_MAX_GROSS, "sharedCryptoGrossCap": CRYPTO_GROSS_CAP,',
    '"penguGrossCap": PENGU_MAX_GROSS, "penguNewLongGrossCap": PENGU_NEW_LONG_MAX_GROSS, "sharedCryptoGrossCap": CRYPTO_GROSS_CAP,',
    'architecture gross cap',
)
once(
    '"entryPriority": ["V52", "PENGU_DUAL_LS_V2", "V12"],',
    '"entryPriority": ["V52", "PENGU_DUAL_LS_V2", "PENGU_NEW_LONG", "V12"],',
    'architecture priority',
)
once(
    '"penguNormalSourceMetrics": pengu["modes"]["normal"]["metrics"]},\n',
    '"penguNormalSourceMetrics": pengu["modes"]["normal"]["metrics"], "penguNewLongNormalSourceMetrics": None if pengu_new is None else pengu_new["modes"]["normal"]["metrics"]},\n',
    'source metrics',
)

# Policy marker consumed by the RED/GREEN contract; final adjudication is in the workflow.
src += '\n# V68 independent-sleeve contract: materialIntegratedGain >= MATERIAL_GAIN_MULTIPLE (1.05)\n'
TARGET.write_text(src)
print(f'PATCHED_V68_DCA={TARGET} bytes={TARGET.stat().st_size}')
