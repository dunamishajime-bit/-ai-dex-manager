...
async function fetchArray(url:URL){
  ...
  for(let a=0;a<6;a++){
    try{
      const res=await fetch(url,{headers:{accept:"application/json","user-agent":"DisDex-PENGU-Current-Wave-Audit/20260924"}});
...
async function main(){
  ...
  const formalTrades=between(baselineAll,FORMAL_START,FORMAL_END);
  const rollingStart=RECENT_END-365*24*HOUR;
  const rollingTrades=between(baselineAll,rollingStart,RECENT_END);
  const formal=waveAuditWindow(rows,formalTrades,FORMAL_START,FORMAL_END);
  const rolling=waveAuditWindow(rows,rollingTrades,rollingStart,RECENT_END);
  const result={
    schema:"pengu-current-directional-wave-audit/v1",
    source:{venue:"ASTER_FUTURES_V3",productionSourceSha:process.env.PRODUCTION_SOURCE_SHA||null},
    definition:{
      wave:"close-based directional-change waves; threshold rise/fall confirms a wave and same-threshold reversal terminates it",
      captured:"same-direction current PENGU baseline position overlaps the wave and less than 5% directional move remains after its last exit before the wave extreme",
      partialExitMiss:"same-direction position existed, but >=5% move remained after its last exit",
      fullMiss:"no same-direction current PENGU position overlapped the wave",
      thresholdsPct:[10,15,20,30],
      triggerSnapshotsPct:[3,5],
    },
    baseline:{formal:metrics(formalTrades),rolling365d:metrics(rollingTrades)},
    windows:{formal,rolling365d:rolling},
    safety:{researchOnly:true,ordersSent:false,liveChanged:false,vpsChanged:false,productionChanged:false},
  };
  await fs.mkdir(".research-state/pengu-current-directional-wave-audit",{recursive:true});
  await fs.writeFile(".research-state/pengu-current-directional-wave-audit/result.json",JSON.stringify(result,null,2)+"\n");
  const compact={
    baseline:result.baseline,
    formal:{range:[formal.startIso,formal.endIso],thresholds:Object.fromEntries(Object.entries(formal.byThreshold).map(([k,v]:any)=>[k,v.summary])),major15Missed:formal.major15Missed},
    rolling365d:{range:[rolling.startIso,rolling.endIso],thresholds:Object.fromEntries(Object.entries(rolling.byThreshold).map(([k,v]:any)=>[k,v.summary])),major15Missed:rolling.major15Missed},
  };
  console.log("PENGU_CURRENT_WAVE_AUDIT="+JSON.stringify(compact));
}
