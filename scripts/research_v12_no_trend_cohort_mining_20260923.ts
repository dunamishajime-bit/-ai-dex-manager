...
async function main(){
  ...
  const trades=(baseline.NORMAL.trades||[]).filter((x:any)=>!x.isHC);
  const thirds=[START,START+(END-START)/3,START+2*(END-START)/3,END];
  const cohort=(name:string,pred:(x:any)=>boolean)=>{
    const xs=trades.filter(pred);
    const gp=xs.filter((x:any)=>x.net>0).reduce((a:number,x:any)=>a+x.net,0),gl=-xs.filter((x:any)=>x.net<0).reduce((a:number,x:any)=>a+x.net,0);
    return {name,n:xs.length,wins:xs.filter((x:any)=>x.net>0).length,losses:xs.filter((x:any)=>x.net<0).length,wr:xs.length?100*xs.filter((x:any)=>x.net>0).length/xs.length:0,pf:gl?gp/gl:gp?99:0,net:xs.reduce((a:number,x:any)=>a+x.net,0),avgPct:xs.length?xs.reduce((a:number,x:any)=>a+x.pct,0)/xs.length:0,thirds:[0,1,2].map(k=>{const q=xs.filter((x:any)=>x.entryTs>=thirds[k]&&x.entryTs<thirds[k+1]);return {n:q.length,w:q.filter((x:any)=>x.net>0).length,l:q.filter((x:any)=>x.net<0).length,net:q.reduce((a:number,x:any)=>a+x.net,0)};})};
  };
  const rev=(x:any,e=.08)=>x.btcEr12<e&&x.btc12h<0&&(x.btc6h<0||x.ret6h<0);
  const groups:any[]=[];
  for(const e of [.065,.08,.10]){
    groups.push(cohort("REV_"+e,x=>rev(x,e)));
    for(const route of ["NORMAL_SCORE","STRONG_REGIME_ALT","RELAXED_MOMENTUM_ALT"])groups.push(cohort("REV_"+e+"_"+route,x=>rev(x,e)&&x.route===route));
    for(const rank of [1,2,3])groups.push(cohort("REV_"+e+"_R"+rank,x=>rev(x,e)&&x.rank===rank));
    for(const [lo,hi] of [[0,.25],[.25,.35],[.35,.5],[.5,99]])groups.push(cohort("REV_"+e+"_S"+lo+"_"+hi,x=>rev(x,e)&&x.score>=lo&&x.score<hi));
    groups.push(cohort("REV_"+e+"_RELNEG",x=>rev(x,e)&&x.rel24h<0));
    groups.push(cohort("REV_"+e+"_VOLLT1",x=>rev(x,e)&&x.volumeRatio<1));
    groups.push(cohort("REV_"+e+"_NO_BREAK",x=>rev(x,e)&&x.breakout12<0&&x.reclaim6<1));
    groups.push(cohort("REV_"+e+"_ALT_LOW",x=>rev(x,e)&&x.route!=="NORMAL_SCORE"&&x.score<.35));
  }
  console.error("COHORTS="+JSON.stringify(groups));
  console.error("BASE="+JSON.stringify({NORMAL:{finalEquity:baseline.NORMAL.finalEquity,winRatePct:baseline.NORMAL.winRatePct,profitFactor:baseline.NORMAL.profitFactor,maxDrawdownPct:baseline.NORMAL.maxDrawdownPct,hcStats:baseline.NORMAL.hcStats},SEVERE:{finalEquity:baseline.SEVERE.finalEquity,winRatePct:baseline.SEVERE.winRatePct,profitFactor:baseline.SEVERE.profitFactor,maxDrawdownPct:baseline.SEVERE.maxDrawdownPct,hcStats:baseline.SEVERE.hcStats}}));
  const out={status:"PASS_RESEARCH_ONLY_COHORT_MINING",groups,baseline};
