...
const SYMBOLS = ["BIO", "DUSK", "PENDLE", "DEXE"] as const;
...
const VARIANTS = {
  ...
  confirmed48: { key: "rank_confirmed_48h", maxHoldHours: 48, lookback: 10, breakoutPct: 0.016, minVolRatio: 1.22, minMom6: 0.045, minMom24: 0.075, minFourHourMom: 0.05, minScore: 32, maxOneHourJump: 0.2, minCloseLocation: 0.6, trailActivationPct: 0.18, trailRetracePct: 0.085, hardStopPct: 0.08, weakExitMinHours: 8 },
  ...
  slow240: { key: "rank_slow_240h", maxHoldHours: 240, lookback: 36, breakoutPct: 0.012, minVolRatio: 0.8, minMom6: -0.02, minMom24: 0.02, minFourHourMom: 0.02, minScore: 9, maxOneHourJump: 0.18, minCloseLocation: 0.42, trailActivationPct: 0.28, trailRetracePct: 0.14, hardStopPct: 0.16, weakExitMinHours: 36 },
} as const;

const STRATEGIES = [
  { key: "ranking_only_bio_dusk_runner72", rules: [{ minWindowHours: 0, maxWindowHours: Infinity, symbols: ["BIO", "DUSK"] as const, variant: VARIANTS.runner72 }] },
  {
    key: "ranking_window_regime_bio_dusk",
    rules: [
      { minWindowHours: 12, maxWindowHours: 72, symbols: ["BIO", "DUSK"] as const, variant: VARIANTS.fast24 },
      { minWindowHours: 48, maxWindowHours: 336, symbols: ["BIO", "DUSK"] as const, variant: VARIANTS.runner72 },
      { minWindowHours: 336, maxWindowHours: Infinity, symbols: ["BIO", "DUSK"] as const, variant: VARIANTS.runner72 },
    ],
  },
  {
    key: "ranking_window_regime_with_bio_dusk",
    rules: [
      { minWindowHours: 12, maxWindowHours: 72, symbols: ["BIO", "DUSK"] as const, variant: VARIANTS.fast24 },
      { minWindowHours: 48, maxWindowHours: 336, symbols: ["BIO", "DUSK"] as const, variant: VARIANTS.confirmed48 },
      { minWindowHours: 336, maxWindowHours: Infinity, symbols: ["BIO", "DUSK", "PENDLE"] as const, variant: VARIANTS.runner72 },
    ],
  },
  {
    key: "ranking_window_regime_with_dexe_long",
    rules: [
      { minWindowHours: 12, maxWindowHours: 72, symbols: ["BIO", "DUSK"] as const, variant: VARIANTS.fast24 },
      { minWindowHours: 48, maxWindowHours: 336, symbols: ["BIO", "DUSK"] as const, variant: VARIANTS.confirmed48 },
      { minWindowHours: 336, maxWindowHours: Infinity, symbols: ["DEXE", "PENDLE"] as const, variant: VARIANTS.slow240 },
    ],
  },
] as const;

type Variant = typeof VARIANTS[keyof typeof VARIANTS];
type Window = { startTs: number; endTs: number; startIso?: string; endIso?: string };
type Signal = { symbol: string; ts: number; close: number; score: number; variant: Variant; windowHours: number };
type Trade = { symbol: string; variant: string; entryTs: number; exitTs: number; entryPrice: number; exitPrice: number; netReturnPct: number; score: number; windowHours: number; exitReason: string };
...
function simulate(candlesBySymbol: Map<string, Candle1h[]>, windows: readonly Window[], strategy: typeof STRATEGIES[number]) {
  ...
  for (const ts of [...tsSet].sort((left, right) => left - right)) {
    ...
    if (open) {
      ...
      trades.push({ symbol: open.symbol, variant: open.variant, entryTs: open.entryTs, exitTs: ts, entryPrice: open.entryPrice, exitPrice: bar.close, netReturnPct: bar.close / open.entryPrice - 1 - (quoteLossPct / 100) * 2 - FEE_RATE * 2, score: open.score, windowHours: open.windowHours, exitReason });
      open = null;
      continue;
    }
    ...
    open = { symbol: best.symbol, variant: best.variant.key, entryTs: ts, exitTs: ts, entryPrice: best.close, exitPrice: best.close, netReturnPct: 0, score: best.score, windowHours: best.windowHours, exitReason: "open", peakPrice: best.close, maxExitTs, activeWindowEndTs: window.endTs, variantRef: best.variant };
  }
  return trades;
}
...
async function main() {
  ...
  const rows = [];
  const tradeRows: Array<{ period: string; strategy: string } & Trade> = [];
  ...
  for (const period of PERIODS) {
    ...
    for (const strategy of STRATEGIES) {
      const trades = simulate(periodCandles, windows, strategy);
      const summary = summarize(trades, period.v7End, periodHours, cashPct);
      rows.push({ period: period.key, strategy: strategy.key, v7End: period.v7End, v7CashPct: round(cashPct, 2), cashWindows: windows.length, ...summary });
      tradeRows.push(...trades.map((trade) => ({ period: period.key, strategy: strategy.key, ...trade })));
      console.log(`${period.key} ${strategy.key}: trades=${summary.trades} cap300=${summary.cap300Pnl} cash ${round(cashPct, 2)} -> ${summary.cashAfterPct}`);
    }
  }
  ...
  const md = [
    "# V7 Ranking + Idle Window Regime Sidecar",
    "",
    "- method: cached engine-direct V7 USDT windows + ranked 1h sidecar simulation",
    "- source windows: reports/v7-idle-candidates-fresh/result.json",
    "- cap: 300 USDT",
    "- quote value loss is charged on entry and exit",
    "",
    "| period | strategy | V7 end | V7 USDT % | USDT after % | reduction pt | trades | win % | PF | avg net % | cap300 PnL | cap300 End | added days | by symbol |",
    "| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- |",
    ...sortedRows.map((row) => `| ${row.period} | ${row.strategy} | ${row.v7End} | ${row.v7CashPct} | ${row.cashAfterPct} | ${row.cashReductionPt} | ${row.trades} | ${row.winPct} | ${row.pf} | ${row.avgNetPct} | ${row.cap300Pnl} | ${row.cap300End} | ${row.addedDays} | ${JSON.stringify(row.bySymbol)} |`),
    "",
    "## Best By Period",
    "",
    ...PERIODS.map((period) => {
      const best = rows.filter((row) => row.period === period.key).sort((left, right) => right.cap300Pnl - left.cap300Pnl)[0];
      return `- ${period.key}: ${best?.strategy ?? "-"} cap300 PnL ${best?.cap300Pnl ?? 0}, End ${best?.cap300End ?? 0}, USDT ${best?.v7CashPct ?? 0}% -> ${best?.cashAfterPct ?? 0}%`;
    }),
  ].join("\n");
