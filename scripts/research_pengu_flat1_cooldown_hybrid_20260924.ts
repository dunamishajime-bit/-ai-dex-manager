...
async function fetchArray(url:URL){
  ...
  for(let a=0;a<6;a++){
    try{
      const res=await fetch(url,{headers:{accept:"application/json","user-agent":"DisDex-PENGU-Flat1-Cooldown-Hybrid/20260924"}});
...
type CooldownMode =
  ...
  | "HYBRID_L48_R36_S48"
  | "HYBRID_L48_R36_S60";
let cooldownMode:CooldownMode="BASE_24";
function hardStopCooldownFor(route:EntryRoute,exitReason:string,base:number){
  if(!exitReason.includes("HARD_STOP"))return base;
  if(cooldownMode==="BASE_24")return base;
  if(cooldownMode==="ALL_36")return Math.max(base,36);
  if(cooldownMode==="ALL_60")return Math.max(base,60);
  let hours=36;
  if((cooldownMode==="HYBRID_L48_R36_S36"||cooldownMode==="HYBRID_L48_R36_S48"||cooldownMode==="HYBRID_L48_R36_S60")&&route==="BASE_V64_LONG")hours=48;
  if((cooldownMode==="HYBRID_L36_R36_S48"||cooldownMode==="HYBRID_L48_R36_S48")&&route==="SHORT_V20")hours=48;
  if(cooldownMode==="HYBRID_L48_R36_S60"&&route==="SHORT_V20")hours=60;
  return Math.max(base,hours);
}
...
function replay(rows:PenguDualLsV2EvaluationRow[],points:FundingPoint[],v:Variant,mode:Mode){
  ...
  while(i<rows.length-2){
    ...
    const researchCooldown=hardStopCooldownFor(route,exitReason,baseCooldown);
...
async function main(){
  ...
  const variants:CooldownMode[]=["BASE_24","ALL_36","ALL_60","HYBRID_L48_R36_S36","HYBRID_L36_R36_S48","HYBRID_L48_R36_S48","HYBRID_L48_R36_S60"];
  const result:any={
    schema:"pengu-flat1-cooldown-hybrid/v1",
    source:{venue:"ASTER_FUTURES_V3",productionSourceSha:process.env.PRODUCTION_SOURCE_SHA||null,logic:"COMBINED_FILTERED fixed"},
    fixedContract:{penguMaximumGross:1.0,everyEntryGross:1.0,entryExitLogic:"unchanged except hard-stop cooldown overlay"},
    candidates:{
      BASE_24:"current 24h hard-stop cooldown",
      ALL_36:"36h all hard stops",
      ALL_60:"60h all hard stops",
      HYBRID_L48_R36_S36:"Base V64 48h, Recovery 36h, Short 36h",
      HYBRID_L36_R36_S48:"Base V64 36h, Recovery 36h, Short V20 48h",
      HYBRID_L48_R36_S48:"Base V64 48h, Recovery 36h, Short V20 48h",
      HYBRID_L48_R36_S60:"Base V64 48h, Recovery 36h, Short V20 60h"
    },
  ...
  for(const w of windows){
    const cases:any={};
    for(const candidate of variants){
      const normal=between(cache[`${candidate}:NORMAL`],w.start,w.end),severe=between(cache[`${candidate}:SEVERE`],w.start,w.end);
      cases[candidate]={NORMAL:metrics(normal),SEVERE:metrics(severe),decompositionNormal:drawdownDecomposition(normal),decompositionSevere:drawdownDecomposition(severe),folds:w.name==="FORMAL"?{NORMAL:formalFolds(normal),SEVERE:formalFolds(severe)}:undefined};
    }
    const base=cases.BASE_24;
    for(const candidate of variants.slice(1)){
      const row=cases[candidate];row.deltaVsBaseline={
        normalReturnPct:row.NORMAL.returnPct-base.NORMAL.returnPct,severeReturnPct:row.SEVERE.returnPct-base.SEVERE.returnPct,
        normalPf:(row.NORMAL.profitFactor??0)-(base.NORMAL.profitFactor??0),severePf:(row.SEVERE.profitFactor??0)-(base.SEVERE.profitFactor??0),
        normalDdPct:row.NORMAL.maxDrawdownPct-base.NORMAL.maxDrawdownPct,severeDdPct:row.SEVERE.maxDrawdownPct-base.SEVERE.maxDrawdownPct,
        normalTrades:row.NORMAL.trades-base.NORMAL.trades,severeTrades:row.SEVERE.trades-base.SEVERE.trades};
    }
    result.windows[w.name]={start:new Date(w.start).toISOString(),end:new Date(w.end).toISOString(),cases};
  }
  const formal=result.windows.FORMAL.cases.BASE_24,rolling=result.windows.ROLLING365.cases.BASE_24;
  approx(formal.NORMAL.returnPct,1307.9130447891462,"formal baseline");approx(formal.SEVERE.returnPct,769.7792392486292,"formal severe");
  approx(rolling.NORMAL.returnPct,978.2039888080867,"rolling baseline");approx(rolling.SEVERE.returnPct,550.8813814669275,"rolling severe");
  await fs.mkdir(".research-state/pengu-flat1-cooldown-hybrid",{recursive:true});
  await fs.writeFile(".research-state/pengu-flat1-cooldown-hybrid/result.json",JSON.stringify(result,null,2)+"\n");
  const compact=Object.fromEntries(Object.entries(result.windows).map(([window,value]:any)=>[window,Object.fromEntries(variants.map(candidate=>[candidate,{NORMAL:value.cases[candidate].NORMAL,SEVERE:value.cases[candidate].SEVERE,delta:value.cases[candidate].deltaVsBaseline,ddNormal:value.cases[candidate].decompositionNormal,ddSevere:value.cases[candidate].decompositionSevere,folds:value.cases[candidate].folds}]))]));
  console.log("PENGU_FLAT1_COOLDOWN_HYBRID="+JSON.stringify(compact));
  console.log("PENGU_FLAT1_COOLDOWN_HYBRID_PASS="+JSON.stringify({gross:1,logic:"COMBINED_FILTERED",baselineParity:true,researchOnly:true}));
}
