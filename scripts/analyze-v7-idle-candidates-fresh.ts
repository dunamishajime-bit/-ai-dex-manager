...
const REPORT_DIR = path.join(process.cwd(), "reports", "v7-idle-candidates-fresh");
...
async function main() {
  ...
  const baseline = await runHybridBacktest("RETQ22", {
    ...baseOptions(),
    label: "base_v7_idle_candidate_fresh_scan",
  });
  ...
  for (const symbol of CANDIDATES) {
    try {
      ...
      // Skip symbols without usable candle history.
    }
  }
