...
const CROSS_VENUE = String(process.env.PENGU_CROSS_VENUE || "ASTER").toUpperCase();
...
async function getJson(url:URL){
  ...
  for(let a=0;a<6;a++){
    try{
      const res=await fetch(url,{headers:{accept:"application/json","user-agent":"DisDex-PENGU-Continuation-CrossVenue/20260923"}});
      if(!res.ok)throw new Error(`HTTP ${res.status}: ${(await res.text()).slice(0,180)}`);
...
async function asterCandles(symbol:string){
  const map=new Map<number,DisDexV35Candle>();let cursor=WARM_START;
  while(cursor<RECENT_END){
    const u=new URL("https://fapi.asterdex.com/fapi/v3/klines");
    u.searchParams.set("symbol",symbol);u.searchParams.set("interval","1h");u.searchParams.set("startTime",String(cursor));u.searchParams.set("endTime",String(RECENT_END-1));u.searchParams.set("limit","1500");
    const rows=await getJson(u);if(!Array.isArray(rows)||!rows.length)break;
    let last=cursor;
    for(const r of rows){
      const openTime=Number(r[0]);last=Math.max(last,openTime);
      const c={openTime,open:Number(r[1]),high:Number(r[2]),low:Number(r[3]),close:Number(r[4]),volume:Number(r[5]),closeTime:Number(r[6]??openTime+HOUR-1)};
      if(openTime>=WARM_START&&openTime<RECENT_END&&Object.values(c).every(Number.isFinite))map.set(openTime,c);
    }
    const next=last+HOUR;if(!(next>cursor))break;cursor=next;await sleep(45);
  }
  return [...map.values()].sort((a,b)=>a.openTime-b.openTime);
}
async function bitgetCandles(symbol:string){
  const map=new Map<number,DisDexV35Candle>();let cursor=WARM_START;
  while(cursor<RECENT_END){
    const end=Math.min(RECENT_END-1,cursor+199*HOUR);
    const u=new URL("https://api.bitget.com/api/v2/mix/market/history-candles");
    u.searchParams.set("symbol",symbol);u.searchParams.set("productType","usdt-futures");u.searchParams.set("granularity","1H");u.searchParams.set("startTime",String(cursor));u.searchParams.set("endTime",String(end));u.searchParams.set("limit","200");
    const x=await getJson(u);if(x?.code!=="00000"||!Array.isArray(x?.data))throw new Error(`Bitget code=${x?.code}`);
    ...
    cursor=end+1;await sleep(35);
  }
  return [...map.values()].sort((a,b)=>a.openTime-b.openTime);
}
async function okxCandles(symbol:string){
  const inst=symbol==="PENGUUSDT"?"PENGU-USDT-SWAP":"BTC-USDT-SWAP";
  const map=new Map<number,DisDexV35Candle>();let cursor=RECENT_END;
  for(let page=0;page<260;page++){
    const u=new URL("https://www.okx.com/api/v5/market/history-candles");
    u.searchParams.set("instId",inst);u.searchParams.set("bar","1H");u.searchParams.set("after",String(cursor));u.searchParams.set("limit","100");
    const x=await getJson(u);const rows=Array.isArray(x?.data)?x.data:[];
    if(!rows.length)break;let oldest=Number.POSITIVE_INFINITY;
    for(const r of rows){
      const openTime=Number(r[0]);oldest=Math.min(oldest,openTime);
      const confirmed=r.length<9||String(r[8])==="1";
      const c={openTime,open:Number(r[1]),high:Number(r[2]),low:Number(r[3]),close:Number(r[4]),volume:Number(r[5]),closeTime:openTime+HOUR-1};
      if(confirmed&&openTime>=WARM_START&&openTime<RECENT_END&&Object.values(c).every(Number.isFinite))map.set(openTime,c);
    }
    if(!Number.isFinite(oldest)||oldest<=WARM_START)break;cursor=oldest-1;await sleep(45);
  }
  return [...map.values()].sort((a,b)=>a.openTime-b.openTime);
}
async function gateCandles(symbol:string){
  ...
  while(cursor<RECENT_END){
    ...
    u.searchParams.set("contract",contract);u.searchParams.set("from",String(Math.floor(cursor/1000)));u.searchParams.set("to",String(Math.floor((end-1)/1000)));u.searchParams.set("interval","1h");u.searchParams.set("timezone","utc0");
    const rows=await getJson(u);if(!Array.isArray(rows))throw new Error("Gate response not array");
    for(const r of rows){
      const openTime=Number(r.t)*1000;const c={openTime,open:Number(r.o),high:Number(r.h),low:Number(r.l),close:Number(r.c),volume:Number(r.v??r.sum??0),closeTime:openTime+HOUR-1};
      if(openTime>=WARM_START&&openTime<RECENT_END&&Object.values(c).every(Number.isFinite))map.set(openTime,c);
    }
    cursor=end;await sleep(80);
  }
  return [...map.values()].sort((a,b)=>a.openTime-b.openTime);
}
async function candles(symbol:string){
  if(CROSS_VENUE==="ASTER")return asterCandles(symbol);
  if(CROSS_VENUE==="BITGET")return bitgetCandles(symbol);
  if(CROSS_VENUE==="OKX")return okxCandles(symbol);
  if(CROSS_VENUE==="GATE")return gateCandles(symbol);
  throw new Error(`unsupported cross venue ${CROSS_VENUE}`);
}
...
let MOM_RET6_MIN=0.03;
let MOM_DIST_ATR_MAX=0.40;
function continuationRoute(rows:readonly PenguDualLsV2EvaluationRow[],i:number,v:Variant,arm?:ContinuationArm):EntryRoute|undefined{
  ...
  if(v==="TRAIL_MOMENTUM"){
    ...
    return ret6(rows,i)>=MOM_RET6_MIN&&distanceAtr<=MOM_DIST_ATR_MAX?"TRAIL_MOMENTUM_LONG":undefined;
  }
...
async function main(){
  ...
  assert.equal(pengu.length,btc.length);assert.ok(pengu.length>=8000,`insufficient common rows ${pengu.length}`);
  console.log(`PENGU_GRID_RANGE venue=${CROSS_VENUE} ${new Date(pengu[0].openTime).toISOString()}..${new Date(pengu.at(-1)!.openTime+HOUR).toISOString()} rows=${pengu.length}`);
  ...
  const baselineAll=replay(rows,fp,"BASELINE","NORMAL"),baselineFormal=between(baselineAll,FORMAL_START,FORMAL_END),baselineRecent=between(baselineAll,FORMAL_END,RECENT_END);
  const severeBaseAll=replay(rows,fp,"BASELINE","SEVERE"),severeBaseFormal=between(severeBaseAll,FORMAL_START,FORMAL_END);
  const retGrid=[0.02,0.025,0.03,0.035,0.04],distGrid=[0.25,0.40,0.55,0.70];
  const configs:any[]=[];
  for(const r6 of retGrid)for(const dist of distGrid){
    MOM_RET6_MIN=r6;MOM_DIST_ATR_MAX=dist;
    const nAll=replay(rows,fp,"TRAIL_MOMENTUM","NORMAL"),sAll=replay(rows,fp,"TRAIL_MOMENTUM","SEVERE");
    const formal=between(nAll,FORMAL_START,FORMAL_END),recent=between(nAll,FORMAL_END,RECENT_END),severe=between(sAll,FORMAL_START,FORMAL_END);
    const cont=formal.filter(t=>t.route==="TRAIL_MOMENTUM_LONG"),closed=cont.filter(t=>!t.openAtWindowEnd);
    const current=recent.filter(t=>t.route==="TRAIL_MOMENTUM_LONG"&&t.entryTs>=Date.parse("2026-09-23T00:00:00Z"));
    const fm=metrics(formal),sm=metrics(severe),bm=metrics(baselineFormal),sbm=metrics(severeBaseFormal);
    configs.push({ret6Min:r6,distanceAtrMax:dist,formal:fm,severeFormal:sm,continuation:{trades:closed.length,wins:closed.filter(t=>t.accountReturn>0).length,losses:closed.filter(t=>t.accountReturn<=0).length,winRatePct:closed.length?closed.filter(t=>t.accountReturn>0).length/closed.length*100:null,accountReturnPct:closed.reduce((a,t)=>a+t.accountReturn,0)*100,ledger:continuationLedger(cont)},currentSep23Triggered:current.length>0,currentSep23:continuationLedger(current),delta:{returnPct:fm.returnPct-bm.returnPct,winRatePct:(fm.winRatePct??0)-(bm.winRatePct??0),pf:(fm.profitFactor??0)-(bm.profitFactor??0),ddPct:fm.maxDrawdownPct-bm.maxDrawdownPct,severeReturnPct:sm.returnPct-sbm.returnPct,severePf:(sm.profitFactor??0)-(sbm.profitFactor??0),severeDdPct:sm.maxDrawdownPct-sbm.maxDrawdownPct}});
  }
  const result={schema:"pengu-trail-momentum-sensitivity/v1",venue:CROSS_VENUE,range:{start:new Date(pengu[0].openTime).toISOString(),endExclusive:new Date(pengu.at(-1)!.openTime+HOUR).toISOString(),rows:pengu.length},baseline:{normal:metrics(baselineFormal),severe:metrics(severeBaseFormal),recent:metrics(baselineRecent)},grid:configs,safety:{researchOnly:true,ordersSent:false,liveChanged:false,vpsChanged:false,productionChanged:false}};
  const outDir=`.research-state/pengu-trail-momentum-grid-${CROSS_VENUE.toLowerCase()}`;await fs.mkdir(outDir,{recursive:true});await fs.writeFile(path.join(outDir,"result.json"),JSON.stringify(result,null,2)+"\n");
  console.log("PENGU_MOMENTUM_GRID="+JSON.stringify(result));
}
