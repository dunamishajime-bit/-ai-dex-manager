...
const REPORT_DIR = path.join(process.cwd(), "reports", "v7-other-idle-symbols-standalone");
...
function buildSignal(candles: Candle1h[], index: number, fourHourCandles: Candle1h[], variant: typeof VARIANTS[number]) {
  ...
  const score = mom6 * 120 + mom24 * 80 + fourHourMom * 110 + breakoutPct * 160 + Math.min(3.5, volRatio) * 2;
...
function summarize(trades: Trade[], capUsd = 300) {
  ...
  return { trades: trades.length, winPct: round((wins.length / Math.max(1, trades.length)) * 100), pf: losses > 0 ? round(gains / losses, 3) : gains > 0 ? 999 : 0, pnl: round(pnl, 2), days: round(hours / 24, 2) };
}
...
async function main() {
  ...
  for (const period of PERIODS) {
    const baseline = await runHybridBacktest("RETQ22", { ...baseOptions(period), label: `v7_other_${period.key}` });
    const windows = cashWindowsFromBaseline(baseline);
    const v7CashPct = 100 - baseline.summary.exposure_pct;
    for (const symbol of SYMBOLS) {
      const candles = (await loadCandles(symbol, period.startTs, period.endTs)).filter((bar) => bar.ts >= period.startTs - 160 * HOUR_MS && bar.ts <= period.endTs);
      for (const variant of VARIANTS) {
        const trades = simulate(symbol, candles, windows, variant);
        const summary = summarize(trades);
        rows.push({ period: period.key, symbol, variant: variant.key, v7CashPct: round(v7CashPct, 2), ...summary });
      }
  ...
  const md = [
    "# V7 Other Idle Symbols Standalone",
    "",
  ...
  console.log(JSON.stringify({ report: path.join(REPORT_DIR, "result.md"), top: rows.slice(0, 20) }, null, 2));
}
