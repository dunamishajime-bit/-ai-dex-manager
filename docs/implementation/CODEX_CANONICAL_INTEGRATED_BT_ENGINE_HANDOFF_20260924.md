...
## 2026-09-26 supplemental user source: Gross1 six-way PENGU study

A *separate, successful* source-checked research commit, `research/pengu-gross1-fixed-production-target-20260924` at `007ed8f9ccd775e5b3bb9e5348b829abc7911503`, has real [CI success 35920417464](https://github.com/dunamishajime-bit/-ai-dex-manager/actions/runs/35920417464). It validates six **PENGU-STANDALONE** allocation candidates, all maximum Gross1, including (i) `CAP1_ALL_OLD_ORDERS_LINEAR`: each OLD 0.85-cap order ×(1/0.85), clamped at Gross1, preserving variable entry sizing; and (ii) `CAP1_EVERY_ENTRY_FLAT`: every new entry Gross1, Recovery half-defence closes 0.5. Formal-year standalone NORMAL: linear +820.07% / PF4.638 / DD-14.01%, flat +1307.91% / PF3.979 / DD-20.82%; SEVERE: linear +550.39% / PF3.684 / DD-16.09%, flat +769.78% / PF3.147 / DD-22.60%. These outcomes are **not** five-sleeve integrated results. Separately, adding source-causal Q60 same-route hard-stop and PENGU-only realized DD17/H72 to FLAT1 changes its standalone signal path to NORMAL 69 / SEVERE 68 trades; this is NOT the same as the six-way unguarded flat1 study.

Read [the addendum](../research-results/PENGU_GROSS1_SIX_WAY_ADDITIONAL_SOURCE_20260926.md) and [machine-readable four-arm source contract](../../scripts/research/canonical_integrated_bt/pengu_variant_comparison_contract_20260926.json) before writing adapters. Once — and only once — the ORIGINAL formal 740,771,278.01/65,669,109.00 JPY historical anchor achieves exact global event parity, keep its untouched baseline and evaluate `CAP1_ALL_OLD_ORDERS_LINEAR`, `CAP1_EVERY_ENTRY_FLAT` and `FLAT1_Q60_DD170_H72` as three **distinct independent from-zero integrated portfolios**. Do not infer that the six-way 71 standalone trades equal the Q60 69/68 or the actual integrated admission counts. If original original-anchor causal ledgers/data/allocator remain missing, retain `BLOCKED_MISSING_CANONICAL_SOURCE` and report the new study as source-verified standalone only. This addendum does not modify the G0–G4 acceptance gates or authorize LIVE modifications.


## 2026-09-26 V12 HC1.75 / BTC Fast regime / post-loss cooldown reference

Supplemental user V12-only research is documented in [V12 HC175 reference and five-sleeve experiment plan](../research-results/V12_HC175_FAST_REGIME_LOSS_COOLDOWN_REFERENCE_20260926.md). This is a DIFFERENT standalone test path from the published original Top3+FET+Q102 Governor+PENGU+V52 740,771,278.01 JPY five-sleeve anchor; the former observed HC175-only 872 trades / 481 wins / 391 losses and ~JPY1.043m standalone NORMAL ending assets, and may postdate the anchor. In source `e1b58060d6263a3af7ced51bec854d3e211d2f35`, Sep23 HC175 is already coded as a high-confidence entry Gross multiplier, but the Sep24 canonical branch source configuration predates that gate. Before using ANY of the 872 per-trade observations to claim adapter parity, locate the exact report/source commit and frozen price/cache identity; do not silently transplant newer 1.75x overlays into the older 740m anchor.

**After** original G0/G1 proven, keep existing G2/G3 PENGU comparisons separate and add fully independent V12 sensitivity arms: HC175 only (when not in original), NON-HC narrow adverse BTC12h AND BTC24h AND previous-closed 2h volume ratio>1.0 block, same-symbol 6h NON-HC cooldown after accepted realized net loss with HC override, mild adverse 0.50 vs 0.75 of requested Gross, conditional severe adverse+hot-volume+relative-to-BTC24h<0 block, and fractional sizing combined with cooldown. User's V12-only reference: HC175-only NORMAL JPY1,043,269 PF4.014 DD-6.42%; narrow hard-block JPY1,034,329 PF4.063 DD-6.42% (but slightly better separate severe ending equity); 6h cooldown JPY1,042,797 PF4.227 DD-5.50%; naive hard-block+cooldown JPY1,020,687. No assumption that removing losing trades improves compounding portfolio equity; report blocked **future winners** as well as avoided losers and downstream FET/Q102/PENGU/V52 capacity effects. HC bypass is only for strategy-local cooldown, NEVER global Kill Switch, margin, source-data fail-closed, or hard portfolio gross limits. Do not invent omitted numeric thresholds for BTC6h Fast classification or claim source-verified backtest artifacts from the user-provided summary.

Original G0 source and original G1 anchor parity remain mandatory; research documents are **not** evidence of successful five-sleeve backtesting. No LIVE changes.
